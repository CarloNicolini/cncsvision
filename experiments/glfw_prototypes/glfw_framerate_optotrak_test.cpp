...
{
    initOptotrak();
    GLFWwindow *window = initOpenGLContext(SCREEN_WIDTH,SCREEN_HEIGHT,SCREEN_REFRESH_RATE,false,false,1);
...
#define COLLECTION_TIME 1000
    ...
    while ( frame < COLLECTION_TIME )
    {
        ...
        GLText text(1024,768,glBlack);
        text.enterTextInputMode();
        for (int i=1; i<20; i++)
        {
            stringstream marker;
            marker <<  markers[i].p.transpose() << endl;
            text.draw(marker.str());
        }
