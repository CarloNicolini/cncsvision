...
Vector3d showBtmMidBoard(415.2, (-323.8+(153.3*2)), -601.8);
Vector3d showBtmBoard(415.2, (-323.8+(153.3*3)), -601.8);
...
BalanceFactor<double> trial; //make trial a vector if you want to have blocks
...
void initTrial()
{
	...
	currentObj = trial.getCurrent().at("ObjPosn");
	...
	if (!quickStart)
	{
		switch (currentObj)
		{
			case TOP_OBJL:
			case TOP_OBJR:
				RoveretoMotorFunctions::moveObjectAbsolute(showTopBoard,motorHomePosn,5000);
				break;
			case TOPMID_OBJL:
			case TOPMID_OBJR:
				RoveretoMotorFunctions::moveObjectAbsolute(showTopMidBoard,motorHomePosn,5000);
				break;
			case BTMMID_OBJL:
			case BTMMID_OBJR:
				RoveretoMotorFunctions::moveObjectAbsolute(showBtmMidBoard,motorHomePosn,5000);
				break;
			case BTM_OBJL:
			case BTM_OBJR:
				RoveretoMotorFunctions::moveObjectAbsolute(showBtmBoard,motorHomePosn,5000);
				break;
		}
	}
	//else // Annoying nested switches for relative movement
	//{
	//	switch (previousObj)
	//	{
	//		case TOP_OBJL:
	...
	//				case TOPMID_OBJL:
	//				case TOPMID_OBJR:
	//					RoveretoMotorFunctions::moveObject(moveUpOne,4000);
	//					break;
	//				case BTMMID_OBJL:
	//				case BTMMID_OBJR:
	//					RoveretoMotorFunctions::moveObject(moveUpTwo,4000);
	//					break;
	//				case BTM_OBJL:
	//				case BTM_OBJR:
	//					RoveretoMotorFunctions::moveObject(moveUpThree,4000);
	//					break;
	//			}
	//			break;
	//		case TOPMID_OBJL:
	...
	//				case TOP_OBJL:
	//				case TOP_OBJR:
	//					RoveretoMotorFunctions::moveObject(moveDownOne,4000);
	//					break;
	//				case TOPMID_OBJL:
	//				case TOPMID_OBJR:
	//					//do nothing
	...
	//				case TOP_OBJL:
	//				case TOP_OBJR:
	//					RoveretoMotorFunctions::moveObject(moveDownTwo,4000);
	//					break;
	//				case TOPMID_OBJL:
	//				case TOPMID_OBJR:
	//					RoveretoMotorFunctions::moveObject(moveDownOne,4000);
	...
	//				case TOP_OBJL:
	//				case TOP_OBJR:
	//					RoveretoMotorFunctions::moveObject(moveDownThree,4000);
	//					break;
	//				case TOPMID_OBJL:
	//				case TOPMID_OBJR:
	//					RoveretoMotorFunctions::moveObject(moveDownTwo,4000);
    ...
    {
        case TOP_OBJL:
            target_height = top_objL_height;
			con_inc = 2;
			sm_lg = 2;
            break;
        case TOP_OBJR:
            target_height = top_objR_height;
			con_inc = 2;
			sm_lg = 1;
            break;
        case TOPMID_OBJL:
            target_height = topmid_objL_height;
			con_inc = 1;
			sm_lg = 2;
            break;
        case TOPMID_OBJR:
            target_height = topmid_objR_height;
			con_inc = 1;
			sm_lg = 1;
            break;
        case BTMMID_OBJL:
            target_height = btmmid_objL_height;
			con_inc = 1;
			sm_lg = 1;
            break;
        case BTMMID_OBJR:
            target_height = btmmid_objR_height;
			con_inc = 1;
			sm_lg = 2;
            break;
        case BTM_OBJL:
            target_height = btm_objL_height;
			con_inc = 2;
			sm_lg = 1;
            break;
        case BTM_OBJR:
            target_height = btm_objR_height;
			con_inc = 2;
			sm_lg = 2;
            break;
    }
	...
	string trialFileName = "C:/Documents and Settings/Fulvio/My Documents/Google Drive/ponzoGraspRovereto/" + parameters.find("SubjectName") + "/" + parameters.find("SubjectName") + "_trial_" + stringify<int>(trialNumber) + ".txt";
...
void advanceTrial()
{
	...
	/*
		...
		if (block_num < num_blocks)
		{
			...
			responseFile << fixed << "subjName\ttrialN\ttrialDuration\tobjPosn\tobjSize\tobjX\tobjY\tobjZ\tvisualSizeDifference\tblock" << endl;
...
void idle()
{
	...
	if (fingerCalibrationDone==3 && !finished) // After we've calibrated
	{
		...
		grip_distanceToTarget = sqrt((x_dist*x_dist)+(z_dist*z_dist));

        // If hand is in flight
		if (!reachedObject && started) {
			// TGA Threshold
			if ( (grip_distanceToTarget <= 10) && (grip_aperture < (target_height+10)))
			{
				reachedObject = true;
				TGA_frame = frameNumber;
...
void initStreams()
{
	...
	parametersFile.open("C:/cncsvisiondata/parametersFiles/Evan/ponzoGraspRovereto/parametersPonzoGraspRovereto.txt"); //is this correct?
	...
	string dirName  = "C:/Documents and Settings/Fulvio/My Documents/Google Drive/ponzoGraspRovereto/" + subjectName;
	mkdir(dirName.c_str());

	// Response file
    string responseFileName =  "C:/Documents and Settings/Fulvio/My Documents/Google Drive/ponzoGraspRovereto/" + subjectName + "/ponzoGraspRovereto_" + subjectName + ".txt";
