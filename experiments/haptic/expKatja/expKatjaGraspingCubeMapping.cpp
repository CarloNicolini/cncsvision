...
glsl::glShaderManager SM[2];
glsl::glShader *shader[2];
bool attached=true;
bool showWire=true;
bool useShaderWithShininess=false;
bool rotationOnYAxis=false;
bool rotationOfEnvironment=false;
double frame=0.0;
...
void drawSomething()
{
	...
	if (attached)
	{
		...
		{
			Vector3d t = rigidStart.getFullTransformation().translation();
			glTranslated(0,0,focalDistance);
			glRotated(toDegrees( rigidStart.getYaw() ), 0.0,1.0,0.0);
		}
	}
	else
	{
		...
		glTranslated(0,0,focalDistance);
		double k=3;
		glRotated(45*sin(6.28*frame/(100.0*k))+45,0.0,1.0,0.0);
	}

	
	Vector3d eyeDirection = headEyeCoords.getRigidStart().getFullTransformation().linear()*Vector3d(0,0,-1);
	if ( !useShaderWithShininess )
	{
		shader[0]->begin();
		shader[0]->setUniform3f("eyeDirection",eyeDirection.x(),eyeDirection.y(),eyeDirection.z());
		//if (!attached)
	...
	{
		shader[1]->begin();
		shader[1]->setUniform3f("eyeDirection",eyeDirection.x(),eyeDirection.y(),eyeDirection.z());
		//if (!attached)
		//	shader->setUniform1f("Time",frame/100.0);
		glScaled(2.0,1.0,1.0);
		glutSolidSphere(30,50,50);
...
{   
	...
	case 'l':
	{
		useShaderWithShininess=!useShaderWithShininess;
	}
	break;
	}
}
...
{
    shader[0] = SM[0].loadfromFile((char*)"../../data/glslshaders/CubeMapOptotrak.vert", (char*)"../../data/glslshaders/CubeMapOptotrak.frag");
	shader[1] = SM[1].loadfromFile((char*)"../../data/glslshaders/CubeMapOptotrakADS.vert", (char*)"../../data/glslshaders/CubeMapOptotrakADS.frag");
}
