...
// Egg rigidbody motion variables
RigidTransformation<6> rigidAux;
vector < Marker > eggMarkers;
bool leastMarkersVisible=false;
bool objectCalibrationDone=false;
...
double hz=8.8;
/******* FUNCTIONS ***********************************/
bool initObjectReferencePoints()
{
	vector<Marker> startPoints(6);

	startPoints[0] = markers[24];
	startPoints[1] = markers[21];
	startPoints[2] = markers[23];
	startPoints[3] = markers[14];
	startPoints[4] = markers[10];
	startPoints[5] = markers[22];

	bool allVisible=true;
	Vector3d centroid(0,0,0);
	for (int i=0; i<6; i++)
	{
		allVisible = allVisible && startPoints[i].isVisible();
		centroid+=startPoints[i].p;	
	}
	centroid/=6;
	for (int i=0; i<6; i++)
	{
		startPoints[i].p-=centroid;	
	}

	if ( allVisible )
		rigidAux.init( startPoints );
	return allVisible;
}
...
void drawInfo()
{
    if ( infoDraw )
    {
        ...
        {
			text.draw(util::stringify<int>((int)leastMarkersVisible));
        }
        break;
        }
	...
	if ( infoDraw && ! objectCalibrationDone )
	{
		GLText text;
        if ( gameMode )
            text.init(SCREEN_WIDTH,SCREEN_HEIGHT,glWhite,GLUT_BITMAP_HELVETICA_18);
            ...
            text.init(640,480,glWhite,GLUT_BITMAP_HELVETICA_12);
        text.enterTextInputMode();
		text.draw("Move the object slowly until all points are visible, when this message will disappear calibrations is done");
		objectCalibrationDone = initObjectReferencePoints();
		text.leaveTextInputMode();
	}
}
...
void idle()
{
	...
	eggMarkers.resize(6);
	eggMarkers[0] = markers[24];
	eggMarkers[1] = markers[21];
	eggMarkers[2] = markers[23];
	eggMarkers[3] = markers[14];
	eggMarkers[4] = markers[10];
	eggMarkers[5] = markers[22];

	if ( objectCalibrationDone )
	leastMarkersVisible = rigidAux.update(eggMarkers,true);
...
{   
    ...
    if ( optotrak.init("../../src/optotrak/cameraFiles/Aligned20110823",24) != 0)
    {   cleanup();
        exit(0);
    }
