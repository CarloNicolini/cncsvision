...
void advanceTrial()
{
    ...
    } else
    {
		...
		double grip_aperture = sqrt(pow(index.x() - thumb.x(), 2) + pow(index.y() - thumb.y(), 2) + pow(index.z() - thumb.z(), 2));
        ...
        if ( (invisibleIndexFrames / ((double)drawingTrialFrame) > maxInvisibleFramesPercent ) ||
			 ...
			 (grasping==0 && grip_aperture > 35.0) ||
			 (grasping==1 && grip_aperture < 20.0))
        {
			good_trial = 0;
...
void idle()
{
    ...
    if (headCalibrationDone==3 && fingerCalibrationDone==4)
    {
        ...
        thumbProjectedInside = feedback && ( thumbProjection.x() >= 0 && thumbProjection.x() <= SCREEN_WIDTH ) && ( thumbProjection.y() >= 0 && thumbProjection.y() <= SCREEN_HEIGHT );
		Vector2d indexProjection = cam.computeProjected(index);
        indexProjectedInside = feedback && ( indexProjection.x() >= 0 && indexProjection.x() <= SCREEN_WIDTH ) && ( indexProjection.y() >= 0 && indexProjection.y() <= SCREEN_HEIGHT );
        frameN++;
...
void drawStimulus()
{
	...
	if(!grasping)
	{
		...
		//{
			glPushMatrix();
			glTranslated(0.0, 12.5, objdepth/2.0);
			glutSolidSphere(3.0,10,10);
			glPopMatrix();
		/*} else
		{
			glPushMatrix();
			glTranslated(0.0, (objdepth/2.0)+5.0, -objdepth/2.0);
			glutSolidSphere(3.0,10,10);
			glPopMatrix();
		}*/
	}
	...
	//Back surface
	glBegin(GL_QUADS);
	glVertex3d(-objwidth,-(objdepth/2.0)-10.0, -(objdepth/2.0)-6.0);
	glVertex3d(-objwidth, (objdepth/2.0)+10.0, -(objdepth/2.0)-6.0);
	glVertex3d(10.0, (objdepth/2.0)+10.0, -(objdepth/2.0)-6.0);
	glVertex3d(10.0,-(objdepth/2.0)-10.0, -(objdepth/2.0)-6.0);
	glEnd();
/*	
	// Left rear rod
	glPushMatrix();
	glTranslated(-objwidth, 0, -(objdepth/2.0)-6.0);
	stimDrawer[1].draw();
	glPopMatrix();
*/
	// Front rod
...
void calculate_object_3Dproperties()
{
	...
	{
		if( factors.getCurrent().at("AbsDepth") == 0.0 || 
			factors.getCurrent().at("AbsDepth") == 40.0 || 
			factors.getCurrent().at("AbsDepth") == 80.0 ||
			factors.getCurrent().at("AbsDepth") == 120.0 || 
