...
bool allVisibleIndex=false, allVisibleThumb=false;
...
void initProjectionScreen(double _focalDist, const Affine3d &_transformation, bool synchronous)
{
    ...
    cam.init(screen);
	if ( synchronous )
		RoveretoMotorFunctions::moveScreenAbsolute(_focalDist,homeFocalDistance,4500);
	else
		RoveretoMotorFunctions::moveScreenAbsoluteAsynchronous(_focalDist,homeFocalDistance,4500);
}
...
void idle()
{
	...
	allVisibleHead = allVisiblePatch && isVisible(markers[4].p);
	...
	if ( allVisibleIndex )
	{
		indexCoords.update(markers[7].p, markers[8].p, markers[9].p );
		viewingFrames=1;
	...
	if ( allVisibleThumb )
	{
		indexCoords.update(markers[11].p, markers[12].p, markers[13].p );
		viewingFrames=1;
		occludedFrames = occludedFrames;
	}
...
void initMotors()
{
	RoveretoMotorFunctions::homeEverything(3500,3500,3500);
}
...
int main(int argc, char*argv[])
{
    ...
    {   glutInitWindowSize( 640,480 );
        glutCreateWindow("EXP SPRING 13 STAIRCASE POINTING 2");
//glutFullScreen();
    }
    else
