...
char *parametersFileNames[] ={
	"C:/cncsvisiondata/parametersFiles/Robert/expGraspingTwoFingers/parametersExpRobertTwoFingers_nohaptic_training.txt",
	"C:/cncsvisiondata/parametersFiles/Robert/expGraspingTwoFingers/parametersExpRobertTwoFingers_nohaptic.txt"	};
/********* SOUNDS ********/
#ifdef _WIN32
/***** SOUND THINGS *****/
...
void advanceTrial()
{
    ...
    else // trialMode == STIMULUSMODE
    {
        ...
        totalTrialNumber++;
		double oldScreenDistance = factors.getCurrent().at("Distances");
        factors.next();
        generateSphereStimulus( factors.getCurrent().at("StimulusRadius") );
        initProjectionScreen(factors.getCurrent().at("Distances"));
		if ( oldScreenDistance == factors.getCurrent().at("Distances") )
		{
			RoveretoMotorFunctions::moveScreenAbsolute(factors.getCurrent().at("Distances")-20,homeFocalDistance,3500);
			RoveretoMotorFunctions::moveScreenAbsolute(factors.getCurrent().at("Distances"),homeFocalDistance,3500);
		}
		/*
		isSaving=false;
		idle();
		//if ( oldScreenDistance == factors.getCurrent().at("Distance") )
		Timer pauseTimer; pauseTimer.sleep(2000);
		idle();
		isSaving=true;
...
void idle()
{
    ...
    if (headCalibrationDone==3 && fingerCalibrationDone==3)
    {
        ...
        if (trialMode == STIMULUSMODE )
        {
            ...
            {
					boost::thread trialBeepBack(beepBack);
                    advanceTrial();
            }
