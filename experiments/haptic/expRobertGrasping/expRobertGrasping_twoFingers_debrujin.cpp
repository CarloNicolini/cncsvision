...
char *parametersFileNames[] ={
	"C:/cncsvisiondata/parametersFiles/Robert/expGraspingTwoFingers/parametersTwoFingers_DeBrujin_training.txt",
	"C:/cncsvisiondata/parametersFiles/Robert/expGraspingTwoFingers/parametersTwoFingers_DeBrujin.txt"};
/********* SOUNDS ********/
#ifdef _WIN32
/***** SOUND THINGS *****/
...
**/
double moveStimulusObject(const Vector3d& v, int speed)
{
	...
	double constantOffsetX=-4;
	//double constantOffsetY=-64;
	double constantOffsetY=-4; // paletto parte anteriore
	//double constantOffsetZ=0;
	double constantOffsetZ=23; // paletto parte anteriore
	
	Vector3d finalPosition=v+Vector3d(constantOffsetX,constantOffsetY,constantOffsetZ -(markerThickness+rodRadius/2));
	...
	Timer coveredTimer; coveredTimer.start();
	while (!isVisible(markers.at(5).p))
	{
		// Also this call updates marker 5
		idle();
	}
	double coveredMarker5Time=coveredTimer.getElapsedTimeInMilliSec();
    ...
    for ( int i=0; i<2; i++)
    {
		...
		Vector3d delta = finalPosition - markers.at(5).p;
		RoveretoMotorFunctions::moveObject(delta,speed);
    }
	isSaving=true; //riattiva la scrittura su file
	return coveredMarker5Time;
}
...
void advanceTrial()
{
    ...
    else // trialMode == STIMULUSMODE
    {
		...
		paintGL();
		beepTrial();
		oldDistance = factors.getCurrent().at("Distances");
		...
		*/
		...
		Timer xTimer;xTimer.start();
		double marker5z = markers.at(5).p.z();
		generateSphereStimulus( factors.getCurrent().at("StimulusRadius") );
		//Beep(440,1000);
        Timer trialLengthTimer;trialLengthTimer.start();
		initProjectionScreen(factors.getCurrent().at("Distances"));
		// visualStimCenter is modified and updated by generateSphereStimulus
        double coveredMarker5Time = moveStimulusObject(visualStimCenter,3500);
		double effectiveTrialTime = trialLengthTimer.getElapsedTimeInMilliSec()-coveredMarker5Time;
		double maxPossibleTime =1000*abs(maxCoveredDistance)/(ROVERETO_MOTORSTEPSIZE*3500)*1.4;
		// messo 1.4 perchè sennò si rischia di prendere differenze negative
		/*
		cerr << "== Old D = " << oldDistance << " new D= " << factors.getCurrent().at("Distances")  << " [mm]" << endl;	
		cerr << "== DeltaD = " << factors.getCurrent().at("Distances") - oldDistance << " [mm]" << endl;
		cerr << "== Trial took " << effectiveTrialTime << " [ms]" << endl;
		cerr << "== Max Possible time= " << maxPossibleTime << " [ms]" << endl;
		cerr << "== Time to wait= " << maxPossibleTime - effectiveTrialTime << " [ms]" << endl;	
		*/
		if ( effectiveTrialTime > maxPossibleTime)
			maxPossibleTime = effectiveTrialTime;
		Timer sleepTimer; sleepTimer.sleep((maxPossibleTime - effectiveTrialTime));	
		//cout << "TOTAL BLACK TIMER " << xTimer.getElapsedTimeInMilliSec() << endl;
    }
}
...
void idle()
{
	...
	/*
	if ( !isVisible(markers.at(11).p) || !isVisible(markers.at(12).p) || !isVisible(markers.at(13).p) )
		thumb = Vector3d(-9999,-9999,-9999);
	if ( !isVisible(markers.at(7).p) || !isVisible(markers.at(8).p) || !isVisible(markers.at(9).p) )
		index = Vector3d(-9999,-9999,-9999);
	*/
    ...
    if (headCalibrationDone==3 && fingerCalibrationDone==3)
    {
		...
		{
			if ((int)parameters.get("AudioFeedback")==1)
        ...
        double gain = factors.getCurrent().at("Gain");
        if ( d.norm() > clamp )
        {
...
void initProjectionScreen(double _focalDist)
{
	...
	RoveretoMotorFunctions::moveScreenAbsoluteAsynchronous(_focalDist,homeFocalDistance,3500);
}
...
void initStreams()
{
	...
	if ( util::fileExists( markersFileName ) )
    {
		MessageBox(NULL, (LPCSTR)"MARKER FILE ALREADY EXISTS\n",NULL, NULL);
		exit(0);
    }
	else
    ...
    markersFile << fixed << setprecision(3) <<
                "SubjectName\tFingerDist\tTrialNumber\tTrialFrame\tTotTime\tVisualStimX\tVisualStimY\tVisualStimZ\tfStimulusRadius\tfDistances\tfGain\tEyeLeftXraw\tEyeLeftYraw\tEyeLeftZraw\tEyeRightXraw\tEyeRightYraw\tEyeRightZraw\tWristXraw\tWristYraw\tWristZraw\tThumbXraw\tThumbYraw\tThumbZraw\tIndexXraw\tIndexYraw\tIndexZraw\tVisualThumbXraw\tVisualThumbYraw\tVisualThumbZraw\tVisualIndexXraw\tVisualIndexYraw\tVisualIndexZraw\tIsDrawing\tIsThumbProjected\tIsIndexProjected\tFingersLeaning\tOldDistance" << endl;
}
...
void initializeExperiment()
{
	...
	//factors.print();
	cerr << "TOTAL TRIALS= " << factors.size() << endl;
	factors.next();
	...
	vector<double> d;
	vector<string> distances = parameters.findAsVector("fDistances");
	for (unsigned int i=0; i<distances.size(); i++)
		d.push_back( -util::str2num<double>(distances.at(i)));

	double maxDist = *std::max_element(d.begin(),d.end());
	double minDist = *std::min_element(d.begin(),d.end());

	maxCoveredDistance = maxDist-minDist;
