...
const double stimulusDensity=0.025;	// points per mm^2, it means 150 points on a h=70, radius=15 cylinder area
// PRE-POST TEST PHASE
double testStimHeight;
...
void initStimulus(double height, double radius )
{
    ...
    int nStimulusPoints=2*(int)floor(twopi*radius*height*stimulusDensity);
    cerr << "nStimulusPoints " << nStimulusPoints  << endl;
    ...
    {   bool pretrialMode=trialMode;
        ...
        case 0:  //pre-test
        {
            ...
            {   beepTrial();
                ...
                initStimulus(testStimHeight,factors.at("TestStimRadius"));
            ...
            {   trialMode = HANDONSTARTMODE;
                ...
                initStimulus(testStimHeight, str2num<double>(parameters.find("AdaptStimRadius")) );
        ...
        case 1:  // adaptation
        {
            ...
            {   beepTrial();
                initStimulus(testStimHeight,str2num<double>(parameters.find("AdaptStimulusRadius")));
                double zadaptmin = str2num<double>(parameters.find("AdaptZMin"));
                double zadaptmax = str2num<double>(parameters.find("AdaptZMax"));
            ...
            {   beepLong();
                ...
                initStimulus(testStimHeight,factors.at("TestStimulusRadius"));
                visualRodCenter = Vector3d(0,0,factors.at("Distances"));
                hapticRodCenter = rodAway;
...
{   ifstream inputParameters;
    inputParameters.open("parametersRobertGrasping_v4_thumb_cylinder_training.txt");
...
{   if ( fingerCalibrationDone==3 && headCalibrationDone==3 )
    {
        ...
        {   // Define a sphere around physicalRigidBodyTip in which the thumb must stay
            ...
            {   bool triggerCondition=false;
                ...
                if ( triggerCondition ) //quando rientra o è già dentro
                {   indexInside[0]=true;
                    if ( indexInside[1]==false) //è appena rientrato
                        globalTimer.start();
                }
                else	//quando esce o è già fuori
                {   indexInside[0]=false;
        ...
        if ( headCalibrationDone==3 && fingerCalibrationDone==3 )
        {
            ...
            if ( totalFrame==0)
            {
                markersFile << fixed << setprecision(3) <<
                            "SubjectName\tAdaptFinger\tTestFinger\tAdaptOffset\tAdaptHapticFeedback\tFingerDist\tTotalTrial\tPhase\tTrialNumber\tTrialFrame\tdT\tTotTime\tIndexInside\tIsDrawing\tTrialOk\tVisualStimX\tVisualStimY\tVisualStimZ\tHapticRodX\tHapticRodY\tHapticRodZ\tfTestStimulusRadius\tfDistances\tEyeLeftXraw\tEyeLeftYraw\tEyeLeftZraw\tEyeRightXraw\tEyeRightYraw\tEyeRightZraw\tWristXraw\tWristYraw\tWristZraw\tThumbXraw\tThumbYraw\tThumbZraw\tIndexXraw\tIndexYraw\tIndexZraw" << endl;
                totalFrame++;
            }
            markersFile << fixed << setprecision(3) <<
                        ...
                        ((block.at("Phase")!=1) ? factors.at("TestStimulusRadius") : 9999) << "\t" <<
