...
bool mirroredThumbProjectedInside=false;
bool mirroredIndexProjectedInside=false;
bool infoDraw=true;
bool fingNow=false;
bool isSaving=true;
...
char *parametersFileNames[] ={
	"C:/cncsvisiondata/parametersFiles/Robert/expGraspingTwoFingers/parametersExpRobertTwoFingersMirrored_training.txt",
	"C:/cncsvisiondata/parametersFiles/Robert/expGraspingTwoFingers/parametersExpRobertTwoFingersMirrored.txt"	};
/********* SOUNDS ********/
#ifdef _WIN32
/***** SOUND THINGS *****/
...
void moveStimulusObject(const Vector3d& v, int speed)
{
	...
	double constantOffsetY=-56;
	double constantOffsetZ=-8;
...
void idle()
{
    ...
    if (headCalibrationDone==3 && fingerCalibrationDone==3)
    {
        ...
        Vector2d mirroredThumbProjection = cam.computeProjected(Vector3d(-thumb.x(),thumb.y(),thumb.z()));
        mirroredThumbProjectedInside = ( mirroredThumbProjection.x() >= 0 && mirroredThumbProjection.x() <= SCREEN_WIDTH ) && ( mirroredThumbProjection.y() >= 0 && mirroredThumbProjection.y() <= SCREEN_HEIGHT );
		
        Vector2d mirroredIndexProjection = cam.computeProjected(Vector3d(-index.x(),index.y(),index.z()));
        mirroredIndexProjectedInside = ( mirroredIndexProjection.x() >= 0 && mirroredIndexProjection.x() <= SCREEN_WIDTH ) && ( mirroredIndexProjection.y() >= 0 && mirroredIndexProjection.y() <= SCREEN_HEIGHT );
        ...
        markersFile << fixed << setprecision(3) <<
                    ...
                    isDrawing << "\t" <<
                    (thumbProjectedInside && mirroredThumbProjectedInside) << "\t" <<
                    (indexProjectedInside && mirroredIndexProjectedInside) << "\t" <<
					fingNow <<
                    endl;
...
void drawStimulus()
{
    ...
    */
    ...
    Vector3d mirroredThumb;
    mirroredThumb << -visualThumb.x(), visualThumb.y(), visualThumb.z();
    Vector3d mirroredIndex;
    mirroredIndex << -visualIndex.x(), visualIndex.y(), visualIndex.z();

    if ( (int) parameters.get("FingersMirrored") )
    {
		glColor3fv(glRed);
        glBegin(GL_POINTS);
        if (thumbProjectedInside && mirroredThumbProjectedInside )
            glVertex3dv(mirroredThumb.data());
        if (indexProjectedInside && mirroredIndexProjectedInside )
            glVertex3dv(mirroredIndex.data());
        glEnd();
