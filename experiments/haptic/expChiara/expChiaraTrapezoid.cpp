...
bool fingersInTrapezeVolume(int selectedFinger);
void paintGL();
void idle(void);
...
{
    "C:/cncsvisiondata/parametersFiles/Chiara/experimentTrapezoid/parametersExpChiaraTrapezoid_training.txt",
    "C:/cncsvisiondata/parametersFiles/Chiara/experimentTrapezoid/parametersExpChiaraTrapezoid.txt"
};
...
{   if ( infoDraw )
    {   glDisable(GL_COLOR_MATERIAL);
        ...
        {   text.draw("Trial mode " + stringify<int>(trialMode ));
            ...
            text.draw("Leaning= " + stringify<int>(fingersInTrapezeVolume(selectedFinger)));
...
bool fingersInTrapezeVolume(int _selectedFinger)
{
	double tol = parameters.get("LeaningTolerance");
	/*
			(front view)
        4____5______6____7
        |    |      |    |
	...
	*/
	...
	Vector3d pThumb,pIndex;
	pThumb << thumbCoords.getP1().p;
    pIndex << indexCoords.getP1().p;
	Vector3d *P = stim.getVertices();
	// Check for thumb x
	bool thumbXValid = (pThumb.x() >= P[0].x() - tol) && ( pThumb.x() <= P[3].x()+ tol );
	bool thumbYValid = (pThumb.y() >= P[1].y() - tol) && ( pThumb.y() <= P[5].y() + tol);
	bool thumbZValid = (pThumb.z() <= P[1].z() + tol/2) && ( pThumb.z() >= P[1].z() -tol/2 );

	bool indexXValid = (pIndex.x() >= P[0].x() - tol) && ( pIndex.x() <= P[3].x()+ tol );
	bool indexYValid = (pIndex.y() >= P[1].y() - tol) && ( pIndex.y() <= P[5].y() + tol);
	bool indexZValid = (pIndex.z() <= P[0].z() +tol/2 ) && ( pIndex.z() >= P[0].z() - tol/2 );

    switch (_selectedFinger)
    {
    case ONLY_THUMB:
    {
        return  thumbXValid && thumbYValid && thumbZValid;
    }
        break;
    case ONLY_INDEX:
    {
        return indexXValid && indexYValid && indexZValid;
    }
        break;
    case BOTH_INDEX_AND_THUMB:
    {
        return thumbXValid && thumbYValid && thumbZValid && indexXValid && indexYValid && indexZValid;
    }
	default:
		return false;
	}
...
void idle()
{
    ...
    if (headCalibrationDone==3 && fingerCalibrationDone==4)
    {
        ...
        fingNow = fingersInTrapezeVolume(selectedFinger);
        if (!isSaving)
            return;
...
void drawFingers()
{
	...
	/*
	if (fingersInTrapezeVolume(selectedFinger))
        glColor3fv(glGreen);
	glBegin(GL_POINTS);
	glVertex3dv(indexCoords.getP1().p.data());
	glVertex3dv(thumbCoords.getP1().p.data());
	glEnd();
	*/
	glPushMatrix();
	//glTranslated(-10,0,0);
...
void generateStimulus()
{
    ...
    stim.setCenter(Vector3d(0,factors.getCurrent().at("StimCenterY")+cyclopeanY,factors.getCurrent().at("Distances")));
