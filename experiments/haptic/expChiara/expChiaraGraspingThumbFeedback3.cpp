...
RigidBody rigidStartThumb,rigidCurrentThumb;
RigidBody rigidStartIndex,rigidCurrentThumbIndex;
...
// 12 novembre 2013
//Vector3d object20RelDisplacement(-3,68,52);
//Vector3d object40RelDisplacement(-4,-1,40);
...
BalanceFactor<double> balanceFactor[10];
vector<int> blocks;
int blocksTrialNumber[10]= {0,0,0,0,0,0,0,0,0};
int blockIndex=0;
...
char *blockNames[] = {"NF","VF","HF","FF","NF2","2FF","TACT","MIX_NF","MIX_TACT","HFTI"};
...
void handleKeypress(unsigned char key, int x, int y)
{
    ...
    case 'F':
    {
        ...
        if ( (fingerCalibrationDone==1) && allVisibleFingers )
        {
            ...
            beepOk();
            if ( blocks.at(blockIndex)==0 || blocks.at(blockIndex)==1 || blocks.at(blockIndex)==4 || blocks.at(blockIndex)==5 || blocks.at(blockIndex)==7 )
                RoveretoMotorFunctions::moveObjectAbsoluteAsynchronous(rodAway,rodTipHome,RODSPEED);
            else
             {
                Vector3d destination = visualRodCenter + Vector3d(0,0, balanceFactor[blocks.at(blockIndex)].getCurrent().at("RelDepth")/2.0-13.0/2.0);
                if ( blocks.at(blockIndex) == 6 || blocks.at(blockIndex) == 9 )
                {
                    if ( (int) balanceFactor[blocks.at(blockIndex)].getCurrent().at("RelDepth")==20 )
                    {
                        destination.y() -= util::str2num<double>( parameters.find("BaseCylinderHeight") );
                        destination += object20RelDisplacement;
                    }
                    else
                    {
                        destination.y() += util::str2num<double>( parameters.find("BaseCylinderHeight") );
                        destination += object40RelDisplacement;
                    }
                    destination.z() -= 200;
                }
                moveRod(destination,RODSPEED);
            }
            break;
        }
...
void initVariables()
{
    ...
    {
        cerr << "!! " << markersFileName << " already exists, press Enter to exit" << endl;
		cin.ignore(1E6,'\n');
		exit(0);
    }
    ...
    for ( int i=0; i<10; i++ )
    {
		...
		if (i==7)
		{
			vector<double> mixFactors; mixFactors.push_back(0); mixFactors.push_back(1);
			balanceFactor[i].addFactor("MixFactor",mixFactors);
		}
    }
	
	cerr << "====== MIX FACTOR TRIALS ====" << endl;
	balanceFactor[7].print(cerr);
	cerr << "==============================" << endl;
...
void idle()
{
    ...
    if ( headCalibrationDone==3 && fingerCalibrationDone==3 && trialMode==STIMULUSMODE )
    {
        ...
        if ( trialTime >= drawStimTime )
        {
            if (delayedTimer.getElapsedTimeInMilliSec() > delayTime )
            {
                ...
                int oldRelDepth = (int)balanceFactor[blocks.at(blockIndex)].getCurrent().at("RelDepth");
                // Advance to the next factors list
                ...
                if ( hasOtherFactors )
                {
                    ...
                    if ( blocks.at(blockIndex) == 2 || blocks.at(blockIndex) == 3 || blocks.at(blockIndex)==6 || blocks.at(blockIndex)==7 || blocks.at(blockIndex) == 9 )
                    {
                        Vector3d destination = visualRodCenter + Vector3d(0,0, balanceFactor[blocks.at(blockIndex)].getCurrent().at("RelDepth")/2.0-13.0/2.0);
                        // Questo hack serve per trattare il blocco 7 che è composto randomicamente dal blocco 0 e dal 6
						int tmpBlock = blocks.at(blockIndex);
						if (tmpBlock==7)
						{
							tmpBlock = balanceFactor[blocks.at(blockIndex)].getCurrent().at("MixFactor")*6;
						}
						// Per lasciare gli altri blocchi funzionanti e cambiare il meno possibile devo controllare che dal 7 non sia venuto fuori un blocco 0
						// e di conseguenza si dovrebbe muovere l'oggetto lontano. Questa condizione di controllo vale anche se non siamo nel blocco 7
						if (tmpBlock!=0)
						{
							if ( tmpBlock == 6 || tmpBlock == 9 )
							{
								if ( (int) balanceFactor[blocks.at(blockIndex)].getCurrent().at("RelDepth")==20 )
								{
									destination.y() -= util::str2num<double>( parameters.find("BaseCylinderHeight") );
						...
						else // siamo nel caso in cui dal blocco mix c'è un blocco 0 e quindi dobbiamo muovere via tutto.
						{
							Vector3d tmpRodAway = rodAway;
								if ( blocks.at(blockIndex)==7 )
									tmpRodAway = visualRodCenter - Vector3d(0.0,0.0,mathcommon::unifRand(300.0,350.0));
								RoveretoMotorFunctions::moveObjectAbsolute(tmpRodAway,rodTipHome,RODSPEED);
						}
					}
                    else
                        RoveretoMotorFunctions::moveObjectAbsolute(rodAway,rodTipHome,RODSPEED);
                ...
                else // else we go to the next block
                {
                    ...
                    if ( blockIndex >= (int)blocks.size() )	// exceeded the total blocks
                    {
    ...
    if ( !isMovingRod )
    {
        ...
        if ( headCalibrationDone==3 && fingerCalibrationDone==3 )
        {
			...
			if ( blocks.at(blockIndex)==7 )
				currentBlockName = blockNames[blocks.at(blockIndex)+(int)(balanceFactor[blocks.at(blockIndex)].getCurrent().at("MixFactor"))];
            markersFile << fixed << setprecision(3) <<
...
{   if ( infoDraw )
    {   GLText text;
        ...
        if ( headCalibrationDone==3 && fingerCalibrationDone==3 )
        {
			...
			if (blocks.at(blockIndex) == 7 )
			{
				text.draw("SubBlock= " + stringify<int>(balanceFactor[7].getCurrent().at("MixFactor")*6) );
			}
...
void drawFingerTips()
{
	int currentBlock =  blocks.at(blockIndex);
	if (currentBlock==7)
	{
		currentBlock = balanceFactor[blocks.at(blockIndex)].getCurrent().at("MixFactor")*6;
	}
    ...
    // Draw fingertips for the haptic condition, only thumb
    if ( currentBlock == 6 && blocks.at(blockIndex)==7 )
    {
		//return; // Modifica fatta 14 febbraio per non disegnare il pollice nel blocco 7 mix_tact
        glPushMatrix();
        glMultMatrixd(rigidStartThumb.getFullTransformation().data());
        glColor3fv(glRed);
        glScaled(1,1,0.001);
