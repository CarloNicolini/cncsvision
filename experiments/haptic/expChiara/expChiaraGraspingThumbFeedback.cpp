...
BalanceFactor<double> balanceFactor[4];
vector<int> blocks;
int blocksTrialNumber[4]= {0,0,0,0};
int blockIndex=0;
...
void handleKeypress(unsigned char key, int x, int y)
{
    ...
    case 'F':
    {
        ...
        {   
            ...
            beepOk();
			if (blocks.at(blockIndex)==0 || blocks.at(blockIndex)==1 )
    ...
    if ( headCalibrationDone==3 && fingerCalibrationDone==3 && !indexInside[0] )
    {
        ...
        if ( !visibleFinger && trialMode==STIMULUSMODE )
        {
            ...
            if ( str2num<int>(parameters.find("AudioFeedback") ) )
                boost::thread beepInvisible( beepInvisible);
        }
        ...
        if ( trialTime >= drawStimTime )
        {
			if (delayedTimer.getElapsedTimeInMilliSec() > delayTime )
			{
				...
				if (oldDistance == (int)(balanceFactor[blocks.at(blockIndex)].getCurrent().at("Distances")) && 
					(blocks.at(blockIndex)==0 || blocks.at(blockIndex)==1 ) )
				{
...
{   if ( infoDraw )
    {   GLText text;
        ...
        if ( headCalibrationDone==3 && fingerCalibrationDone==3 )
        {
            ...
            text.draw("CurrentBlock= " + stringify<int>(blocks[blockIndex]));
            text.draw("TotTrialNumber=" + stringify<int>(totalTrialNumber));
			text.draw("Remaining trials= " + stringify<int>( balanceFactor[blocks[blockIndex]].getRemainingTrials() ));
...
void drawFingerTips()
{
	...
	if ( blocks[blockIndex] == 1 || blocks[blockIndex] == 3 )
	{
