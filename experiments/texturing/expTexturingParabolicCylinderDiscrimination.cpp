...
ParaboloidPointsStimulus parabolicCylinderPoints[2];
...
void updateStimulus(double curvatureReference, double curvatureStaircase)
{
    ...
    for (int i=0; i<2;i++)
    {
        ...
        parabolicCylinderPoints[i].setNpoints(nRandomDots);
        parabolicCylinderPoints[i].setCurvature(c[i]);
        parabolicCylinderPoints[i].setFluffiness(0.0);
        parabolicCylinderPoints[i].compute();

        stimDrawerBlack[i].setSpheres(false);
        stimDrawerWhite[i].setSpheres(false);

        stimDrawerWhite[i].setStimulus(&parabolicCylinderPoints[i]);
        stimDrawerBlack[i].setStimulus(&parabolicCylinderPoints[i]);

        if ( parameters.find("RandomNoiseColor")=="RedAndBlack" )
        {
            stimDrawerWhite[i].initList(&parabolicCylinderPoints[i],glWhite,randomDotsSize);
            parabolicCylinderPoints[i].compute();
            stimDrawerBlack[i].initList(&parabolicCylinderPoints[i],glBlack,randomDotsSize);
        }
        else
        {
            stimDrawerWhite[i].initList(&parabolicCylinderPoints[i],glWhite,randomDotsSize);
...
void initVariables()
{
    ...
    for ( int i=0; i<2; i++ )
    {
        ...
        surface[i].initializeSurfaceShaders(VolumetricSurfaceIntersection::SurfaceParabolicCylinder);
    }
    updateStimulus(trial.getCurrent().first.at("CurvatureZ"),trial.getCurrent().second->getCurrentStaircase()->getState());
}
...
{
#ifdef _WIN32
    string parametersFileName("C:/cncsvisiondata/parametersFiles/Fulvio/expTexturing/parametersExpTexturingParabolicCylinderDiscrimination.txt");
#endif
#ifdef __linux__
    string parametersFileName("/home/carlo/Desktop/params/parametersExpTexturingParabolicCylinderDiscrimination.txt");
#endif
