...
// Timing variables
long int frame=0;
long int timeFrame=0;
double variable =0;
double oldvariable=0;
static const int PERIOD=1995; //1500 ms is the period of triangular wave
...
{   // Draw the stimulus ( red-dots plane )
    ...
    double zNewTranslation =0.0;
    bool isInside = ((projPoint - Vector3d(0,0,focalDistance) ).norm()) < (circleRadius-4);

    if ( (int)factors["FollowingSpeed"] == 0 )
        variable = mathcommon::trianglewave( (double)frame , PERIOD);
    ...
    {
    case 1:
        currentYaw = toDegrees(headEyeCoords.getRigidStart().getFullTransformation().rotation().eulerAngles(0,1,2).y());
        zNewTranslation = -sqrt(pow(fixationPoint.x(),2) + pow(fixationPoint.z(),2) );
        glRotated( currentYaw, 0,1,0);
        break;
    case 2:
        currentYaw = toDegrees(headEyeCoords.getRigidStart().getFullTransformation().rotation().eulerAngles(0,1,2).x());
        zNewTranslation = -sqrt(pow(fixationPoint.y(),2) + pow(fixationPoint.z(),2) );
        glRotated( currentYaw, 1,0,0);
        break;
    }
    if ( (int) factors["FollowingSpeed"] == 0 )
        currentYaw = variable*10;
    // Here the rotation to face the observer
    glTranslated( 0, 0, zNewTranslation );
    switch ( (int) factors["Tilt"] )
    {
    case 0:
        glRotated( -90-factors["Slant"] - currentYaw*factors["RotationSpeed"], 0,1,0 );
        break;
    case 90:
        glRotated( -90+factors["Slant"]-currentYaw*factors["RotationSpeed"], 1,0,0 );
        break;
    case 180:
        glRotated( -90+factors["Slant"]+currentYaw*factors["RotationSpeed"], 0,1,0 );
        break;
    ...
    double angleRad = toRadians(-90+factors["Slant"]+currentYaw*factors["RotationSpeed"]);
    double zEye=0;
    double zStim=focalDistance;
    double xStim=0;
    double projectedHeight = 25*sin(angleRad)*( focalDistance-zEye )/zStim;
    double projectedWidth = ( xStim + 25 )/(zStim+25*cos(angleRad))*(focalDistance-zEye);
    anglesFile << trialNumber << " " << toDegrees(angleRad) << " " << projectedHeight << " " << projectedWidth << " " << factors["Tilt"] << endl;

    //drawCircle();
}

void drawProbe(double startingAngle)
{
    double probeRadius=30;
    ...
    glBegin(GL_LINES);
    glVertex3f(0,0,focalDistance);
    glVertex3f(probeRadius*cos(probeAngle+startingAngle), probeRadius*sin(probeAngle+startingAngle),focalDistance);
    glEnd();
    glColor3fv(glRed);
...
void mouseFunc(int button, int state, int _x , int _y)
{
    if ( trialMode == PROBEMODE )
    {
        ...
        if( (button== GLUT_MIDDLE_BUTTON) && (state==1) )
        {
            frame=0;
            timeFrame=0; //this put the stimulus in the center each central time mouse is clicked in
            if ( trial.isEmpty() )
            {
                responseFile << setw(6) << left <<  trialNumber << " " << factors["Tilt"] <<" " << factors["Slant"] << " " << factors["RotationSpeed"] << " " << (focalDistance - eyeCalibration.z()) << " " << mod(toDegrees(probeAngle+probeStartingAngle),360) << " " << factors["Anchored"] << endl;
                cleanup();
                exit(0);
            }
            else
            {
                // responseFile << setw(6) << left <<  trialNumber << " " << factors["Tilt"] <<" " << factors["Slant"] << " " << factors["RotationSpeed"] << " " << (focalDistance - eyeCalibration.z()) << " " << mod(toDegrees(probeAngle+probeStartingAngle),360) << " 120" << endl;
                responseFile << setw(6) << left <<  trialNumber << " " << factors["Tilt"] <<" " << factors["Slant"] << " " << factors["RotationSpeed"] << " " << (focalDistance - eyeCalibration.z()) << " " << mod(toDegrees(probeAngle+probeStartingAngle),360) << " " << factors["Anchored"] << endl;
            ...
            advanceTrial();
            probeStartingAngle=floor(unifRand(0.0,2.0*M_PI));
            redDotsPlane.setDimensions(50,50,0.1);
            redDotsPlane.setSlantTilt( factors["Slant"] , (int) factors["Tilt"] );
            redDotsPlane.compute();
            stimDrawer.initList(&redDotsPlane);
...
{   if ( trialMode == PROBEMODE )
    {   if (mouseState!=1)
        {   if ( mouseButton == GLUT_LEFT_BUTTON )
                probeAngle+=2*M_PI/180.0; // +1 degree
...
void mousePassiveMotion(int _x, int _y)
{
    if ( _x > SCREEN_WIDTH )
        glutWarpPointer(SCREEN_WIDTH-1,_y);

    if ( _y > SCREEN_HEIGHT )
        glutWarpPointer(_x,SCREEN_HEIGHT-1);

    if ( _x < 0 )
        glutWarpPointer(1,_y);
    if ( _y < 0 )
        glutWarpPointer(_x,1);
...
void drawGLScene()
{
    ...
    {   glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        ...
        glMatrixMode(GL_MODELVIEW);
        glLoadIdentity();
        cam.setEye(headEyeCoords.getCyclopeanEye());
        drawTrial();
        glutSwapBuffers();
    }
}
...
void checkBounds()
{
    if ((trialMode!=PROBEMODE) )
    {
        conditionInside = ((projPoint - Vector3d(0,0,focalDistance) ).norm()) < (circleRadius-4) ; // here we subtract 4 to work with trianglewave precision
...
void update(int value)
{
    ...
    if ( trialMode != PROBEMODE )
    {
        ...
        variable = mathcommon::trianglewave( (double)timeFrame , PERIOD );
        timeFrame+=TIMER_MS*factors["FollowingSpeed"];
        // This is the condition to advance
        ...
        int HALFPERIOD = 990;
        if ( ((frame)%(PERIOD) == HALFPERIOD ) && (int)(factors["FollowingSpeed"]==0)  )
        {
            advanceTrial();
        }
    }
    ...
    double maxRotationAngle = toDegrees(atan(circleRadius/(abs(focalDistance))));
    //cerr << maxRotationAngle << endl;
    switch ( (int)factors["Anchored"] )
    {
    case 0:
        R = Matrix3d::Identity();
        break;
    case 1:
        R = AngleAxis<double>(toRadians(variable*maxRotationAngle),Vector3d::UnitY() ).toRotationMatrix();
        break;
    case 2:
        R = AngleAxis<double>(toRadians(variable*maxRotationAngle),Vector3d::UnitX() ).toRotationMatrix();
        break;
    }

    markers[1] = R*markers[1];
    markers[2] = R*markers[2];
    markers[3] = R*markers[3];
...
void idleFunc()
{
    if ( trialMode == PROBEMODE )
    {
        if ( mouseButton == 0 && mouseState == 0)
            probeAngle+=M_PI/180*0.5;
        if ( mouseButton == 2 && mouseState == 0)
            probeAngle -=M_PI/180*0.5;
    }
    glutPostRedisplay();
}
