...
double variable =0;
static const double BASEPERIOD=1000; // is the period of triangular wave
...
{   // Draw the stimulus ( red-dots plane )
    ...
    if ( factors["FollowingSpeed"] < 1E-2 )
    {
        timeFrame+=TIMER_MS;
        variable = mathcommon::trianglewave(timeFrame , BASEPERIOD );
    }
    glPushMatrix();
...
void mouseFunc(int button, int state, int _x , int _y)
{
    if ( trialMode == PROBEMODE )
    {
        ...
        {
            frame=timeFrame=0; //this put the stimulus in the center each time mouse is clicked
            if ( trial.isEmpty() )
            {
...
void update(int value)
{
    ...
    if ( trialMode != PROBEMODE )
    {
        ...
        {
            variable = -factors["Onset"]*mathcommon::trianglewave( timeFrame , BASEPERIOD/(TIMER_MS*factors["FollowingSpeed"]) );
            cout << variable << endl;
            timeFrame+=1;
        }
        ...
        if ( (isInside && (sumOutside > stimulusEmiCycles)) ||(  (factors["FollowingSpeed"]<1E-2) && (stimulusTimer.getElapsedTimeInMilliSec()>BASEPERIOD/2 ) ))
        {
            sumOutside=0;
            advanceTrial();
        }
