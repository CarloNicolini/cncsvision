...
void keyPressed()
{
    ...
    if ( responseTime > clickDelay )
    {
        ...
        {
            responseFile << setw(6) << left <<  trialNumber << " " << factors["Def"] <<" " << factors["Tilt"] <<" " << factors["Slant"] << " " << factors["Anchored"] <<  " " << factors["FollowingSpeed"] << " " << factors["Onset"] << " " << (focalDistance - eyeCalibration.z()) << " " << probeAngle << " " << responseTime << " " << contraction << endl;
            cleanup();
            exit(0);
        }
        else
...
void initStreams()
{
    string parametersPassiveFileName("parametersPassiveCdef.txt");
