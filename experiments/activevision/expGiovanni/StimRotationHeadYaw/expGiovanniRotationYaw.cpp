...
{   // Draw the stimulus ( red-dots plane )
	...
	case 2:
	{
		...
		*AngleAxis<double>(eulerAngles.getPitch(), Vector3d::UnitX())).toRotationMatrix();
		...
		instantPlaneSlant = projPointAngleX*factors["RotationSpeed"] +factors["Slant"];
	...
	case 1:
	{
		...
		*AngleAxis<double>(eulerAngles.getPitch()*factors["FollowingSpeed"], Vector3d::UnitX())).toRotationMatrix();
		...
		double projPointAngleY = toDegrees( atan( (projPointEyeRight.y()-eyeRight.y())/abs(projPointEyeRight.z())));
		instantPlaneSlant = projPointAngleY*factors["RotationSpeed"] +factors["Slant"];
		stimulusCenter = objrotation*Vector3d(0,0,focalDistance)+headEyeCoords.getRigidStart().getFullTransformation().translation();
		objectActiveTransformation.linear()=objrotation;
	}
	break;
...
void keyPressed()
{
	if ( trialMode == PROBEMODE )
	{
		...
		"TrialNumber Tilt Slant Rotation RotationSpeed FollowingSpeed Onset EyeCalx EyeCaly EyeCalz StimFrames ProbeAngle" << endl;
		}
	...
	if ( (headCalibrationDone > 2) && (trialMode!=PROBEMODE) )
	{
		...
		if ( !( conditionInside || (!wasInside) ) )
		{
			...
			{	// se siam in stimulusMode bippa sempre quando vengon raggiunti i limiti
				...
				double timeElapsed = rythmMaker.getElapsedTimeInMilliSec();
				if ( (timeElapsed > minOscTime) && (timeElapsed < maxOscTime) && (trialMode!=STIMULUSMODE) )
				{
				boost::thread beepOkThread(beepOk);
				beepOkThread.detach();
				switch ( (int) factors["Rotation"] )
				...
				case 0:
				{
				if ( (factors["Onset"]*optotrak.getAllVelocities()[3].x() < 0) )
				...
				case 1:
				{
					if ( (factors["Onset"]*optotrak.getAllVelocities()[3].y() > 0) )
					{
					if ( abs(eyeToCenterAngleY) < maxAllowedTranslationYaw )
						sumOutside++;
					else
...
Vector2d getPlaneDimensions()
	...
	Point3D *pLowCenter = redDotsPlane.specialPointsRand[7];
