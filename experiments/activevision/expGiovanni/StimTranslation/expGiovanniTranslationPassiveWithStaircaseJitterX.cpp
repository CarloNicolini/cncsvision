...
double jitterScreenAlignmentX=0;
   ...
   {  if ( trialNumber==0)
      {  responseFile << setw(6) << left <<
        ...
        responseFile << fixed << 
	        trialNumber << " " << 
		factors["Rotation"] << " " <<
			jitterScreenAlignmentX << " " <<  // jitter on screen alignment to avoid spatial cues
...
#endif
   if ( !fileExists((baseDir+"stimFilePassive_"+subjectName+".txt")) )
    stimFile.open(std::string(baseDir+"stimFilePassive_"+subjectName+".txt").c_str());
   if ( !fileExists((baseDir+"coordinateFilePassive_"+subjectName+".txt")) )
...
// Read the experiment from file, if the file is finished exit suddenly
   ...
   {  // This reads a line (frame) in inputStream
      ...
      if ( trialMode==STIMULUSMODE && lastTrialMode!=STIMULUSMODE )
      {
        ...
        stimulusDuration.start();
		jitterScreenAlignmentX=unifRand(-25.0,25.0);
		//cout << jitterScreenAlignmentX << endl;
      }
      ...
      if ( passiveMode )
      {  initProjectionScreen(0, headEyeCoords.getRigidStart().getFullTransformation()*Translation3d(center+Vector3d(jitterScreenAlignmentX,0,0)));
      }
      else
         initProjectionScreen(focalDistance, Affine3d::Identity());
	  ...
	  if ( trialMode == STIMULUSMODE )
		{
	  coordinateFile << fixed << trialNumber << "\t" << whichDrawFirst.at(trialNumber) << "\t" << objectPassiveTransformation.translation().transpose() << "\t" << jitterScreenAlignmentX <<endl;
