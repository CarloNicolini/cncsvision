...
Affine3d passiveTransformationMatrix= Affine3d::Identity();
...
void drawCube()
{
	...
	//cout << projPointEyeRight.transpose() << endl;
	...
	//cout << cubePassive.transpose() << endl;
	glPushMatrix();
	glLoadIdentity();
...
{   // Draw the stimulus ( red-dots plane )
	...
	Affine3d info=Affine3d::Identity();
...
void drawGLScene()
{
	...
	if ( trialMode == STIMULUSMODE )
		...
		cam.setEye(eyeRight-eyeCalibration);
	drawInfo();
	drawTrial();
	glutSwapBuffers();
	}
...
void update(int value)
{
	...
	Eigen::ParametrizedLine<double,3> lineOfSightLeft = Eigen::ParametrizedLine<double,3>::Through( eyeLeft-eyeCalibration, eyeLeft-eyeCalibration+directionOfSight );
	...
	cout << "PROJ="  << projPointEyeRight.transpose() << endl;
	projPointEyeLeft= lineOfSightLeftDistanceToFocalPlane * (directionOfSight) + (eyeLeft-eyeCalibration);
	/*
	// VECCHIO MODO= DI FARE PUNTO PROIETTATO
	Vector3d fixationPoint = headEyeCoords.getRigidStart().getFullTransformation() * ( center - eyeCalibration + eyeRight );
    // Projection of view normal on the focal plane
    Eigen::ParametrizedLine<double,3> pline = Eigen::ParametrizedLine<double,3>::Through(eyeRight-eyeCalibration,fixationPoint-eyeCalibration);
    
	Vector3d projPoint = pline.intersection(focalPlane)*((fixationPoint - eyeRight).normalized()) + eyeRight;
	
	double pdist = (projPointEyeRight- fixationPoint).norm();
	//cout << "PP dist= " << (projPointEyeRight- fixationPoint).norm() << endl;
	*/
	...
	Affine3d focalPlaneTransform(headEyeCoords.getRigidStart().getFullTransformation());
	/*
	if ( passiveMode )
	{
		screen.setFocalDistance(0);
		double dist = (eyeRight-projPointEyeRight).norm();
		//cout << "InitDist= " << focalDistance-eyeCalibration.z() << endl;
		//focalPlaneTransform*=Translation3d(Vector3d(0,0,focalDistance-eyeCalibration.z()));
		screen.rotate( focalPlaneTransform.rotation());
		screen.translate( 0,0,-eyeCalibration.z());
		Vector3d focalPlaneLocal = focalPlaneTransform*Vector3d(0,0,focalDistance-eyeCalibration.z());
		screen.translate( -focalPlaneLocal.x(),-focalPlaneLocal.y(),-focalPlaneLocal.z() );
		//screen.print(screenFile);
		//cout << 0.5*(screen.pa + screen.pd).transpose() << " === " << 0.5*(screen.pb + screen.pc).transpose() << endl;
		//cout << "Dist= " << dist << endl;
		//cout << (focalPlaneTransform).translation().transpose() << endl;
	}
	*/
	...
	if ( passiveMode )
	{
		screen.setFocalDistance(0);
		//cout << "InitDist= " << focalDistance-eyeCalibration.z() << endl;
		Affine3d prova = focalPlaneTransform;
		prova*=Translation3d(Vector3d(0,0,focalDistance)-eyeCalibration);
		screen.transform(prova);
	}	
	...
	if ( trialMode == STIMULUSMODE && headCalibrationDone > 2)
	{
		...
		anglesFile << setw(6) << left << 
			...
			eulerAngles.getYaw() << " " <<
			eulerAngles.getRoll() << " " << endl;
	}
	passiveTransformationMatrix = headEyeCoords.getRigidStart().getFullTransformation().inverse();
	...
	for ( int i=0; i<16; i++ )
	{
		double val =passiveTransformationMatrix.matrix()(i);
		if ( isnan(val) || isinf(val) )
			markersFile << numeric_limits<double>::min() << " ";
		else
			markersFile << val << " " ;
	}
	markersFile << factors["Tilt"] << " " << factors["Slant"] << factors["SlantDirection"] << endl; //XXX METTERE GLI ALTRI FATTORI

	// Salvataggio continuo su file

	// Creazione matrice passiva
	Vector3d fixationShifted = headEyeCoords.getRigidStart().getFullTransformation()* Vector3d(0,0,focalDistance);
	Affine3d passive = headEyeCoords.getRigidStart().getFullTransformation().inverse() * Translation3d( fixationShifted ) ;
	ofstream outputfile;
