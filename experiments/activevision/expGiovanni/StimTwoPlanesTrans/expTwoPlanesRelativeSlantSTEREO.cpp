...
void initStreams()
{
    string parametersFileName = "parametersActiveTwoPlanesTraining.txt";
    ...
    case '4':
    {
        if ( trialMode == PROBEMODE )
        {
			...
			if( (int) factors.at("OverallTilt") == 0 )
				...
				answer=true;
			/* VECCHIO CON CAST DA DOUBLE A BOOL SBAGLIATO
			if ( !(bool)factors.at("OverallTilt") )
				answer = false;
			else
				answer=true;
			*/
			...
			if ( str2num<int>(parameters.find("RandomizeOverallTilt")))
				...
				overallTilt = 1;
			//overallTilt=(unifRand(0.0,1.0)<0.5);
			*/
		}
    }
    break;
