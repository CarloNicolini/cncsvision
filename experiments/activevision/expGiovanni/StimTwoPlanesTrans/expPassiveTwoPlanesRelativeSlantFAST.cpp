...
{   // Load the parameters file
	...
	string flowsFileMovingName = baseDir + "flowsFileMoving_"+subjectName +"_auto.txt";
	string flowsFileStillName = baseDir + "flowsFileStill_"+subjectName +"_auto.txt";
...
{   interoculardistance = str2num<double>(parameters.find("IOD"));
    ...
    for (int i=0; i<2; i++)
    {
        redDotsPlane[i].setNpoints(150);  //XXX controllare densita di distribuzione dei punti
        redDotsPlane[i].setDimensions(50,50,0.01);
    ...
    {   // This reads a line (frame) in inputStream
	  ...
	  projPointEyeRight=secondProjection ;
        ...
        // **************** COMPUTE THE OPTIC FLOWS **************************
         ...
         angVelocitiesMoving /= ((double)10/(double)1000);
         ...
         angVelocitiesStill /= ((double)10/(double)1000);
         ...
         flowsFileStill << fixed << trialNumber << "\t" <<  // 1
						...
						overallTilt << " " << // 5
                        projPointsStillPlane[4].transpose() << " " << // 6,7,8
                        projPointsStillPlane[5].transpose() << " " << // 9,10,11
                        projPointsStillPlane[8].transpose() << " " << // 12,13,14
                        angVelocitiesStill.transpose() << " " << // 15, 16, 17, 18, 19, 20
                        velocitiesStill.transpose() << endl;	// 21, 22, 23, 24, 25, 26
