...
void stepper_rotate(double final_theta);
...
void phidgets_linear_move(double desired_distance, int axis)
{
	...
	//		cerr << "Initial current_position: " << current_position << endl;
			...
			// motors move
			CPhidgetMotorControl_setVelocity (motoControl, axis, direction*speed[stroke]);
...
//					cerr << "current: " << current << endl;
	...
	//		cerr << "Loop #: " << stroke << endl;
		...
		CPhidgetInterfaceKit_getSensorValue(ifKit, 1, &current_position);
...
//		cerr << "Initial current_position: " << current_position << endl;
		...
		while(1)
		{
			...
			{
				// partial position
				double part_distance = abs(desired_distance*10 - current_position);
				// proportion of distance still to be covered
				double prop_to_travel = part_distance / abs(distance);

				// set a scaling factor for velocity
				double vel_scaling = pow(prop_to_travel, .5);
...
void stepper_rotate(double final_theta)
{
	...
	CPhidgetStepper_setAcceleration(stepper, 0, maxAccel/100.0);
	CPhidgetStepper_getVelocityMax(stepper, 0, &maxVel);
	CPhidgetStepper_setVelocityLimit(stepper, 0, maxVel/100.0);
	...
	// how many steps?
	int num_steps = step_theta / 0.1125;
	
	// rotate
	CPhidgetStepper_setTargetPosition (stepper, 0, num_steps);
