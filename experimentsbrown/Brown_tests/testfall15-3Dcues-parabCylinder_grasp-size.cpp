...
#ifndef SIMULATION
string experiment_directory = "S:/Domini-ShapeLab/carlo/2014-2015/3Dcues-parabolicCylinder/size/";
//string experiment_directory = "C:/Users/visionlab/Desktop/";
#else
string experiment_directory = "/media/shapelab/Domini-ShapeLab/carlo/2014-2015/3Dcues-parabolicCylinder/size/";
#endif

// parameters file directory and name
string parametersFile_directory = experiment_directory + "parameters_3Dcues-parabCylinder_grasp-size.txt";

// response file name
string responseFile_name = "fall15-3Dcues-parabCylinder_grasp-size_";
...
// markers file name
string markersFile_name = "fall15-3Dcues-parabCylinder_grasp-size_";
...
void drawInfo()
{
	if ( visibleInfo )
	{
		...
		text.draw(" ");
		text.draw("####### fall15-parabCylinder_grasp-size #######");
		text.draw("");
		...
		if ( isVisible(markers.at(stepper).p) )
			glColor3fv(glGreen);
		else
			glColor3fv(glRed);
...
void draw_height()
{
	...
	// top dot
	glPushMatrix();
	glTranslated(0.0, objwidth, current_depth);
...
/*
		...
		bool objVisibility[4] = {isVisible(markers.at(obj_markers[0]).p), isVisible(markers.at(obj_markers[1]).p), isVisible(markers.at(obj_markers[2]).p), isVisible(markers.at(obj_markers[3]).p)};
		// pick the first visible
		Vector3d orientation_marker(99.9, 99.9, 99.9);
		for(int i = 0; i < 4; i++)
		{
			if(objVisibility[i] == 1)
			{
				orientation_marker = markers.at(obj_markers[i]).p;
				obj_visible_marker = obj_markers[i];
				break;
			}
		}

		// calculate the current offset angle of the visible marker
		beta = atan((orientation_marker.z() - markers.at(stepper).p.z()) / orientation_marker.x());
		stepper_set_angle(rotTable, 0);

		__int64 pos;
		CPhidgetStepper_getCurrentPosition(rotTable, 0, &pos);
		cerr << "Current pos = " << pos << endl;

		double current_angle = 0.0;
		
		if(abs(beta*180.0/M_PI) > 0.5)
		{
			// rotate orientation_marker perpendicularly to the line of sight
			stepper_rotate(rotTable, -beta*180.0/M_PI);
			// now set the current angle for the stepper
			if(obj_visible_marker == 9)
				current_angle = 270.0;
			if(obj_visible_marker == 23)
				current_angle = 0.0;
			if(obj_visible_marker == 24)
				current_angle = 90.0;
			if(obj_visible_marker == 10)
				current_angle = 180.0;

			updateTheMarkers();
			stepper_set_angle(rotTable, current_angle);
			CPhidgetStepper_getCurrentPosition(rotTable, 0, &pos);
			cerr << "current_angle after correction = " << current_angle << ". Current pos2 = " << pos << ". Calculated angle based on pos2 = " << (double)pos/884 << endl;
		}
...
*/
		...
		if(current_depth == 20.0)
			depth_offset = 27.0;
		if(current_depth == 30.0)
			depth_offset = 29.0;
		if(current_depth == 40.0)
			depth_offset = 36.0;
...
/*		
		CPhidgetStepper_getCurrentPosition(rotTable, 0, &pos);
		double pos_theta = pos/884;

		cerr << "\nCurrent angle of " << current_angle << ". Current position = " << pos_theta << ". Incoming depth of " << current_depth << ". Will rotate by  " << target_angle-current_angle << ".\n" << endl;
*/
...
/*
		...
		cerr << "Position after rotation = " << pos_theta << ".\n" << endl;
*/
		// draw the stimulus	
		beepOk(0);
