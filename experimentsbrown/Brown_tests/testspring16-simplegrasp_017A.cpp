...
Vector3d ind(0,0,0), thu(0,0,0), platformIndex(0,0,0), platformThumb(0,0,0), noindex(-999,-999,-999), nothumb(-999,-999,-999), target_position(0,0,0), home_position(0,0,0), grip_position(0,0,0);
...
string parametersFile_directory = experiment_directory + "parameters_grasp.txt";

// response file name
string responseFile_name = "grasp_results_";

// summary file headers
string responseFile_headers = "subjName\ttrialN\ttrialDuration\tgood_trial";

// markers file name
string markersFile_name = "grasp_trajectories_";

// response file headers
string markersFile_headers = "subjName\ttrialN\ttime\tframeN\tindexXraw\tindexYraw\tindexZraw\tthumbXraw\tthumbYraw\tthumbZraw\tfingersOccluded\tframesOccluded\tiGrasped";
...
{   
	...
	{   
		...
		case 'F':
		{
			...
			if ( fingerCalibrationDone==4  && allVisibleFingers && !pressingF)
			{
...
void online_trial()
{
	...
	if(fingerCalibrationDone==5)
	{
		...
		if(ind.z() - target_position.z() < 20.0)
			iGrasped = true;
