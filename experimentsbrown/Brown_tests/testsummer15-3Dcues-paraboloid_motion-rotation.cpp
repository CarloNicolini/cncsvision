...
double custom_depth = 30.0;
bool disparity_training = true;
...
void drawInfo()
{
	if ( visibleInfo )
	{
		...
		text.draw(" " );
		if(disparity_training)
			text.draw("custom_depth: " +stringify<double>(custom_depth));
		else
...
void drawStimulus()
{
	if(checkglasses)
	{
		IOD = interoculardistance;
		draw_stereocheck();
	} else if (disparity_training) 
	{
		glLoadIdentity();
		glTranslated(0, 0, objdistance);
		IOD = interoculardistance;
		glRotated(motion_theta * direction_motion2, 1, 0, 0);
		drawParaboloid(objheight, custom_depth);
