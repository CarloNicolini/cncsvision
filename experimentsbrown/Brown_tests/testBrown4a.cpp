...
/********** EYES AND MARKERS **********************/
Vector3d eyeLeft(0,0,0), eyeRight(0,0,0), ind(0,0,0), thu(0,0,0), platformIndex(0,0,0), platformThumb(0,0,0), noindex(-999,-999,-999), nothumb(-999,-999,-999);
...
/********** STREAMS **************/
ofstream responseFile, summaryFile, testFile;
...
void initOptotrak();
void initRendering();
void advanceTrial(bool response, int numP);
void initTrial();
void drawFingers();
void repeatTrial();
...
string parametersFile_directory = experiment_directory + "parameters_fall14ErnstBanks_test1-test.txt";

// response file name
string responseFile_name = "fall14-1_ernstbanksreplica_";
...
string testFile_headers = "trialN\tstimX\tstimY\tstimZ";

// summary file headers
string summaryFile_headers = "subjName\tIOD\ttrialN\tAbsDepth\tstairID\tbar_height\treversals\tstairAscending\tstepsDone\tfirst_stimulus\tsecond_stimulus\tnoise_level\tresponsekey";
...
*/
	...
	// test file
	string testFileName = experiment_directory + responseFile_name + subjectName + "_" + stringify<int>(condition) + "_" + stringify<int>(stimulus_noise) + "_test.txt";

	// Check for output file existence
    if (util::fileExists(testFileName))
    {
        string error_on_file_io = testFileName + string(" already exists");
        cerr << error_on_file_io << endl;
#ifndef SIMULATION
        MessageBox(NULL, (LPCSTR)"FILE ALREADY EXISTS\n Please check the parameters file.",NULL, NULL);
#endif
    ...
    else
    {
        ...
        cerr << "File " << testFileName << " loaded successfully" << endl;
    }

	testFile << fixed << testFile_headers << endl;
}
...
void drawStimulus()
{
	...
	{	
		...
		{
			glTranslatef(0, whereTop.y()-((stimulus_height[first_interval])/2.0), absDepth);
			whichStimulus = first_interval;
			drawStandardComparison();
		} else if(timer.getElapsedTimeInMilliSec() >= 2050 && timer.getElapsedTimeInMilliSec() <= 3050)
		{
			glTranslatef(0, whereTop.y()-((stimulus_height[second_interval])/2.0), absDepth);
	...
	if(condition == 1)
	{
		if (fingerCalibrationDone==3)
		{
			...
			if(!indexOnPlace && experimentBegun && !endTrial)
			{
				drawFingers();
				...
				glPopMatrix();
			} /*else if(experimentBegun && !endTrial)
			{
				time_progress = timer.getElapsedTimeInMilliSec()/1150.0;
				glTranslated(0, 0, absDepth);
				drawProgressBar(time_progress);
			}*/
			// roughly 1 second from beginning of trial, move the object back
			// and then prepare the second height
			if(timer.getElapsedTimeInMilliSec() > 1100 && timer.getElapsedTimeInMilliSec() < 1150 && h_stimulus==1)
				prepareSecondHeight();
			// after roughly 1 second from onset of second stimulus, move the object back
			// to wait for the next trial
			if(timer.getElapsedTimeInMilliSec() > 1100 && timer.getElapsedTimeInMilliSec() < 1150 && h_stimulus==2)
				moveAway();
		}
	}
	...
	if (button==GLUT_LEFT_BUTTON)
	{
		 if(state==GLUT_DOWN)
		 {
		    if(experimentBegun)
			{
				...
				if(condition == 0)
				{
					if(timer.getElapsedTimeInMilliSec() > 2650)
					{
						advanceTrial(stimulus_height[first_interval] > stimulus_height[second_interval] && stimulus_height[1] == stimulus_height[first_interval], numPoints);
					}
				}
				if(condition == 1)
				{
		 ...
		 if(state==GLUT_DOWN)
		 {
		    if(experimentBegun)
			{
				...
				if(condition == 0)
				{
					if(timer.getElapsedTimeInMilliSec() > 2650)
					{
						advanceTrial(stimulus_height[second_interval] > stimulus_height[first_interval] && stimulus_height[1] == stimulus_height[second_interval], numPoints);
					}
				}
				if(condition == 1)
				{
...
void drawProgressBar(double progress)
{
		...
		glVertex3d(-edge,whereTop.y()+30,0.0);
		glVertex3d(edge,whereTop.y()+30,0.0);
		glVertex3d(edge,whereTop.y()+20,0.0);
		glVertex3d(-edge,whereTop.y()+20,0.0);
		glEnd();
		
		glBegin(GL_QUADS);
		glVertex3d(-edge,whereTop.y()+30,0.0);
		glVertex3d(-edge + edge*(progress+.04)*2,whereTop.y()+30,0.0);
		glVertex3d(-edge + edge*(progress+.04)*2,whereTop.y()+20,0.0);
		glVertex3d(-edge,whereTop.y()+20,0.0);
		glEnd();
}
...
void initTrial()
{
	...
	if(!trial.isEmpty())
	{
		...
		// subject JND for simulation
		double JND = rand_normal(0.0, str2num<double>(parameters.find("JND")));
		...
		//--- comparison
		stimulus_height[1] = trial.getCurrent().second->getCurrentStaircase()->getState() + JND; 
		// EGOCENTRIC DISTANCE
		...
		advanceTrial(stimulus_height[1] > stimulus_height[0], numPoints);
