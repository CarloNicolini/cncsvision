...
void drawParaboloid(double z[max_points], double v[max_points]);
...
#ifndef SIMULATION
string experiment_directory = "S:/Domini-ShapeLab/carlo/2014-2015/3Dcues-paraboloid/october15/";
//string experiment_directory = "C:/Users/visionlab/Desktop/";
#else
string experiment_directory = "/media/shapelab/Domini-ShapeLab/carlo/2014-2015/3Dcues-paraboloid/october15/";
#endif

// parameters file directory and name
string parametersFile_directory = experiment_directory + "parameters_3Dcues-paraboloid_mse.txt";

// response file name
string responseFile_name = "fall15-3Dcues-paraboloid_mse_";

// summary file headers
string responseFile_headers = "subjName\tIOD\ttrialN\trepetitions\tRelDepthObj\tdepth_cue\tAbsDepth\tdimension\tMSE\ttrialDuration\tgood_trial";

// markers file name
string markersFile_name = "fall15-3Dcues-paraboloid_mse_";
...
void initStreams()
{
	...
	// Subject name
	subjectName = parameters.find("SubjectName") + "_" + dimension;
...
void drawParaboloid(double z[max_points], double v[max_points])
{
	...
	{
		double x = fixed_size*sqrt(z[i]/fixed_deltaz)*cos(v[i]);
		double y = fixed_size*sqrt(z[i]/fixed_deltaz)*sin(v[i]);

		glPushMatrix();
		glTranslated(x, y, -z[i]+current_depth);
...
void initVariables()
{
	...
	dimension = parameters.find("dimension");
...
void initTrial()
{
		...
		for(int i = 0; i < num_points; i++)
		{
			...
			z[i] = unifRand(0.0, current_depth);
		}
