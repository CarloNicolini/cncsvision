...
string parametersFile_directory = experiment_directory + "parameters_realObj_wedges_MSEgrasp.txt";

// response file name
string responseFile_name = "spring16-realObj_wedges_";

// summary file headers
string responseFile_headers = "subjName\ttrialN\trepetitions\tCue\tRelDepthObj\ttheta\tFGA\tgood_trial\ttrialDuration";
...
int CCONV PositionChangeHandlerServoRocking(CPhidgetAdvancedServoHandle ADVSERVO, void *usrptr, int Index, double Value)
{
	switch(step)
	{
		...
		case 3:
		{
			if(Index == 1 && abs(Value - (40.0 + (90.0 * 8/9))) < .5)
			{
				servo_rotate(wedgeMotors, 1, motor_theta, 200.0);
				step = 4;
			}
		} break;
		...
		case 5:
		{
			if(motion && handFarFromObject)
			{
					// rocking is controlled by the following loop
				if(Index == 1 && abs(Value - (40.0 + (motor_theta - 7.5) * 8/9)) < .5)
					CPhidgetAdvancedServo_setPosition (ADVSERVO, 1, (40.0 + (motor_theta + 7.5) * 8/9));
				if(Index == 1 && abs(Value - (40.0 + (motor_theta + 7.5) * 8/9)) < .5)
					CPhidgetAdvancedServo_setPosition (ADVSERVO, 1, (40.0 + (motor_theta - 7.5) * 8/9));
			}
		} break;
...
void initTrial()
{
		...
		{
			current_theta = 90.0 + theta - 3.0; // 3 degrees correction due to a misalignment caused by how the box is attached to the motor
			motion = false;
			servo_rotate(wedgeMotors, 3, current_theta);
			servo_rock();
		} else if(current_object == "Motion")
		{
			current_theta = 90.0 - 3.0;
			motion = true;
			servo_rotate(wedgeMotors, 3, current_theta);
			servo_rock();
		} else if(current_object == "StereoMotion")
		{
			current_theta = 90.0 + theta - 3.0;
			motion = true;
			servo_rotate(wedgeMotors, 3, current_theta);
			servo_rock();
		}

		cerr << current_object << " | " << current_depth << " | " << theta << endl;
...
void online_trial()
{
	...
	if(fingerCalibrationDone==4)
	{
		...
		if(task == "grasp")
		{
			if(objdistance - dist_to_home < 50.0)
			{
