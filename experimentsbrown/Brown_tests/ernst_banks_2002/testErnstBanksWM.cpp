...
//OUT #include <direct.h> NOT WORKING ON LINUX
...
/********** EYES AND MARKERS ********************** */
Vector3d eyeLeft(0.0,0.0,0.0), eyeRight(0.0,0.0,0.0), ind(0.0,0.0,0.0), thu(0.0,0.0,0.0), platformIndex(0.0,0.0,0.0), platformThumb(0.0,0.0,0.0);
...
void drawStandardComparison(int display);
	...
	if ( visibleInfo )
	{
		...
		if(interoculardistance>45.0)
		{
			...
			if(condition == 1)
			{
				...
				glColor3fv(glWhite); 
				text.draw(" " );
				text.draw("Visual height: "+ stringify<double[2]>(stimulus_height)+ " [mm]" );
				text.draw("Haptic height: "+ stringify<double>(markers[4].p.y() - markers[3].p.y())+ " [mm]" );
			} else
			{
				text.draw(" ");
				text.draw("Press SPACEBAR to start");
			}
...
void drawStimulus()
{
	...
	{	
		...
		if(timer.getElapsedTimeInMilliSec() >= 150 && timer.getElapsedTimeInMilliSec() <= 1150)
		{
			...
			drawStandardComparison(whichStimulus);
		} else if(timer.getElapsedTimeInMilliSec() >= 1550 && timer.getElapsedTimeInMilliSec() <= 2650)
		{
	...
	if(condition == 1)
	{
		...
		{
			// after roughly 1 second from beginning of trial, move the object back
	...
	if (button==GLUT_LEFT_BUTTON)
	{
		 if(state==GLUT_DOWN)
		 {
		    if(experimentBegun)
			{
				...
				if(condition == 1)
				{
					if(h_stimulus==2 && timer.getElapsedTimeInMilliSec() > 1150)
					{
						if(stimulus_height[first_interval] > stimulus_height[second_interval])
							advanceTrial(true);
						else
...
void prepareSecondHeight()
{
	...
	Vector3d moveReset(0.0,whereTop.y()-20.0,absDepth);
...
{
	// move bottom surface up to (topY - 20.0)
	Vector3d moveBack(0.0,whereTop.y()-20.0,absDepth);
...
void buildStandard(double fluffiness, double distance)
{
	...
	numPoints = ceil(density * 2 * stimulus_visual_angle);
