...
string parametersFile_directory = experiment_directory + "parameters_3Dcues-cylinder_perc.txt";

// response file name
string responseFile_name = "summer15-3Dcues-cylinder_perc_";
...
void drawCylinder(double size, double deltaz)
{
	...
	{
		double Z = deltaz * pow(sin(theta*DEG2RAD), 2);
...
void drawWedge(double size, double deltaz)
{
	...
	for(double Y = -size; Y <= size; Y += step)
	{
		...
		glBegin(GL_LINES);
		glVertex3f(-size - rect_width*0.8, Y, Z);
		glVertex3f(size + rect_width*0.8, Y, Z);
		glEnd();
	}
}
