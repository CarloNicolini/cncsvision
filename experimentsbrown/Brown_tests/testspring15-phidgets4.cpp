...
void phidgets_linear_move(double desired_distance, int * _axis);
// phidgets stepper
...
void reset_phidgets_linear()
{
	...
	for(int stroke = 0; stroke < 3; stroke++)
	{
		...
		linear_distance = 0 - current_position;
...
int CCONV SensorChangeHandler(CPhidgetInterfaceKitHandle IFK, void *phidgetPtr, int Index, int current_position)
{
	...
	double velocity = (double)direction*100.0;
	...
	if(part_distance > 20.0)
	{
		...
		// if the partial distance is less than 30 mm slow down
		if(part_distance < 30.0) // we use slow so that velocity changes only once
		{
			velocity = 25.0;
		} 

		if(!should_travel) // we use slow so that velocity changes only once
		{
			velocity = 0.0;
			stopped = true;
		} 

		CPhidgetMotorControl_setVelocity (motoControl, linear_axis, velocity);
...
//	double current;
	...
	//Change the sensitivity trigger of the sensors and other setups
	for(i = 0; i < numSensors; i++)
	{
	...
	// where is the motor?
	CPhidgetInterfaceKit_getSensorValue(ifKit, linear_axis, &current_position);

	// how much does the motor have to travel?
	linear_distance = desired_distance*10 - current_position;

	// interface kit callback
	CPhidgetInterfaceKit_set_OnSensorChange_Handler (ifKit, SensorChangeHandler, &motoControl);

	// give it a little "kick" (hack to trigger the callback)
	CPhidgetMotorControl_setVelocity (motoControl, linear_axis, 25.0);
...
{   
	...
	{   
		...
		case '8':
		{
			linear_axis = which_axis%2;
			phidgets_linear_move(phi_distance, &linear_axis);
		} break;
