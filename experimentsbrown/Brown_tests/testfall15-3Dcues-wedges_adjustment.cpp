...
void drawCylinder(double x[max_points], double y[max_points], double deltaz);
...
string parametersFile_directory = experiment_directory + "parameters_3Dcues-wedges_adjustment.txt";

// response file name
string responseFile_name = "fall15-3Dcues-wedge_adjustment_";
...
/*
	...
	{
		double Z = C / 2.0 * (pow(x[i], 2) + pow(y[i], 2));
		if(x[i]*x[i] + pow(y[i], 2) < pow(trial.getCurrent()["RelDepthObj"]/2.0, 2))
		{
			glPushMatrix();
			glTranslated(x[i], y[i], -Z);
...
void drawCylinder(double x[max_points], double y[max_points], double deltaz)
{
	for(int i = 0; i < max_points; i++)
	{
		double z_coord = - sqrt(pow(deltaz, 2) - pow(x[i], 2)) + deltaz/2.0;
		z_coord = z_coord * signZcoord[i];
		glPushMatrix();
		glTranslated(y[i], x[i], z_coord);
		glutSolidSphere(pointsT,10,10);
		glPopMatrix();
	}
}
...
//		draw_rampwings_texture(current_depth, current_rampLength, current_rampTheta);
	...
	else
	{
		if(obj == "pedestal")
		{
			...
			//drawCylinder(x_coord_p, y_coord_p, current_depth);
		}
		else
		{
			//drawThreeRods(current_depth);
			...
			drawParaboloid(z, v, depth_pedestal, current_depth);
			//drawCylinder(x_coord_s, y_coord_s, current_depth);
		}
	}
}
...
/*
		x_coord_p[i] = unifRand(-depth_pedestal, depth_pedestal);
		y_coord_p[i] = unifRand(-depth_pedestal, depth_pedestal);

		x_coord_s[i] = unifRand(-depth_pedestal, depth_pedestal);
		y_coord_s[i] = unifRand(-depth_pedestal, depth_pedestal);
*/
		signZcoord[i] = unifRand(-1.0,1.0);
	}
