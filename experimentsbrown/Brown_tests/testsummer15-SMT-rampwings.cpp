...
// stimuli
StimulusDrawer stereocheckDrawer[4], fixCrossDrawer[2], rampPedestalDrawer, rampStimulusDrawer, frontPlaneDrawer, backRearDrawer;
BoxNoiseStimulus stereocheck[4], fixCross[2], frontPlane, rampPedestal, rampStimulus, backRear;
...
double rampLengthPedestal = 0.0, rampLengthStimulus = 0.0;
double rampThetaPedestal = 0.0, rampThetaStimulus = 0.0;
...
double frontPlaneX = 0.0, rampX = 0.0;
double frontPlaneK = 0.0, backRearK = 0.0;
double current_rampLength = 0.0, current_rampTheta = 0.0;
...
void build_rampwings_dots(double length_pedestal, double length_stimulus);
void build_fixation_cross();
void build_objects();

void draw_rampwings_dots(double deltaz);
void draw_stereocheck();
void draw_fixation_cross();
void draw_rampwings_texture(double deltaz, double rampLength, double rampTheta);
...
#ifndef SIMULATION
string experiment_directory = "S:/Domini-ShapeLab/carlo/2014-2015/SMT-rampwings/";
//string experiment_directory = "C:/Users/visionlab/Desktop/";
#else
string experiment_directory = "/media/shapelab/Domini-ShapeLab/carlo/2014-2015/SMT-rampwings/";
#endif

// paramters file directory and name
string parametersFile_directory = experiment_directory + "parameters_SMT-rampwings_perc.txt";

// response file name
string responseFile_name = "summer15-SMT-rampwings_perc_";

// summary file headers
string responseFile_headers = "subjName\tIOD\ttrialN\tAbsDepth\tobjheight\tdepth_pedestal\tdepth_stimulus\tfirst_object\tsecond_object\tdepth_cue_pedestal\tdepth_cue_stimulus\tstairID\tstairState\tstairReversals\tstairAscending\tstairStepsDone\tresponsekey\tresp\tDepthCue\ttrialDuration";
...
{
	// ramp pedestal
	rampPedestal.setDimensions(length_pedestal, objheight, 0.1);;
	rampPedestal.setNpoints(length_pedestal*objheight*0.225);
	rampPedestal.compute();

	rampPedestalDrawer.setStimulus(&rampPedestal);
	rampPedestalDrawer.setSpheres(true);
	rampPedestalDrawer.initList(&rampPedestal, glRed);

	// ramp stimulus
	rampStimulus.setDimensions(length_stimulus, objheight, 0.1);;
	rampStimulus.setNpoints(length_stimulus*objheight*0.225);
	rampStimulus.compute();

	rampStimulusDrawer.setStimulus(&rampStimulus);
	rampStimulusDrawer.setSpheres(true);
	rampStimulusDrawer.initList(&rampStimulus, glRed);

	// left front
	frontPlane.setDimensions(frontPlaneX, objheight, 0.1);
	frontPlane.setNpoints(frontPlaneX * objheight* 0.225);
	frontPlane.compute();

	frontPlaneDrawer.setStimulus(&frontPlane);
	frontPlaneDrawer.setSpheres(true);
	frontPlaneDrawer.initList(&frontPlane, glRed);

	// back rear
	backRear.setDimensions(rampX, objheight*1.5, 0.1);
	backRear.setNpoints(rampX * objheight*1.5 * 0.225);
	backRear.compute();

	backRearDrawer.setStimulus(&backRear);
	backRearDrawer.setSpheres(true);
	backRearDrawer.initList(&backRear, glRed);
}
...
void build_objects()
{
	if (!haveBothObjectsTexture)
		build_rampwings_dots(rampLengthPedestal, rampLengthStimulus);
...
void draw_rampwings_texture(double deltaz, double rampLength, double rampTheta)
{
	// compression constants for textures (frontPlane and backRear)
	frontPlaneK = frontPlaneX / rampLength;
	backRearK = rampX / rampLength;
	...
	glRotated(rampTheta, 0, 1, 0);
	glBegin(GL_QUADS);
	// ramp
	glTexCoord2f(0.0, 0.0); glVertex3f(-rampLength/2, -objheight/2, 0);
	glTexCoord2f(0.0, 1.0); glVertex3f(-rampLength/2, objheight/2, 0);
	glTexCoord2f(1.0, 1.0); glVertex3f(rampLength/2, objheight/2, 0);
	glTexCoord2f(1.0, 0.0); glVertex3f(rampLength/2, -objheight/2, 0);
	glEnd();

	glRotated(-rampTheta, 0, 1, 0);
	glBegin(GL_QUADS);
	// front left
	glTexCoord2f(0.0, 0.0); glVertex3f(-rampX/2, -objheight/2, deltaz/2);
	glTexCoord2f(0.0, 1.0); glVertex3f(-rampX/2, objheight/2, deltaz/2);
	glTexCoord2f(frontPlaneK, 1.0); glVertex3f(-(frontPlaneX + rampX/2), objheight/2, deltaz/2);
	glTexCoord2f(frontPlaneK, 0.0); glVertex3f(-(frontPlaneX + rampX/2), -objheight/2, deltaz/2);

	// back rear
	glTexCoord2f(0.0, 0.0); glVertex3f(-rampX/2, -objheight*3/4, -deltaz/2);
	glTexCoord2f(0.0, 1.0); glVertex3f(-rampX/2, objheight*3/4, -deltaz/2);
	glTexCoord2f(backRearK, 1.0); glVertex3f(rampX/2, objheight*3/4, -deltaz/2);
	glTexCoord2f(backRearK, 0.0); glVertex3f(rampX/2, -objheight*3/4, -deltaz/2);
...
void draw_rampwings_dots(double deltaz)
{
	...
	{
		// Oblique plane
		glRotated(rampThetaStimulus, 0, 1, 0);
		rampStimulusDrawer.draw();
	} else 
	{
		glRotated(rampThetaPedestal, 0, 1, 0);
		rampPedestalDrawer.draw();
	}
	glPopMatrix();

	// Front left
	glPushMatrix();
	glTranslated(-(frontPlaneX/2 + rampX/2), 0, deltaz/2);
	frontPlaneDrawer.draw();
	glPopMatrix();
	...
	} else if ( isStimulusDrawn )
	{
		...
		if (timer.getElapsedTimeInMilliSec() > 200 && timer.getElapsedTimeInMilliSec() < trial_time+200)
		{
			...
			if (first_object == "pedestal")
			{
				...
				current_rampLength = rampLengthPedestal;
				current_rampTheta = rampThetaPedestal;
			} else
			{
				current_cue = depth_cue_stimulus;
				current_depth = depth_stimulus;
				current_rampLength = rampLengthStimulus;
				current_rampTheta = rampThetaStimulus;
			}
			...
			if (current_cue.find('t') != std::string::npos)
				...
				draw_rampwings_dots(current_depth);
		} 
		...
		cerr << current_object << " with " << current_depth << endl;
	}
}
...
void initVariables()
{
	...
	frontPlaneX = str2num<double>(parameters.find("frontPlaneX"));
	rampX = str2num<double>(parameters.find("rampX"));
...
//		bool decision = ((depth_stimulus + JND) > (depth_pedestal + PSE) && first_object == "stimulus") || (depth_stimulus + JND) > (depth_pedestal + PSE) && second_object == "stimulus";
	...
	//----- calculate the length of the ramp of the two objects
	rampLengthPedestal = sqrt( pow(depth_pedestal, 2) + pow(rampX, 2) );
	rampLengthStimulus = sqrt( pow(depth_stimulus, 2) + pow(rampX, 2) );

	// calculate the slant of the oblique sides
	rampThetaPedestal = atan( depth_pedestal/rampX ) * 180/M_PI;
	rampThetaStimulus = atan( depth_stimulus/rampX ) * 180/M_PI;
