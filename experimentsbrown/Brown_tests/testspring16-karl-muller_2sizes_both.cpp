...
string parametersFile_directory = experiment_directory + "parameters_spring16-karl-muller_2sizes_both.txt";

// response file name
string responseFile_name = "spring16-karl-muller_xy_12_";
...
void drawInfo()
{
	if ( visibleInfo )
	{
		...
		text.draw(" ");
		text.draw("####### testspring16-karl-muller_xy_12 #######");
		text.draw("");
...
void drawMullerLyer(double body_width, double body_height, double finAngle)
{
	...
	if(finAngle < 90) {
		vertDisplacement[0] = body_height/2.0+yCorr-teeth_width*sin(DEG2RAD*finAngle);
		vertDisplacement[1] =  -yCorr-body_height/2.0+teeth_height*sin(DEG2RAD*finAngle);
		vertDisplacement[2] =  body_height/2.0+yCorr-teeth_height*sin(DEG2RAD*finAngle);
		vertDisplacement[3] =  -yCorr-body_height/2.0+teeth_height*sin(DEG2RAD*finAngle);
	}
...
//		Vector3d object_position(0.0,object_reset_position.y(),objdistance-depth_offset); // depth offset depends on the block
		...
		// rotate teh carousel
		if(unifRand(-1.0, 1.0) > 0) {
			stepper_rotate(rotTable, target_angle-30.0);
		}else {
			stepper_rotate(rotTable, target_angle+50.0);
		}
