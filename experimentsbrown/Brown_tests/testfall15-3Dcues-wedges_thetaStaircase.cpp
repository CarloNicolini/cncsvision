...
string upper_object = "empty", lower_object = "empty";
...
double theta_pedestal = 0.0, theta_stimulus = 0.0, theta_sign = 0.0;
...
string parametersFile_directory = experiment_directory + "parameters_3Dcues-wedge_thetaStaircase_perc.txt";

// response file name
string responseFile_name = "summer15-3Dcues-wedge_thetaStaircase_perc";

// summary file headers
string responseFile_headers = "subjName\tIOD\ttrialN\tAbsDepth\tobjheight\tobjtheta\ttheta_stimulus\tupper_object\tlower_object\tdepth_cue_pedestal\tdepth_cue_stimulus\tstairID\tstairState\tstairReversals\tstairAscending\tstairStepsDone\tresponsekey\tresp\tDepthCue\ttrialDuration";
...
void drawInfo()
{
	if ( visibleInfo )
	{
		...
		if(upper_object == "pedestal")
		{
			text.draw("upper depth: " + stringify<double>(depth_pedestal));
			text.draw("lower depth: " + stringify<double>(depth_stimulus));
		} else
		{
			text.draw("upper depth: " + stringify<double>(depth_stimulus));
			text.draw("lower depth: " + stringify<double>(depth_pedestal));
		}
...
//	glRotated(90, 0, 1, 0);
	...
	// Left occluder
	glTranslated( -(objheight + rect_width/2.0), 0, 0);
	...
	// Right occluder
	glTranslated(2.0 * (objheight + rect_width/2.0), 0, 0);
	glColor3fv(glDarkRed);
	glBegin(GL_QUADS);
	glVertex3f(-rect_width/2.0, objheight+5.0, 0);
	glVertex3f(-rect_width/2.0, -objheight-5.0, 0);
	...
	glTranslated( - 2.0 * (50.0/2.0 + objheight + rect_width), 0, 0);
	glColor3fv(glBlack);
	glBegin(GL_QUADS);
	glVertex3f(-50.0/2.0, objheight+5.0, 0);
	glVertex3f(-50.0/2.0, -objheight-5.0, 0);
...
//	glRotated(90, 0, 1, 0);
	...
	if (current_cue.find('s') != std::string::npos)
	{
		...
		//glRotated(theta_stimulus, 0, 0, 1);
	}
	else
		IOD = 0.0;
...
//		draw_rampwings_texture(current_depth, current_rampLength, current_rampTheta);
		...
		drawWedge(objheight, current_depth);
}
...
void drawStimulus()
{
	if(checkglasses)
	{
		IOD = interoculardistance;
		draw_stereocheck();
	} else if ( isStimulusDrawn && !inPause )
	{
		glLoadIdentity();
		glTranslated(0, 0, objdistance);
//		glRotated(90, 0, 1, 0);
...
void initTrial()
{
	...
	isTrialDone = false;
	motion = 0.0;
	jitterz = unifRand(0.0, jitterz_max);
	...
	{
		upper_object = "pedestal";
		lower_object = "stimulus";
		direction_motion = 1.0;
		direction_motion2 = 1.0;
	} else 
	{
		upper_object = "stimulus";
		lower_object = "pedestal";
		direction_motion = - 1.0;
		direction_motion2 = - 1.0;
	}
	...
	if(experiment)
	{
		...
		// set the theta of the moving object (pedestal)
		theta_pedestal = 0.0;

		// set the theta of the stimulus
		theta_stimulus = trial.getCurrent().second->getCurrentStaircase()->getState();
...
{   
	...
	{   
		...
		case '8': // stimulus shallower than pedestal
		{
			if(training)
				initTrial();
			else
		...
		case '5': // stimulus deeper than pedestal
		{
			if(training)
				initTrial();
			else
