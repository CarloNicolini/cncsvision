...
void drawCylinder(double size, double deltaz);
...
#ifndef SIMULATION
string experiment_directory = "S:/Domini-ShapeLab/carlo/2014-2015/3Dcues-cylinder/";
//string experiment_directory = "C:/Users/visionlab/Desktop/";
#else
string experiment_directory = "/media/shapelab/Domini-ShapeLab/carlo/2014-2015/3Dcues-cylinder/";
#endif

// paramters file directory and name
string parametersFile_directory = experiment_directory + "parameters_3Dcues-cylinder_thetaStaircase_perc.txt";

// response file name
string responseFile_name = "summer15-3Dcues-cylinder_thetaStaircase_perc_";

// summary file headers
string responseFile_headers = "subjName\tIOD\ttrialN\tAbsDepth\tobjheight\tobjtheta\ttheta_stimulus\tfirst_object\tsecond_object\tdepth_cue_pedestal\tdepth_cue_stimulus\tstairID\tstairState\tstairReversals\tstairAscending\tstairStepsDone\tresponsekey\tresp\tDepthCue\ttrialDuration";
...
void drawCylinder(double size, double deltaz)
{
	...
	double step = 180/ (objdensity * 180);

	for(double theta = 0; theta <= 180; theta += step)
	{
		double Z = deltaz * pow(sin(theta*DEG2RAD), expo + 1);
		double Y = size * cos(theta*DEG2RAD);
...
void drawWedge(double size, double deltaz)
{
	...
	for(double Y = -size; Y <= size; Y += step)
	{
		...
		glBegin(GL_LINES);
		glVertex3f(-size - rect_width*0.6, Y, Z);
		glVertex3f(size + rect_width*0.6, Y, Z);
		glEnd();
	}
}
...
void draw_occlusors()
{
	...
	glBegin(GL_QUADS);
	glVertex3f(-objheight-rect_width, objheight+5.0, 0);
	glVertex3f(-objheight-rect_width, -objheight-5.0, 0);
	glVertex3f(-objheight+5.0, -objheight-5.0, 0);
	glVertex3f(-objheight+5.0, objheight+5.0, 0);
	glEnd();
...
void draw_all(string obj)
{
	...
	glTranslated(0, 0, -current_depth-1.0);
...
//		drawCylinder(objheight, current_depth);
