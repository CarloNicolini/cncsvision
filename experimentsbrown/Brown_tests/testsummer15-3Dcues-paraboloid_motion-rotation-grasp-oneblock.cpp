...
string parametersFile_directory = experiment_directory + "parameters_3Dcues-paraboloid_grasp-oneblock.txt";

// response file name
string responseFile_name = "summer15-3Dcues-paraboloid_grasp-oneblock_";

// summary file headers
string responseFile_headers = "subjName\tIOD\ttrialN\trepetitions\tobject\tRelDepthObj\tdepth_cue\tAbsDepth\tobjheight\ttrialDuration\tgood_trial";

// markers file name
string markersFile_name = "summer15-3Dcues-paraboloid_grasp-oneblock_";
...
void initTrial()
{
		...
		if(experiment)
		{
			...
			if(current_object == 1) // if it's the pedestal
				current_depth = nominal_depth;
			else // if it's the stimulus
			{
				if(trial.getCurrent()["Cue"] == 1) // if it's stereo
					current_depth = pse_object1;
				else 				 // if it's stereomotion
