...
double obj1_dz1 = 0.0, obj1_dz2 = 0.0, obj1_dz3 = 0.0, obj2_dz1 = 0.0, obj2_dz2 = 0.0, obj2_dz3 = 0.0;
double wedgetheta = 0.0;
double x_coord[max_points], y_coord[max_points];

double motion_step = 0.0, motion_theta = 0.0;
double objtheta = 90.0, objdensity = 0.0;
double objwidth = 0.0, objheight = 0.0;

// openGL
GLfloat glDarkRed[3] = {0.9,0,0};
...
#ifndef SIMULATION
string experiment_directory = "S:/Domini-ShapeLab/carlo/2014-2015/3Dcues-wedge/grasp/";
//string experiment_directory = "C:/Users/visionlab/Desktop/";
#else
string experiment_directory = "/media/shapelab/Domini-ShapeLab/carlo/2014-2015/3Dcues-wedge/grasp/";
#endif

// parameters file directory and name
string parametersFile_directory = experiment_directory + "parameters_3Dcues-wedge_grasp.txt";

// response file name
string responseFile_name = "fall15-3Dcues-wedge_mse_";

// summary file headers
string responseFile_headers = "subjName\tIOD\ttrialN\trepetitions\tobject\tRelDepthObj\tdepth_cue\tAbsDepth\ttrialDuration\tgood_trial";

// markers file name
string markersFile_name = "fall15-3Dcues-wedge_mse_";
...
void draw_all()
{
	glLoadIdentity();
	glTranslated(0, 0, objdistance);
	glTranslated(0,0,-current_depth/2.0);
	glRotated(0, 0, 0, 1);
	...
	{
		drawFrontCar(x_coord, y_coord, objheight, current_depth);
	}
}
...
void initVariables()
{
	...
	obj1_dz1 = str2num<double>(report.find("obj1_dz_stereo"));
	obj1_dz2 = str2num<double>(report.find("obj1_dz_stereomotion"));

	obj2_dz1 = str2num<double>(report.find("obj2_dz_stereo"));
	obj2_dz2 = str2num<double>(report.find("obj2_dz_stereomotion"));

	objwidth = str2num<double>(report.find("objwidth"));
...
void initTrial()
{
		...
		if(experiment)
		{
			...
			// what depth does the object have?
		
			if (trial.getCurrent()["objDz"] == 1)
			{
				current_cue = "s";
				if(current_object == 1)
					current_depth = obj1_dz1;
				if(current_object == 2)
					current_depth = obj2_dz1;
			}

			if (trial.getCurrent()["objDz"] == 2)
			{
				current_cue = "sm";
				if(current_object == 1)
					current_depth = obj1_dz2;
				if(current_object == 2)
					current_depth = obj2_dz2;
			}
		}
		...
		// set the phidgets
		phidgets_linear_move(current_depth-8.0, axisZ);
		...
		// calculate the semi-angle at the vertex of the wedge
		wedgetheta = atan((right_edge/2)/current_depth);
...
void online_trial()
{
	if(fingerCalibrationDone==4)
	{
		...
		if(experiment)
		{
			index_behind_the_stimulus = (abs(ind.x()) < objheight/2 && ind.z() < (objdistance-(current_depth/2)));
			index_inside_the_stimulus = (atan(ind.x()/abs(ind.z()-objdistance)) < wedgetheta && ind.z() < objdistance && ind.z() > (objdistance-(current_depth/2)));
