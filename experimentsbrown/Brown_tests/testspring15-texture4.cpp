...
void makeCheckImage()
{
	...
	heightMapBuilder.SetDestSize (checkImageWidth, checkImageHeight);
	heightMapBuilder.SetBounds (1.0, 10.0, 1.0, 10.0);
	heightMapBuilder.Build ();

	const float* pSource3 = heightMap.GetConstSlabPtr(1);
...
void drawDots()
{
	...
	glVertex3f(-100.0f, -200.0f, 0.5f);		// Top Left
	glVertex3f( 100.0f, -200.0f, 0.5f);		// Top Right
	glVertex3f( 100.0f, -30.0f, 0.5f);		// Bottom Right
	glVertex3f(-100.0f, -30.0f, 0.5f);		// Bottom Left

	glVertex3f(-100.0f, 30.0f, 0.5f);		// Top Left
	glVertex3f( 100.0f, 30.0f, 0.5f);		// Top Right
...
void idle()
{
	...
	if(motion > -60.0)
		motion -= trial.getCurrent()["Motion"];
	else
		motion = 60.0;
...
void initGLVariables()
{
	...
	cylinder.setRadiusAndHeight(objlength, 0.0, 60.0); // raggio (mm) altezza (mm)
