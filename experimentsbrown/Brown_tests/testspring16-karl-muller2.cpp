...
string parametersFile_directory = experiment_directory + "parameters_spring16-karl-muller.txt";

// response file name
string responseFile_name = "spring16-karl-muller_";

// summary file headers
string responseFile_headers = "subjName\tIOD\ttrialN\trepetitions\tMLheight\tAbsDepth\ttrialDuration\tgood_trial";

// markers file name
string markersFile_name = "spring16-karl-muller_";

// response file headers
string markersFile_headers = "subjName\tIOD\ttrialN\ttime\tglobalTime\tframeN\tindexXraw\tindexYraw\tindexZraw\tthumbXraw\tthumbYraw\tthumbZraw\teyeRXraw\teyeRYraw\teyeRZraw\teyeLXraw\teyeLYraw\teyeLZraw\tfingersOccluded\tframesOccluded\tAbsDepth\tMLheight\tframesToGrasp";
...
void drawMullerLyer(double body_width, double body_height, double finAngle)
{
	...
	double teeth_width = 5.0;
	double teeth_height = 10.0;
	...
	// main body
	glLoadIdentity();
	glTranslated(0.0, 0.0, objdistance);
	...
	{
		glLoadIdentity();
		glTranslated(latDisplacement[i], vertDisplacement[i], objdistance);
...
void draw_all()
{
	...
	drawMullerLyer(15.0, current_height, current_angle);
}
