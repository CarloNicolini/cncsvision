...
// stimuli
StimulusDrawer stereocheckDrawer[4], fixCrossDrawer[2], cageDrawer;
BoxNoiseStimulus stereocheck[4], fixCross[2], cage;
...
double stereocheckZ[4] = {10.0, 25.0, 40.0, 55.0};
double objtheta = 0.0, objdensity = 0.0, theta_sign = 0.0;
double direction_motion2 = 0.0;
double shuffleZ = 0.0;
double x_coord[max_points], y_coord[max_points];
...
// stimuli
void build_cage(double x, double y, double z);
void drawWedge(double width, double height, double deltaz);
...
#ifndef SIMULATION
string experiment_directory = "S:/Domini-ShapeLab/carlo/2014-2015/3Dcues-wedge/";
//string experiment_directory = "C:/Users/visionlab/Desktop/";
#else
string experiment_directory = "/media/shapelab/Domini-ShapeLab/carlo/2014-2015/3Dcues-wedge/";
#endif

// paramters file directory and name
string parametersFile_directory = experiment_directory + "parameters_3Dcues-wedges_adjustment-ss.txt";

// response file name
string responseFile_name = "fall15-3Dcues-wedge_adjustment-ss_";

// summary file headers
string responseFile_headers = "subjName\tIOD\ttrialN\tAbsDepth\tobjheight\tobjtheta\tdepth_pedestal\tdepth_stimulus\tfixed_object\tadjusted_object\tdepth_cue_pedestal\tdepth_cue_stimulus\tDepthCue\ttrialDuration";
...
void drawInfo()
{
	if ( visibleInfo )
	{
		...
		glColor3fv(glWhite);
		text.draw("####### fall15-3Dcues-wedge_adjustment-ss #######");
		text.draw("");
...
void drawWedge(double width, double height, double deltaz)
{
	double step = 2.0*height / (objdensity - 1.0);

	for(double Y = -height; Y <= height; Y += step)
	{
		double Z = - abs(deltaz/height*Y) + deltaz/2.0;
...
/*
	...
	{
		double z_coord = - abs(deltaz/height*y_coord[i]) + deltaz/2.0;
		glPushMatrix();
		glTranslated(x_coord[i], y_coord[i], z_coord);
		glColor3fv(glRed);
...
{
	// BOX left
	cage.setDimensions(x, y, z);
	cage.setNpoints((int)100 * x*y*z/10);
	cage.compute();

	cageDrawer.setStimulus(&cage);
	cageDrawer.setSpheres(true);
	cageDrawer.initList(&cage, glRed);
}
...
//	glRotated(90, 1, 0, 0);
	...
	glTranslated(-25.0, 25.0, 0.0);
	glRotated(motion, 0, 1, 0);
	drawWedge(objwidth, objheight, stereocheckZ[0]);

	// Upper right
	glLoadIdentity();
	glTranslated(0, 0, objdistance);
	glTranslated(25.0, 25.0, 0.0);
	glRotated(motion*1.1, 0, 1, 0);
	drawWedge(objwidth, objheight, stereocheckZ[1]);

	// Lower left
	glLoadIdentity();
	glTranslated(0, 0, objdistance);
	glTranslated(-25.0, -25.0, 0.0);
	glRotated(motion*1.2, 0, 1, 0);
	drawWedge(objwidth, objheight, stereocheckZ[2]);

	// Lower right
	glLoadIdentity();
	glTranslated(0, 0, objdistance);
	glTranslated(25.0, -25.0, 0.0);
	glRotated(motion*1.3, 0, 1, 0);
	drawWedge(objwidth, objheight, stereocheckZ[3]);
}
...
{	
	...
	glLoadIdentity();
	glTranslated(0, 0, objdistance);
	glTranslated(0,0,-current_depth/2 - 2.0);
//	glRotated(90, 0, 1, 0);
...
//		draw_rampwings_texture(current_depth, current_rampLength, current_rampTheta);
	...
	{
		//draw_cage();
		drawWedge(objwidth, objheight, current_depth);
	}
}
...
void drawGLScene()
{
    ...
    {   
		...
		if(current_cue.find('s') != std::string::npos && !checkglasses)
		{
			drawStimulus();
			drawInfo();
		}
...
void initTrial()
{
	...
	if(training)
	{
		depth_pedestal = unifRand(20.0, 60.0);
	...
	// calculate the x and y coords of the points on the two wedges
	for(int i = 0; i < max_points; i++)
	{
		x_coord[i] = unifRand(-objwidth, objwidth);
		y_coord[i] = unifRand(-objheight, objheight);
	}
