...
bool visibleInfo=true, visibleSpecs=false;
...
{   switch (key)
    ...
    break;
	  case '4': // the stimulus is perceptually flatter than an equilateral triangle
      {  
		/*responsekey=4;
		if ( trial.isEmpty() )
			exit(0);
		//if ( trial.getCurrent().first["RelDepthObj"] > abs(trial.getCurrent().second->getCurrentStaircase()->getState()) )
			advanceTrial(false);
		//else
			//advanceTrial(false);
			*/
      ...
      break;
      case '6': // the stimulus is perceptually more elongated than an equilateral triangle
      {  
		/*responsekey=6;
		if ( trial.isEmpty() )
			exit(0);
