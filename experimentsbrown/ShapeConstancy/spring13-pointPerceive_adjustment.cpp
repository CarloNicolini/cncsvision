...
/********** EYES AND MARKERS **********************/
Vector3d eyeLeft, eyeRight,  thumb;
...
void drawInfo()
{
	...
	text.draw("# frame: " +stringify<int>(frame));
	text.draw("#");
	/*
...
void drawStimulus()
{
	...
	// Rear Left rod
	glPushMatrix();
	glTranslated(-(trial.getCurrent()["RelDepthObj"]/2), 0, -((trial.getCurrent()["RelDepthObj"]/2)+dz/2));
	stimDrawer[1].draw();
	glPopMatrix();
...
void initStreams()
{
	...
	parametersFile.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parameterspointPerceive_adjustment.txt");
    ...
    string responseFileName =  "pointPerceive_adjustment_"   + subjectName + ".txt";
	...
	responseFile << fixed << "NSubjName\tIOD\ttrialN\tAbsDepth\tRelDepthObj\tzedge\tAdjDepth\tjitter\ttrialDuration" << endl;
