...
static const Vector3d centercal(-21.8,-327.2,-220.0);
Screen screen;
...
void drawStimulus()
{
	...
	glPushMatrix();
	glLoadIdentity();
	glTranslated(0,0,factors["AbsDepth"]+30/2);
	stimDrawer[0].draw();
	glPopMatrix();
	...
	glPushMatrix();
	glLoadIdentity();
	glTranslated(-40/2,0,factors["AbsDepth"]-30/2);
	stimDrawer[1].draw();
	glPopMatrix();
...
void initTrial()
{
	...
	Vector3d pos(0, -44, factors["AbsDepth"]);
...
void initGLVariables()
{
	...
	for (int i=0; i<3; i++)
	{
		...
		cylinder[i].setRadiusAndHeight(4.5,50); // raggio (mm) altezza (mm)
...
void initStreams()
{
	...
	parametersFile.open("../parametersFiles/parameterstestObject.txt");
