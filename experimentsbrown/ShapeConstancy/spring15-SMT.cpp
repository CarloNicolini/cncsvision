...
void drawInfo()
{
	if ( visibleInfo )
	{
		...
		text.draw("# objtheta: " +stringify<double>(objtheta));
		text.draw("#");
		text.draw("#######################");
		text.draw(" ");
...
void initTrial()
{
	...
	// set the phidgets
	stepper_rotate(rotTable, 90-objtheta);
	phidgets_linear_move(objlength-30, axisZ);
...
/*
			...
			Vector3d object_position(0.0,-172.0,object_reset_position.z());
