...
// square measures
double edge = 0.0, dedge = 0, jitterTime = 0.0, xedge = 0.0, zedge = 0.0, jitter = 0.0, theta=0, phi=M_PI*3/4, dz = 0.0, dx = 0.0, r = 0.0, fdx = 1.0;
Timer timer;
Timer globalTimer;
double timeToFHP=0.0;
...
bool resp;
double handZpos = 0.0;
bool frontRodAppearedOnce = false;
...
//	text.draw("Frame= " + stringify< double >(lastPos[0].z() ));
	text.draw("TimeStim= " + stringify<int>(timeStim));
	text.draw("Condition= " + stringify<double>(trial.getCurrent().first["Condition"]));
...
void drawStimulus()
{
	...
	double objDepth = trial.getCurrent().second->getCurrentStaircase()->getState();
	if(trial.getCurrent().first["Condition"]==1)
	{
	    if(frontRodShown)
		{
			if(index.z() < handZpos + 30.0 && timer.getElapsedTimeInMilliSec() < timeToFHP + 800)
			{
				// Front rod
				glPushMatrix();
				...
				// Rear Left rod
				glPushMatrix();
				glTranslated(-(trial.getCurrent().first["RelDepthObj"]/2), 0, -objDepth/2);
				stimDrawer[1].draw();
				glPopMatrix();
	...
	{
		if(timer.getElapsedTimeInMilliSec() > (1000 + jitterTime) && timer.getElapsedTimeInMilliSec() < 1000 + jitterTime + 800)
		{
			// Front rod
			glPushMatrix();
			glTranslated(0, 0, objDepth/2);
...
void advanceTrial(bool response)
{
    ...
    responseFile << 
					...
					trial.getCurrent().first["Condition"] << "\t" <<
					jitterTime << "\t" <<
					endl;
	resp=response;
	if( !trial.isEmpty() )
...
void idle()
{
	...
	if (fingerCalibrationDone==3 )
	{
		...
		if(frontRodShown && !frontRodAppearedOnce)
		{
			handZpos = index.z();
			timeToFHP = timer.getElapsedTimeInMilliSec();
			frontRodAppearedOnce = true;
		} else
		{
			handZpos = handZpos;
			timeToFHP = timeToFHP;
			frontRodAppearedOnce = frontRodAppearedOnce;
		}
...
void initStreams()
{
	...
	parametersFile.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parametersstaircasePointingMixC.txt");
	...
	string markersFileName  = "staircasePointingMC_markersFile_" + subjectName + ".txt";
    string responseFileName =  "staircasePointingMC_"   + subjectName + ".txt";
	...
	responseFile << fixed << "subjName\tIOD\ttrialN\tRelDepthObj\tStairID\tStairState\tReversals\tAscending\tresponsekey\tresp\tjitter\ttime\tstepsDone\tobjdepth\tCondition\tjitterTime" << endl;
