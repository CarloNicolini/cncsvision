...
const int numObj=4;
int objId = 0;
double objSizes[3] = {40,45,50,55};
double x_shift[3] = {0.0, 0.0, 0.0, 0.0};
double y_shift[3] = {0.0, 0.0, 0.0, 0.0};
double z_shift[3] = {0.0, 0.0, 0.0, 0.0};
// Size and position variables for the target objects
double targetOriginsX[3] = {45.0, 45.0, 45.0, 45.0};
double targetOriginsY[3] = {0.0, 0.0, 0.0, 0.0};
double targetOriginsZ[3] = {-400.0, -400.0, -400.0, -400.0};
...
// stepper variable
double objStepperPosns[3] = {0, 80, 160, 240};
__int64 pos;
...
string experiment_directory = "S:/Domini-ShapeLab/evan/spring16-gripAdaptVisual135/";
// paramters file directory and name
string parametersFile_directory = experiment_directory + "spring16-gripAdaptVisual135_parameters.txt";
// summary file headers
string trialFile_headers = "subjName\ttrialN\tattempt\tobjId\tobjHeight\tperturbation\tframeN\tfingersOccluded\ttime\tdistanceToObject\tfingersOff\treachedObject\tindexXraw\tindexYraw\tindexZraw\tthumbXraw\tthumbYraw\tthumbZraw\tGA\ttargetX\ttargetY\ttargetZ";
/*************************** FUNCTIONS ***********************************/
...
void initStreams()
{
	...
	direction = 1;//str2num<double>(parameters.find("Direction"));
...
void initTrial()
{
	...
	perturb=0;
	showObject=true;
	double temp_reps = str2num<double>(parameters.find("Repetitions"));
	if (trialNumber>(temp_reps-1) && trialNumber<=(2*temp_reps-1)){ // 45 - 90 is the range of perturbation trials
		perturbation=1;
		perturb= direction*2.5;
	}
	// Get current trial info
	if (training){
		objId = (trainTrialNumber%4)+1;
		if (practice)
			edge = unifRand(10,25);
	}else{
		objId = trial.getCurrent()["objId"];
		if(perturbation)
			objId=objId+1; // use the larger blocks as well!
	}
