...
double C[96][8];
int rows=0,cols=0, frame=0, judgment=0;
bool frontRodShown=false, beginTask=false;
...
void drawInfo()
{
	...
	text.draw("# frameMax: " +stringify<double>(C[trialNumber][1]*11.5));
	text.draw("# frame+CS: " +stringify<double>(C[trialNumber][1]+C[trialNumber][2]*11.5));
	text.draw("#");
	/*
...
void drawStimulus()
{
	...
	glTranslated(0.0,0,C[trialNumber][5]);
	
		if(timer.getElapsedTimeInMilliSec() > C[trialNumber][1]*11.5 && timer.getElapsedTimeInMilliSec() < (C[trialNumber][1]+C[trialNumber][2])*11.5)
		{
			frontRodShown=true;
			// Front rod
			glPushMatrix();
			glTranslated(0, 0, C[trialNumber][7]/2 );
			stimDrawer[0].draw();
			glPopMatrix();
		}
	...
	// Rear Left rod
	glPushMatrix();
	glTranslated(-(estdepth/2), 0, -((estdepth/2)+dz/2));
	stimDrawer[1].draw();
	glPopMatrix();
...
void initTrial()
{
	...
	frontRodShown=false;
	drawGLScene();
	initProjectionScreen(C[trialNumber][5]);

	frame = 0;

	estdepth = C[trialNumber][4];
	estsd = C[trialNumber][6];
...
void initStreams()
{
	...
	parametersFile.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parameterspointPerceive_perceptual.txt");
	...
	string reportFile  = "C:/workspace/cncsvisioncmake/experimentsbrown/estimates/" + subjectName + "_traj.txt";
	subjParametersFile.open(reportFile.c_str());
	//subjParams.loadParameterFile(subjParametersFile);
    for(rows=0; rows<96; rows++)
      for(cols=0; cols<8; cols++)
        subjParametersFile >> C[rows][cols];


	// Principal streams file
    string responseFileName =  "pointPerceive_perceptual_"   + subjectName + ".txt";
	...
	responseFile << fixed << "NSubjName\tIOD\ttrialN\tAbsDepth\testdepth\testsd\tjudgment\tzedge\tAdjDepth\tjitter\ttrialDuration" << endl;
