...
string experiment_directory = "S:/Domini-ShapeLab/evan/spring16-gripAdaptHaptic135/";
// paramters file directory and name
string parametersFile_directory = experiment_directory + "spring16-gripAdaptHaptic135_parameters.txt";
...
void initStreams()
{
	...
	numReps = str2num<double>(parameters.find("Repetitions"))/numObj;
...
{   switch (key)
	...
	case 'c':
		if(fingersCalibrated==6 && training){
...
void initTrial()
{
	...
	perturb=0;
	showObject=true;
	if (trialNumber>(numReps-1) && trialNumber<=(2*numReps-1)){ // 45 - 90 is the range of perturbation trials
		perturbation=1;
