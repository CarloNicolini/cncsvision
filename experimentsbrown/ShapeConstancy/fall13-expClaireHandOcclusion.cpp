...
/********** EYES AND MARKERS **********************/
Vector3d eyeLeft, eyeRight, index, thumb, platformIndex(0,0,0), platformThumb(0,0,0);
...
void drawInfo()
{
	...
	if(interoculardistance>0.0)
	{
		...
		text.draw(stringify<double>(trial.getCurrent()["AbsDepth"]));
		text.draw(stringify<int>(trial.getCurrent()["RelDepthObj"]));
...
void drawFingers(double offsetZ)
{
	...
	if(indexObjectDistance >= trial.getCurrent()["RelDepthObj"] + offsetZ && !indexDisappeared)
	{
		glPushMatrix();
		glLoadIdentity();
...
void initStreams()
{
	...
	parametersFile.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parametersClaireHandOcclusion.txt");
	...
	responseFile << fixed << "subjName\tIOD\ttrialN\tRelDepthObj\tAbsDepth\toffset\ttime" << endl;
