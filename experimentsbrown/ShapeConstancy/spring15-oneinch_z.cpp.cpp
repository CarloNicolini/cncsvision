...
void initTrial();
void drawThreeRods(double deltax);
...
void drawThreeRods(double deltax)
{
	...
	// Front rod
	glPushMatrix();
	glTranslated(0, 0, deltax/2); // rods are half an inch to the ba
	stimDrawer[0].draw();
	glPopMatrix();

	// Rear Left rod
	glPushMatrix();
	glTranslated(-subjInch/2, 0, -deltax/2); // rods are half an inch to the ba
	stimDrawer[0].draw();
	glPopMatrix();

	// Rear right rod
	glPushMatrix();
	glTranslated(subjInch/2, 0, -deltax/2);
	stimDrawer[0].draw();
	glPopMatrix();
}
...
void initTrial()
{
	...
	string whichDist  = "d" + stringify<int>(abs(trial.getCurrent().first["AbsDepth"]));
	subjInch = str2num<double>(subjParams.find(whichDist.c_str()));
...
void initStreams()
{
	...
	string reportFile  = "C:/Users/visionlab/Dropbox/Shape Lab Shared/Amanda/OneInch/" + subjectName + "Means.txt";
	subjParametersFile.open(reportFile.c_str());
	subjParams.loadParameterFile(subjParametersFile);

		// Principal streams file
    string responseFileName =  "C:/Users/visionlab/Dropbox/Shape Lab Shared/Amanda/OneInch/AmandaOneInch-zResp_"  + subjectName + block + ".txt";
