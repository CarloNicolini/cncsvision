...
/*	// ###### draw fixed stimulus (reference) ######
	glTranslated(-50.0,0,trial.getCurrent()["AbsDepth"]);

	// Front Right rod on focal plane
	glPushMatrix();
	glTranslated( (trial.getCurrent()["RelDepthObj"]/2), -15, trial.getCurrent()["RelDepthObj"]/2);
	stimDrawer[4].draw();
	glPopMatrix();
...
void initStreams()
{
	...
	parametersFile.open("parametersstereopsisTest.txt");
