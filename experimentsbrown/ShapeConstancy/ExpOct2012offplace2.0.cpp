...
void beepOk(int tone)
{
	...
	case 2:
    // Remember to put double slash \\ to specify directories!!!
    PlaySound((LPCSTR) "C:\\cygwin\\home\\visionlab\\workspace\\cncsvision\\data\\beep\\beep-1a.wav", NULL, SND_FILENAME | SND_ASYNC);
			break;
	}
	return;
}
	...
	if( fingerCalibrationDone==3 )
	{
		if( !endTrial )
		{
			...
			} else if (timer.getElapsedTimeInMilliSec() > str2num<double>(parameters.find("PreMovementInterval")) && timer.getElapsedTimeInMilliSec() < str2num<int>(parameters.find("TrialDuration")))	   // stimulus disappears
			{
				...
				{
					if(index.z() < (-40.0) || (index.z() > (-40.0) && index.x() > 40.0) )
						drawTheVoid();
					else
						drawThreeRods();
				} else
