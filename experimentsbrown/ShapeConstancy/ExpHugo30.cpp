...
bool endTrial=false, fingersAtStart=true;
...
void drawStimulus()
{
	...
	/*	if(timer.getElapsedTimeInMilliSec() < 1500)
		{
			if(index.z() > (trial.getCurrent()["AbsDepth"]+2.0) )
				transportView();
			else 
				pointingView();
		} else
...
void initTrial()
{
	...
	if (trial.getCurrent()["ViewingCondition"]==1)			// all binocular
	{
	...
	if (trial.getCurrent()["ViewingCondition"]==2)	// all monocular
	{
	...
	if (trial.getCurrent()["ViewingCondition"]==3)	// binocular then dark
	{
		viewTransportLeft = 1.0;
		viewTransportRight = 1.0;
		viewPointingLeft = 0.0;
	...
	if (trial.getCurrent()["ViewingCondition"]==4)	// monocular then dark
	{
		viewTransportLeft = 0.0;
		viewTransportRight = 1.0;
		viewPointingLeft = 0.0;
	...
	Vector3d pos(0,-250,trial.getCurrent()["AbsDepth"]);
...
void initStreams()
{
	...
	parametersFile.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parametersExpHugo.txt", ios::in);
	...
	responseFile << fixed << "NTrial\tIOD\tEstSize\tEstDepth\tAbsDepth\tTime" << endl;
