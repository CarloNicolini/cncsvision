...
// thumb triggers disappeareance DONE
// randomization blocks
// which blocks 70, 30, 20 , nf, 10, 0 DONE
...
void drawInfo()
{
	...
	if(interoculardistance>0.0)
	{
		...
		text.draw("AbsDepth " + stringify<double>(trial.getCurrent()["AbsDepth"]));
		text.draw("RelDepthObj " + stringify<int>(trial.getCurrent()["RelDepthObj"]));
		text.draw("Offset " + stringify<int>(trial.getCurrent()["Offset"]));
	...
	if (fingerCalibrationDone==4 && isStimulusDrawn)
	{
		...
		if(!endOfTrial){
			if(timer.getElapsedTimeInMilliSec() < str2num<int>(parameters.find("trialDuration")))
			{
				...
				drawFingers(trial.getCurrent()["Offset"]);
...
void drawFingers(double offsetZ)
{
	indexObjectDistance = sqrt( pow(index.x(), 2) + pow( index.z() - trial.getCurrent()["AbsDepth"] , 2) );
	thumbObjectDistance = sqrt( pow(thumb.x(), 2) + pow( thumb.z() - trial.getCurrent()["AbsDepth"] , 2) );
	...
	if(thumbObjectDistance >= trial.getCurrent()["RelDepthObj"] + offsetZ && !thumbDisappeared)
	{
		glPushMatrix();
		glLoadIdentity();
...
void initTrial()
{
	...
	// trial file
	string trialFileName = "C:/Users/visionlab/Dropbox/Shape Lab Shared/Claire/" + parameters.find("SubjectName") + "/" + parameters.find("SubjectName") + "trial_" + stringify<int>(trialNumber) + ".txt";
	trialFile.open(trialFileName.c_str());
	trialFile << fixed << "subjName\ttrialN\ttime\tframeN\tindexXraw\tindexYraw\tindexZraw\tthumbXraw\tthumbYraw\tthumbZraw\teyeRXraw\teyeRYraw\teyeRZraw\teyeLXraw\teyeLYraw\teyeLZraw\tfingersOccluded\tindexDisappeared\tthumbDisappeared\t\tmirrorTheta\tglobalTime" << endl;
...
void initStreams()
{
	...
	parametersFile.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parametersClaireHandOcclusionTraining14.txt");
    ...
    string responseFileName =  "C:/Users/visionlab/Dropbox/Shape Lab Shared/Claire/" + parameters.find("SubjectName") + "/claireHandOcclusionTrain_"   + subjectName + ".txt";
	...
	responseFile << fixed << "subjName\tIOD\ttrialN\tRelDepthObj\tAbsDepth\t\ttime" << endl;
