...
static const int ROD=3;//evan-comment
int trialMode = ROD; 
int trialNumber = 0;
int frameN=0;
...
void drawStimulus()
{
	if (fingerCalibrationDone==3)
	{
		...
		if(isStimulusDrawn)
		{
			...
			//glTranslated(0.0,trial.getCurrent()["ObjHeight"]/2.0,trial.getCurrent()["AbsDepth"]); //evces
...
void drawTrial(double delz)
{
	...
	{
		case ROD:
			drawOneSolidRod(delz, sl, st);
			//gluCylinder(qobj, 2.5, 2.5, 40.0, 1, 1);
		break;
	}
}
...
void initTrial()
{
	...
	}else if(trial.getCurrent()["HapticFB"]==MISMATCH)
	{
		...
		{
			double posArmTopShifted= -15; //move so that top of mount is at -15, 2mm BELOW top of stim
			Vector3d pos(0,posArmTopShifted,trial.getCurrent()["AbsDepth"]+4);
			moveObjectAbsolute(pos, centercal, 6000);
		}else // otherwise we are displaying shorter stim
		{
			double posArmTopShifted= -13; //move so that top of mount is at -13, 2mm ABOVE top of stim
			Vector3d pos(0,posArmTopShifted,trial.getCurrent()["AbsDepth"]+4);
			moveObjectAbsolute(pos, centercal, 6000);
		}
	}

	string trialFileName = "C:/Users/visionlab/Google Drive/DATA/Evan-haptic/" + parameters.find("SubjectName") + "/" + parameters.find("SubjectName") + "trial_" + stringify<double>(trialNumber) + "_" + stringify<double>(attempt) + ".txt";
	trialFile.open(trialFileName.c_str());
	trialFile << fixed << "subjName\ttrialN\ttime\tframeN\tindexXYZraw\tthumbXYZraw\teyeRXYZraw\teyeLXYZraw\tfingersOccluded\tattempt" << endl;
...
void initStreams()
{
	...
	string dirName  = "C:/Users/visionlab/Google Drive/DATA/Evan-haptic/" + subjectName;
	mkdir(dirName.c_str());

	// Principal streams file
    string responseFileName =  "C:/Users/visionlab/Google Drive/DATA/Evan-haptic/" + parameters.find("SubjectName") + "/evan-haptic_" + subjectName + ".txt";
	...
	responseFile << fixed << "subjName\tIOD\ttrialN\tAbsDepth\tObjHeight\ttrialDuration\tHapticFB\trandCond\trandIncrement" << endl;
