...
StimulusDrawer stimDrawer[9], draw_ernst_banks[3];
CylinderPointsStimulus cylinder[9], ernst_banks[3];
...
// square measures
double edge = 0.0, dedge = 0, jitterX = 0.0, xedge = 0.0, zedge = 0.0, jitter = 0.0, theta=0, phi=M_PI*3/4, dz = 0.0, dx = 0.0, r = 0.0, fdx = 1.0, instant = 0.0, axm = 1.0, EBheight = 30.0;
Timer timer;
Timer globalTimer;
...
static const int ELLIPSOIDMODE=2;
static const int FOURRODS=3;
static const int CROSS=4;
static const int LETTER=5;
static const int THREERODS=6;
static const int ERNSTBANKS=7;
...
void drawInfo()
{
	...
	text.draw("#");
	text.draw("# timer: " +stringify<double>(timer.getElapsedTimeInMilliSec() + instant));
	text.draw("#");	
	text.draw("# instant: " +stringify<double>(instant));
	text.draw("#");
	/*
...
void drawErnstBanks(double height)
{
	...
	// print top sector
	glPushMatrix();
	glTranslated(0, height/4, -sepz/2);
...
{   switch (key)
    ...
    break;
	  ...
	  {
		  timer.reset();
		  instant = instant + timer.getElapsedTimeInMilliSec();
	  }
	  break;
	  case 'v':
	  {
...
void initGLVariables()
{
	...
	for (int i=0; i<5; i++)
	{
		...
		cylinder[i].setRadiusAndHeight(2,29); // raggio (mm) altezza (mm)
	...
	// ########### Inizializing single cylinder
		cylinder[4].setNpoints(150);
		cylinder[4].setRadiusAndHeight(10,100); // raggio (mm) altezza (mm)
		// Dispone i punti random sulla superficie cilindrica 
		cylinder[4].compute();
	...
	// ########### Inizializing single ellipsoid
		ellipsoid.setNpoints(150);
		ellipsoid.setAxesAndHeight(45,45,100); // diametro x (mm) diametro z (mm) altezza (mm)
		ellipsoid.compute();
		stimDrawer[5].setStimulus(&ellipsoid);
		stimDrawer[5].setSpheres(true);
		stimDrawer[5].initList(&ellipsoid, glRed);

		// build back surface
		cylinder[6].setNpoints(140);
		cylinder[6].setRadiusAndHeight(40,0,60);
		cylinder[6].compute();
		stimDrawer[6].setStimulus(&cylinder[6]);
		stimDrawer[6].setSpheres(true);
		stimDrawer[6].initList(&cylinder[6], glRed);

		// build front surface
		cylinder[7].setNpoints(100);
		cylinder[7].setRadiusAndHeight(25,0,25);
		cylinder[7].compute();
		stimDrawer[7].setStimulus(&cylinder[7]);
		stimDrawer[7].setSpheres(true);
		stimDrawer[7].initList(&cylinder[7], glRed);
...
void initStreams()
{
	...
	parametersFile.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parametersstereopsisTest.txt");
