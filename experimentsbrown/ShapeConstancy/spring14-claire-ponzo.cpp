...
void beepOk(int tone)
{
	...
	break;
	case 1:
    PlaySound((LPCSTR) "C:\\cygwin\\home\\visionlab\\workspace\\cncsvision\\data\\beep\\smaller.wav", NULL, SND_FILENAME | SND_ASYNC);
	break;
	case 2:
    PlaySound((LPCSTR) "C:\\cygwin\\home\\visionlab\\workspace\\cncsvision\\data\\beep\\larger.wav", NULL, SND_FILENAME | SND_ASYNC);
	break;
	}
	return;
}
...
void initTrial()
{
	...
	string trialFileName = "C:/Users/visionlab/Google Drive/DATA/Claire/Ponzo/" + parameters.find("SubjectName") + "/" + parameters.find("SubjectName") + "trial_" + stringify<double>(trialNumber) + ".txt";
	trialFile.open(trialFileName.c_str());
	trialFile << fixed << "subjName\ttrialN\ttime\tframeN\tindexXraw\tindexYraw\tindexZraw\tthumbXraw\tthumbYraw\tthumbZraw\teyeRXraw\teyeRYraw\teyeRZraw\teyeLXraw\teyeLYraw\teyeLZraw\tfingersOccluded" << endl;
...
void initStreams()
{
	...
	parametersFile.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parametersClairePonzo.txt");
	...
	string dirName  = "C:/Users/visionlab/Google Drive/DATA/Claire/Ponzo/" + subjectName;
	mkdir(dirName.c_str());

	// Principal streams file
    string responseFileName =  "C:/Users/visionlab/Google Drive/DATA/Claire/Ponzo/" + parameters.find("SubjectName") + "/ClairePonzo_" + subjectName + ".txt";
	...
	responseFile << fixed << "subjName\ttrialN\tstimPosn\ttrialDuration\tcondition" << endl; 
...
//glutFullScreen();
	...
	// move the screen
	initProjectionScreen(str2num<int>(parameters.find("AbsDepth")));//trial.getCurrent()["AbsDepth"]); evces 031414

	if(trialNumber==0){
			//Vector3d pos0(0,-35,0);
			//moveObjectAbsolute(pos0, centercal, 6500);
			Vector3d pos1(0,-350,-427);//-500);
