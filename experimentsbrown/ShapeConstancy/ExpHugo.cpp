...
static const Vector3d centercal(8.6,-10.0,-185.0);
...
double viewTransportLeft = 0.0, viewTransportRight = 0.0, viewPointingLeft = 0.0, viewPointingRight = 0.0 ;
...
void drawGLScene()
{
	...
	{
		if (isStimulusDrawn==true)
			{   
				if(fingersAtStart==true && endTrial==false) //fingers at start
				{ 
					glDrawBuffer(GL_BACK);
					// Draw left eye view
					glDrawBuffer(GL_BACK_LEFT);
			        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
					glClearColor(viewTransportLeft,0.0,0.0,1.0);
					cam.setEye(eyeLeft);
					drawInfo();
					...
					glDrawBuffer(GL_BACK_RIGHT);
					glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
					glClearColor(viewTransportRight,0.0,0.0,1.0);
				    cam.setEye(eyeRight);
					drawInfo();
					glutSwapBuffers();
					...
					if(stimTimer.getElapsedTimeInMilliSec() < str2num<int>(parameters.find("TestStimulusDuration"))) // while trial
					{
						if (index.z() > (factors["AbsDepth"] + 40))// && visualSwitch==false) 
							// finger during transport
						{
							glDrawBuffer(GL_BACK);
							// Draw left eye view
							glDrawBuffer(GL_BACK_LEFT);
...
void initTrial()
{
	...
	endTrial=false;
//	visualSwitch=false;
	if (factors["ViewingCondition"]==1)			// all binocular
	{
		viewTransportLeft = 1.0;
		viewTransportRight = 1.0;
		viewPointingLeft = 1.0;
		viewPointingRight = 1.0;
	}
	if (factors["ViewingCondition"]==2)	// all monocular
	{
		viewTransportLeft = 0.0;
		viewTransportRight = 1.0;
		viewPointingLeft = 0.0;
		viewPointingRight = 1.0;
	}
	if (factors["ViewingCondition"]==3)	// binocular then dark
	{
		viewTransportLeft = 1.0;
		viewTransportRight = 1.0;
		viewPointingLeft = 0.0;
		viewPointingRight = 0.0;
	} 
	
	if (factors["ViewingCondition"]==4)	// monocular then dark
	{
		viewTransportLeft = 0.0;
		viewTransportRight = 1.0;
		viewPointingLeft = 0.0;
	...
	initProjectionScreen(-400);
	drawGLScene();

	Vector3d pos(0,-250,factors["AbsDepth"]);
...
void positionObj()
{
	Vector3d objLocation(0,-251,-410);
...
void initStreams()
{
	...
	parametersFile.open("../parametersFiles/parametersExpHugo.txt");
	...
	string markersFileName  = "markersFileOffplace_" + subjectName + ".txt";
	string responseFileName =	"responseFileOffplace_"	+ subjectName + ".txt";
