...
StimulusDrawer stimDrawer[8];
CylinderPointsStimulus cylinder[8];
...
void drawInfo()
{
	...
	text.draw("#");
	text.draw("# orientation: " +stringify<double>(factors["Orientation"]*(str2num<double>(parameters.find("ThetaSign")))+theta));
	text.draw("#");
	text.draw("#######################");
...
void drawStimulus()
{
	...
	glTranslated(50.0,0,factors["AbsDepth"]);
	glRotated(factors["Orientation"]*(str2num<double>(parameters.find("ThetaSign")))+theta,0,1,0);

	// Front Right rod on focal plane
	glPushMatrix();
	glTranslated((dx/2)-15.0, -15, dz/2);
	stimDrawer[0].draw();
	glPopMatrix();
	...
	glLoadIdentity();	
	// ###### draw fixed stimulus (reference) ######
	glTranslated(-50.0,0,factors["AbsDepth"]);

	// Front Right rod on focal plane
	glPushMatrix();
	glTranslated( -15.0, -15, factors["RelDepthObj"]/2);
	stimDrawer[4].draw();
	glPopMatrix();
	...
	glTranslated( -30 , 15, - factors["RelDepthObj"]/2 );
	stimDrawer[6].draw();
	glPopMatrix();
...
void initGLVariables()
{
	// Ora inizializza gli 8 rods
	for (int i=0; i<8; i++)
	{
		cylinder[i].setNpoints(100);
		cylinder[i].setRadiusAndHeight(2,30); // raggio (mm) altezza (mm)
...
void initStreams()
{
	...
	responseFile << fixed << "NSubjName\tIOD\ttrialN\tAbsDepth\tRelDepthObj\txedge\tEstSize\tzedge\tEstDepth\tjitter\ttheta\torientation\ttrialDuration" << endl;
