...
void drawStimulus()
{
	...
	glPushMatrix();
	glLoadIdentity();
	glTranslated(0,0,factors["AbsDepth"]+(zedge + (jitter/2))/2);
	stimDrawer[0].draw();
	glPopMatrix();
	}
}
...
void initGLVariables()
{
	...
	for (int i=0; i<3; i++)
	{
		...
		ellipsoid[i].setAxesAndHeight(50,25,100); // asse maggiore (mm) asse minore (mm) altezza (mm)
//		ellipsoid[i].setRotation(M_PI/4.0);
...
void initStreams()
{
	...
	parametersFile.open("../parametersFiles/parametersShapeSizeConstancy3Depths.txt");
