...
double Sz = 0.0, Sx = 0.0, L = 63.0, Dx = 0.0, Dz = 0.0, Dy = 0.0;
...
/********** EYES AND MARKERS **********************/
Vector3d eyeLeft, eyeRight, index, thumb, platformIndex(0,0,0), platformThumb(0,0,0), noindex(-999,-999,-999), nothumb(-999,-999,-999), thumbPosition(0,0,0), indexPosition(0,0,0);
...
int actualOffset[6] = {0,0,0,0,0,0};
int cycle = 0;
...
//		initProjectionScreen(trial[cycle].getCurrent()["AbsDepth"]);
	...
	case 'F':
		...
		if ( (fingerCalibrationDone==1) && allVisibleFingers )
			...
			thumbPosition=Vector3d(Dx / Sx * L, Dz / Sz * L, Dy / Sz * L);
			indexPosition=Vector3d(thumbPosition.x() + Dx, thumbPosition.y() + Dy, thumbPosition.z() + Dz);
			indexCoords.init(indexPosition, markers.at(13).p, markers.at(14).p, markers.at(16).p );
			thumbCoords.init(thumbPosition, markers.at(15).p, markers.at(17).p, markers.at(18).p );
...
void idle()
{
	...
	Sx = sqrt(
			pow(markers[1].p.x() - markers[2].p.x(), 2.0) +
			pow(markers[1].p.y() - markers[2].p.y(), 2.0)
			);

	Sz = sqrt(
		pow(markers[1].p.z() - markers[2].p.z(), 2.0) +
		pow(markers[1].p.y() - markers[2].p.y(), 2.0)
		);

	Dx = markers[1].p.x() - markers[2].p.x();
	Dy = markers[1].p.y() - markers[2].p.y();
	Dz = markers[1].p.z() - markers[2].p.z();
