...
/********* EYES AND MARKERS ****************************/
Vector3d eyeLeft, eyeRight, index, thumb, platformFingers(0,0,0), platformIndex(0,0,0), platformThumb(0,0,0), singleMarker, upperPinMarker(0,0,0), lowerPinMarker(0,0,0);
...
void initStreams()
{
	...
	*/
	...
	string dirName  = "S:/Domini-ShapeLab/evan/spring15-hapticIllusion/" + subjectName;
	mkdir(dirName.c_str());

	// Principal streams file
	string responseFileName = "S:/Domini-ShapeLab/evan/spring15-hapticIllusion/" + subjectName + "/exp01_" + subjectName + ".txt";
	...
	responseFile << fixed << "subjName\ttrialN\ttrialDuration\thapticSize\tMSE\tobjID\tobjOriginX\tobjOriginY\tobjOriginZ" << endl;
	globalTimer.start();
}
	...
	if ( visibleInfo )
	{
		...
		if (finished) {
			...
			case 3:
				...
				break;
			case 4:
				text.draw("Touch the center of the left object with your index finger, then press F");
				break;
			case 5:
				text.draw("Touch the center of the right object with your index finger, then press F");
				break;
			case 6:
...
void initTrial()
{
	...
	*/
	...
	if (objId==0){
		targetOriginX = leftObjX;
		targetOriginY = leftObjY;
		targetOriginZ = leftObjZ;
	} else if (objId==1) {
		targetOriginX = rightObjX;
		targetOriginY = rightObjY;
		targetOriginZ = rightObjZ;
	}
	...
	string trialFileName = "S:/Domini-ShapeLab/evan/spring15-hapticIllusion/" 
...
void idle()
{
	...
	if (fingerCalibration==7 && !finished)
	{
		...
		if (!reachedObject && started) {
			...
			if ( (distanceGripCenterToObject<=17) && (grip_aperture<(hapticSize+10)) ){
...
int main(int argc, char*argv[])
{
	...
	switch (groupNum){
		...
		case 5:
			{
				for (int n=0; n<20; n++)
					posnSequence[n] = posnSequence5[n];
			}
			break;
		case 6:
			{
				for (int n=0; n<20; n++)
					posnSequence[n] = posnSequence6[n];
			}
			break;
	}
