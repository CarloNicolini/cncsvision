...
void drawStimulus()
{
	if( fingerCalibrationDone==3 )
	{
		if( !endTrial )
		{
			if(timer.getElapsedTimeInMilliSec() < factors["TestStimulusDuration"]) // draw the stimulus
			{
			// Central rod on focal plane
				...
				stimDrawer[2].draw();
				glPopMatrix();
			} else if (timer.getElapsedTimeInMilliSec() > factors["TestStimulusDuration"] && timer.getElapsedTimeInMilliSec() < str2num<int>(parameters.find("TrialDuration")))	   // stimulus disappears
			{
				drawTheVoid();
...
void idle()
{
	...
	if( timer.getElapsedTimeInMilliSec() == 1000 )
		beepOk(0);
...
void initStreams()
{
	...
	parametersFile.open("../parametersFiles/parameters-blindRtG.txt");
