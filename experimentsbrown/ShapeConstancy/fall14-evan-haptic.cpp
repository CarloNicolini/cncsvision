...
// Structure of Blocks!
// 0: H38-V38, 1: H42-V42, 2: H38-V42, 3: H42-V38, 4: H40-V40
// 5: H40-V38/42, 6: H38/42-V38/42
...
void initStreams()
{
	// In this experiment, we have multiple parameters files for different blocks
	...
	string dirName  = "S:/Domini-ShapeLab/evan/fall14-hapticBlocks/" + subjectName;
	//string dirName  = "C:/Users/visionlab/Google Drive/DATA/Evan-fall14/exp01/" + subjectName;
	mkdir(dirName.c_str());

	// Principal streams file
	string responseFileName = "S:/Domini-ShapeLab/evan/fall14-hapticBlocks/" + subjectName + "/exp01_" + subjectName + ".txt";
    //string responseFileName =  "C:/Users/visionlab/Google Drive/DATA/Evan-fall14/exp01/" + subjectName + "/exp01_" + subjectName + ".txt";
...
void drawStimulus()
{
	// don't draw stimulus until calibration is over
...
void drawTrial(int sl, int st)
{
	...
	glTranslated(objLeft+xAdjust, objTopp+yAdjust, objDpth);
...
void initTrial()
{
	...
	if (!burn_in){
		switch (block_order[block]) {
			...
			case 4: // H40-V40
				{ 
					visualSize = trial[block_order[block]].getCurrent()["VisualSize"];
					hapticSize = trial[block_order[block]].getCurrent()["HapticSize"];
				}
				break;
			case 5: //40, Haptic Constant, Vision +/- 2
				{
					hapticSize = trial[block_order[block]].getCurrent()["Size"];
					visualSize = hapticSize + trial[block_order[block]].getCurrent()["Mismatch"];
				}
				break;
			case 6: //40, Haptic-Vision Match, both with +/- 2 variation
				{ 
					hapticSize = trial[block_order[block]].getCurrent()["Size"];
					visualSize = trial[block_order[block]].getCurrent()["Size"];
				}
				break;
		}
	} else {
	...
	string trialFileName = "S:/Domini-ShapeLab/evan/fall14-hapticBlocks/" 
		+ parameters[0].find("SubjectName") + "/" 
		+ parameters[0].find("SubjectName") + "_bl"+ stringify<int>(block) +"_tr" + stringify<double>(trialNumber) + ".txt";
	
	//string trialFileName = "C:/Users/visionlab/Google Drive/DATA/Evan-fall14/exp01/" 
	//	+ parameters[0].find("SubjectName") + "/" 
	//	+ parameters[0].find("SubjectName") + "_bl"+ stringify<int>(block) +"_tr" + stringify<double>(trialNumber) + ".txt";
	trialFile.open(trialFileName.c_str());
	trialFile << fixed << "subjName\tblockN\ttrialN\ttime\tframeN\tindexXYZraw\tthumbXYZraw\tdistanceToObject\tfingersOccluded\treachedObject" << endl;
