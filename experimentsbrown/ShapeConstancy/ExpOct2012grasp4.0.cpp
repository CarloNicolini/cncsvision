...
bool endTrial=false, fingersAtStart=true, screenAtDistance=false;
...
void beepOk(int tone)
{
	...
	case 2:
    // Remember to put double slash \\ to specify directories!!!
    PlaySound((LPCSTR) "C:\\cygwin\\home\\visionlab\\workspace\\cncsvision\\data\\beep\\beep-1b.wav", NULL, SND_FILENAME | SND_ASYNC);
			break;
	}
	return;
}
...
void drawThreeRods()
{
	...
	glPushMatrix();
	glLoadIdentity();
	glTranslated(-40/2,0,factors["AbsDepth"]-factors["RelDepth"]/2);
	stimDrawer[1].draw();
	glPopMatrix();
...
void drawStimulus()
{
	if( fingerCalibrationDone==3 )
	{
		if( !endTrial )
		{
			if(timer.getElapsedTimeInMilliSec() < 1000 ) // draw the stimulus
			{
				isStimulusDrawn=false;
				drawTheVoid();
			}
			else if(timer.getElapsedTimeInMilliSec() < str2num<double>(parameters.find("PreMovementInterval") ) ) // draw the stimulus
			{
...
void initStreams()
{
	...
	parametersFile.open("../parametersFiles/parameters-blindRtG2.txt");
...
*/
	...
	string markersFileName  = "markersFileBlind_" + subjectName + ".txt";
	string responseFileName =	"responseFileBlind_"	+ subjectName + ".txt";
	...
	responseFile << fixed << "NTrial\tIOD\tAbsDepth\tTime\tCondition\tRelDepth" << endl;
