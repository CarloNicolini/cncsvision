...
void drawStimulus()
{
	...
	{
		firstD = trial.getCurrent().first["RelDepthObj"];
		secondD = trial.getCurrent().second->getCurrentStaircase()->getState();
	} else // staircase then fixed
	{
		firstD = trial.getCurrent().second->getCurrentStaircase()->getState();
		secondD = trial.getCurrent().first["RelDepthObj"];
	}
	...
	if(part == 1)
		{
			drawLetter(trial.getCurrent().first["Condition"]); // draw L or P
			if(moveToPartII)
				part += 1;
		} else if (part == 2)
		{
		...
		{		
			drawLetter(trial.getCurrent().first["Condition"] * -1); // draw L or P
			if(moveToPartVI)
				part += 1;
		} else if (part == 6)
		{
...
void idle()
{
	...
	if (fingerCalibrationDone==3 )
	{
		...
		if(trial.getCurrent().first["Condition"] == -1) // vision then pointing
		{
...
void initStreams()
{
	...
	parametersFile.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parametersstaircaseComparison.txt");
	...
	string markersFileName  = "C:/Users/visionlab/Dropbox/Shape Lab Shared/Carlo/spring13-pointingEquilateralTriangle/Staircase/Comparison/staircaseComparison_markersFile_" + subjectName + ".txt";
    string responseFileName =  "C:/Users/visionlab/Dropbox/Shape Lab Shared/Carlo/spring13-pointingEquilateralTriangle/Staircase/Comparison/staircaseComparison_"   + subjectName + ".txt";
	...
	responseFile << fixed << "subjName\tIOD\ttrialN\tRelDepthObj\tStairID\tStairState\tReversals\tAscending\tresponsekey\tresp\tjitter\ttime\tstepsDone\tobjdepth\tCondition\tsequence\ttimeToFHP" << endl;
