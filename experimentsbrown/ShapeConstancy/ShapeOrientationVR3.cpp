...
void drawStimulus()
{
	...
	glTranslated( (factors["RelDepthObj"]/2) , 15, - dz/2 );
	stimDrawer[2].draw();
	glPopMatrix();
...
/*	// ###### draw fixed stimulus (reference) ######
	...
	// Front Right rod on focal plane
	glPushMatrix();
	glTranslated( (factors["RelDepthObj"]/2), -15, factors["RelDepthObj"]/2);
	stimDrawer[4].draw();
	glPopMatrix();
