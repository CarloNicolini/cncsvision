...
int obj40 = 23, obj45 = 10, obj50 = 9, obj55 = 24;
int objMarkers[] = {obj40, obj45, obj50};//, obj55};
...
double targetOriginsZ[3] = {-400.0, -400.0, -400.0};
...
// stepper variable
double objStepperPosns[3] = {0, 80, 160};
__int64 pos;
...
string parametersFile_directory = experiment_directory + "spring16-karl-muller_parameters.txt";
// summary file headers
string trialFile_headers = "subjName\ttrialN\tattempt\tobjHeight\tfinsAngle\tframeN\tfingersOccluded\ttime\tdistanceToObject\treachedObject\tindexXraw\tindexYraw\tindexZraw\tthumbXraw\tthumbYraw\tthumbZraw\tGA\ttargetX\ttargetY\ttargetZ";
/*************************** FUNCTIONS ***********************************/
	...
	case 'p':
		if(fingerCalibrationDone==6&&training){
			if(practice){
				...
				Vector3d moveTo(0.0, -999.9, displayDepth-platterRadius-clearance);
				moveObjectAbsolute(moveTo,centercal,3500);
			}
		}
		break;
...
double radian2degree(double myAngle);
double degree2radian(double myAngle);
vector<double> rotateRect(vector <double> midPoint, vector <double> rectSize, double rectAngle); //8 elements
vector<double> calcML(vector <double> midPoint, vector <double> rectSize, vector <double> finSize, double finAngle); //8*5=40 elements

double pi = 3.14159265;


double radian2degree(double myAngle) {
    //calculates degrees, given a radian
    
    double newAngle = myAngle * ((1/(2*pi))*360);
    return(newAngle);
}
double degree2radian(double myAngle) {
    //calculates degrees, given a radian
...
vector <double> rotateRect(vector <double> midPoint, vector <double> rectSize, double rectAngle) {
    //calculates the new vertices of a rotared rectangle, given
    //  midPoint  = the x and y coordinates of the initial rectangle,
    //  rectSize  = the x and y dimensions of the rectangle,
    //  rectAngle = the angle by which the object is to be rotated.
    //Output format: {TLX, TRX, BRX, BLX, TLY, TRY, BRY, BLY}
    
    //calculate new coordinates
    double TLX = midPoint[0] - cos(degree2radian(rectAngle))*rectSize[0]/2 -
        sin(degree2radian(rectAngle))*rectSize[1]/2;
    double TRX = midPoint[0] + cos(degree2radian(rectAngle))*rectSize[0]/2 -
        sin(degree2radian(rectAngle))*rectSize[1]/2;
    double BRX = midPoint[0] + cos(degree2radian(rectAngle))*rectSize[0]/2 +
        sin(degree2radian(rectAngle))*rectSize[1]/2;
    double BLX = midPoint[0] - cos(degree2radian(rectAngle))*rectSize[0]/2 +
        sin(degree2radian(rectAngle))*rectSize[1]/2;
    
    double TLY = midPoint[1] + cos(degree2radian(rectAngle))*rectSize[1]/2 -
        sin(degree2radian(rectAngle))*rectSize[0]/2;
    double TRY = midPoint[1] + cos(degree2radian(rectAngle))*rectSize[1]/2 +
        sin(degree2radian(rectAngle))*rectSize[0]/2;
    double BRY = midPoint[1] - cos(degree2radian(rectAngle))*rectSize[1]/2 +
        sin(degree2radian(rectAngle))*rectSize[0]/2;
    double BLY = midPoint[1] - cos(degree2radian(rectAngle))*rectSize[1]/2 -
        sin(degree2radian(rectAngle))*rectSize[0]/2;
    
    //store new coordinates
    vector <double> coordArr = {TLX, TRX, BRX, BLX, TLY, TRY, BRY, BLY};
    
    return(coordArr);
}


vector <double> calcML(vector <double> midPoint, vector <double> rectSize, vector <double> finSize, double finAngle) {
    //calculates the vertices defining 5 rectangles of a mueller-lyer figure, given
    //  midPoint = x and y of the midpoint,
    //  rectsize = x and y size of the main part,
    //  finSize  = x and y size of the fins,
    //  finAngle = the rotation angle of the fins.
    //Output: megaVector, 5*8 vector, format: {TL, TR, BR, BL, centre} * {TLX, TRX, BRX, BLX, TLY, TRY, BRY, BLY}
    
    //calculate vertices of main rect.
    vector <double> mainRect = rotateRect(midPoint, rectSize, 0);
    vector <double> TL = {mainRect[0],mainRect[4]};
    vector <double> TR = {mainRect[1],mainRect[5]};
    vector <double> BR = {mainRect[2],mainRect[6]};
    vector <double> BL = {mainRect[3],mainRect[7]};
    
    //calculate correction for fin positions
    double xCorr = sin(degree2radian(finAngle))*finSize[1]/2 -
        fabs(cos(degree2radian(finAngle))*finSize[0]/2);
    double yCorr = cos(degree2radian(finAngle))*finSize[1]/2 -
        fabs(sin(degree2radian(finAngle))*finSize[0]/2);
    
    //calculate centres of fins
    vector <double> TLmid = {TL[0]-xCorr, TL[1]+yCorr};
    vector <double> TRmid = {TR[0]+xCorr, TR[1]+yCorr};
    vector <double> BRmid = {BR[0]+xCorr, BR[1]-yCorr};
    vector <double> BLmid = {BL[0]-xCorr, BL[1]-yCorr};
    
    //create vectors to concatenate
    vector <double> TLcoords = rotateRect(TLmid, finSize, finAngle);
    vector <double> TRcoords = rotateRect(TRmid, finSize, 360-finAngle);
    vector <double> BRcoords = rotateRect(BRmid, finSize, finAngle+180);
    vector <double> BLcoords = rotateRect(BLmid, finSize, 180-finAngle);
    vector <double> CEcoords = rotateRect(midPoint, rectSize, 0);
    
    
    //create final megaVector to be returned
    vector <double> megaVector;
    megaVector.insert(megaVector.end(), TLcoords.begin(), TLcoords.end());
    megaVector.insert(megaVector.end(), TRcoords.begin(), TRcoords.end());
    megaVector.insert(megaVector.end(), BRcoords.begin(), BRcoords.end());
    megaVector.insert(megaVector.end(), BLcoords.begin(), BLcoords.end());
    megaVector.insert(megaVector.end(), CEcoords.begin(), CEcoords.end());
...
void drawCylinder()
{
    ...
    //calculate mueller-lyer
    vector <double> MLcoords = calcML({250,250}, {30,120}, {8,40}, 45);
    
    //draw quads!
    glBegin(GL_QUADS);
    for(int i = 0; i < 5; i++) {
        int myIndex = (i)*8;
        glVertex2f(MLcoords[myIndex], MLcoords[myIndex+4]);
        glVertex2f(MLcoords[myIndex+1], MLcoords[myIndex+5]);
        glVertex2f(MLcoords[myIndex+2], MLcoords[myIndex+6]);
        glVertex2f(MLcoords[myIndex+3], MLcoords[myIndex+7]);
    }
...
void initTrial()
{
	...
	perturb=0;
	showObject=true;
	if (trialNumber>(45-1) && trialNumber<=(90-1)){ // 45 - 90 is the range of perturbation trials
		perturbation=1;
		perturb=-2.5;
	}
	...
	cylHeight = objSizes[objId-1]; // physical target size
	
	if(perturbation)
		cylHeight=cylHeight+2*perturb; // add visual perturbation if needed

	double target_angle = objStepperPosns[objId-1];
...
void advanceTrial()
{
	...
	if ( !reachedObject || not_enough_frames || bad_start ) {
	 ...
	 if (training){
		...
		if(trainingMargins.size()>6)
			trainingMargins.pop(); // make sure we only remember the last 6
...
void online_trial()
{
	...
	if (fingerCalibrationDone==6 && !finished)
	{
		...
		nearby = distanceGripCenterToObject<=15;
