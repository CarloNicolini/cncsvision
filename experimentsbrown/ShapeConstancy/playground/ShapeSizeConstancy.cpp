...
static const Vector3d centercal(-21.8,-327.2,-312.8);
Screen screen;
...
void drawInfo()
{
	...
	text.draw(" ");
	text.draw("size = " +stringify<double>((factors["RelSize"]+xedge + jitter)));
	text.draw("xedge = " +stringify<double>(xedge));
	text.draw(" ");
	text.draw("depth = " +stringify<double>(factors["RelDepth"]+zedge + (jitter/2)));
...
void drawStimulus()
{
	...
	glPushMatrix();
	glLoadIdentity();
	glTranslated(0,0,factors["AbsDepth"]+(factors["RelDepth"]+zedge + (jitter/2))/2);
	stimDrawer[0].draw();
	glPopMatrix();
	...
	glPushMatrix();
	glLoadIdentity();
	glTranslated(-(factors["RelSize"]+xedge+jitter)/2,0,factors["AbsDepth"]-(factors["RelDepth"]+zedge+(jitter/2))/2);
	stimDrawer[1].draw();
	glPopMatrix();
...
void mouseMotionFunc(int x, int y)
{
	...
	} else // if mouse goes downwards
	{
		 if(leftButtonDown == 1) // if left button is down
		{
			if((factors["RelDepth"]+zedge + (jitter/2))>0)
			{
				xedge += 0.0;
				zedge -= 0.25;
			} else {
...
void initVariables()
{
	...
	parametersFile.open("../parametersFiles/parametersShapeSizeConstancy.txt");
...
void initGLVariables()
{
	...
	for (int i=0; i<3; i++)
	{
		...
		cylinder[i].setRadiusAndHeight(1,50); // raggio (mm) altezza (mm)
...
void initStreams()
{
	...
	responseFile << fixed << "NTrial\tRelSize\txedge\tRelDepth\tzedge\tjitter\tAbsDepth\tTime" << endl;
}
