	...
	if( fingerCalibrationDone==3 && index.z() < (startingPoint - 30.0) )
	{
		...
		if(stimTimer.getElapsedTimeInMilliSec() < str2num<int>(parameters.find("TestStimulusDuration")))
		{
			...
			// Central rod on focal plane
				glPushMatrix();
				glLoadIdentity();
				glTranslated(0,0,factors["AbsDepth"]+(factors["RelDepth"]+zedge)/2);
				stimDrawer[0].draw();
				glPopMatrix();
				...
				glPushMatrix();
				glLoadIdentity();
				glTranslated(-(factors["RelSize"]+xedge)/2,0,factors["AbsDepth"]-(factors["RelDepth"]+zedge)/2);
				stimDrawer[1].draw();
				glPopMatrix();
