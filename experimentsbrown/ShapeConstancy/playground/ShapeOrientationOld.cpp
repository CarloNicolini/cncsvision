...
EllipsoidPointsStimulus ellipsoid[3];
...
double angle = 45, lAx = 50, sAx = 35, Sfactor=1;
Timer timer;
Timer globalTimer;

// XXX NICO
Eigen::Matrix4i mouseCoordsMatrix = Eigen::Matrix4i::Zero();
...
void drawStimulus()
{
	...
	glPushMatrix();
	glLoadIdentity();
	glTranslated(0,0, str2num<double>(parameters.find("AbsDepth")) );
	glScaled(Sfactor,1,Sfactor);
	glRotated( factors["Orientation"], 0,1,0);
	stimDrawer[0].draw();
	glPopMatrix();
	}
}
...
void initTrial()
{
	...
	drawGLScene();
	initProjectionScreen( str2num<double>(parameters.find("AbsDepth")) );
	//Vector3d pos(0, -44, (str2num<double>(parameters.find("StartPosObj"))));
	...
	if(trialNumber%15==0)
		...
		beepOk(0);
	lAx = unifRand(str2num<double>(parameters.find("lAxLowBound")),
		str2num<double>(parameters.find("lAxUpBound")));
	timer.start();
...
{
    // XXX NICO
    mouseCoordsMatrix(0,0)=mouseCoordsMatrix(1,0);    // valori vecchi del mouse
    mouseCoordsMatrix(0,1)=mouseCoordsMatrix(1,1);
    
    mouseCoordsMatrix(1,0)=x;  // valori correnti del mouse
    mouseCoordsMatrix(1,1)=y;
    
    // il delta è l'attuale meno il precedente
    int deltaX = mouseCoordsMatrix(0,0) - mouseCoordsMatrix(1,0);
    int deltaY = mouseCoordsMatrix(0,1) - mouseCoordsMatrix(1,1);
    cerr << deltaX << " " << deltaY << endl;
    // quindi quando il delta è positivo allora il mouse va in su (o in giù non ricordo la convenzione dei segni)
    
    // END XXX NICO
	 if(leftButtonDown == 1) // if left button is down
	 {
		 if(deltaY>0) //if mouse goes upwards
			{
				//xedge += 0.0;
				zedge += 0.25;
	...
	} else if(rightButtonDown == 1) // if right button is down
	{
		 if(deltaX>0) //if mouse goes rightwards
			{
				xedge += 0.25;
				zedge += 0.0;
			}
			else // if mouse goes leftwards
			{
				if((jitter+xedge)>0)
				{
				xedge -= 0.25;
...
void drawEllipsoidalCylinder()
{
	for (int i=0; i<3; i++)
	{
		ellipsoid[i].setNpoints(400);
		ellipsoid[i].setAxesAndHeight(lAx,sAx,100); // asse maggiore (mm) asse minore (mm) altezza (mm)
...
void initStreams()
{
	...
	parametersFile.open("../parametersFiles/parametersShapeOrientation.txt");
	parameters.loadParameterFile(parametersFile);
	// Subject name
    string subjectName = parameters.find("SubjectName");

	// Principal streams file
    string responseFileName =  "SOrespFile_"   + subjectName + ".txt";
	...
	responseFile << fixed << "subjName\tIOD\ttrialN\tOrientation\tlongAxis\ttrialDuration" << endl;
