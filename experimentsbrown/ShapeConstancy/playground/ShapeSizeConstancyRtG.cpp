...
void drawStimulus()
{
	if( fingerCalibrationDone==3 && index.z() < (startingPoint - 30.0) )
	{
		...
		if(stimTimer.getElapsedTimeInMilliSec() < str2num<int>(parameters.find("TestStimulusDuration")))
		{
			...
			// Central rod on focal plane
				glPushMatrix();
				glLoadIdentity();
				glTranslated(0,0,factors["AbsDepth"]+(jitter/2));
				stimDrawer[0].draw();
				glPopMatrix();
				...
				glPushMatrix();
				glLoadIdentity();
				glTranslated(-(str2num<int>(subjParams.find("PerceivedSize")))/2,0,factors["AbsDepth"]-(jitter/2));
				stimDrawer[1].draw();
				glPopMatrix();
				...
				glPushMatrix();
				glLoadIdentity();
				glTranslated((str2num<int>(subjParams.find("PerceivedSize"))+xedge)/2,0,factors["AbsDepth"]-(jitter/2));
				stimDrawer[2].draw();
				glPopMatrix();
			} else  {
...
void advanceTrial()
{
		...
		responseFile << fixed << trialNumber << "\t" <<
					str2num<int>(subjParams.find("PerceivedSize")) << "\t" << 
					interoculardistance << "\t" <<
					jitter << "\t" <<
					factors["AbsDepth"] << "\t" <<
					timeElapsed << "\t" <<
...
void initStreams()
{
	...
	subjParametersFile.open("../parametersFiles/subjEst.txt");
	...
	responseFile << fixed << "NTrial\tRelSize\tIOD\tRelDepth\tAbsDepth\tTime\tStimTime" << endl;
}
