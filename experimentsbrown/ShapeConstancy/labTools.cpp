...
double mirrorAlignment=0.0, screenAlignmentY=0.0, screenAlignmentZ=0.0, objectAlignment=0.0;
Screen screen;
...
double timeForGrasping = 0.0;
bool areFingersDrawn=false;
...
#ifndef SIMULATION
string experiment_directory = "S:/Domini-ShapeLab/carlo/2014-2015/SU1-threeRods/";
#else
string experiment_directory = "/media/shapelab/Domini-ShapeLab/carlo/2014-2015/SU1-threeRods/";
#endif
...
// markers file name
string markersFile_name = "summer15-threeRodsDiag_";
...
// response file name
string responseFile_name = "summer15-threeRodsDiag_";
...
void drawInfo()
{
	if ( visibleInfo )
	{
		...
		text.draw("# Object Orientation = " +stringify<double>(objectAlignment));
		text.draw(" ");
		...
		glColor3fv(glWhite);
		text.draw("Marker "+ stringify<int>(mirror1)+stringify< Eigen::Matrix<double,1,3> > (markers.at(mirror1).p.transpose())+ " [mm]" );
		text.draw("Marker "+ stringify<int>(mirror2)+stringify< Eigen::Matrix<double,1,3> > (markers.at(mirror2).p.transpose())+ " [mm]" );
		text.draw("Marker "+ stringify<int>(5)+stringify< Eigen::Matrix<double,1,3> > (markers.at(5).p.transpose())+ " [mm]" );
		...
		if ( isVisible(markers.at(screen1).p) && isVisible(markers.at(screen2).p) && isVisible(markers.at(screen3).p) )
			glColor3fv(glGreen);
		else
			glColor3fv(glRed);
		
		text.draw("Marker "+ stringify<int>(screen1)+stringify< Eigen::Matrix<double,1,3> > (markers.at(screen1).p.transpose())+ " [mm]" );
		text.draw("Marker "+ stringify<int>(screen2)+stringify< Eigen::Matrix<double,1,3> > (markers.at(screen2).p.transpose())+ " [mm]" );
		text.draw("Marker "+ stringify<int>(screen3)+stringify< Eigen::Matrix<double,1,3> > (markers.at(screen3).p.transpose())+ " [mm]" );
		...
		text.draw("Other markers" );

		text.draw("Marker "+ stringify<int>(9)+stringify< Eigen::Matrix<double,1,3> > (markers.at(9).p.transpose())+ " [mm]" );
		text.draw("Marker "+ stringify<int>(10)+stringify< Eigen::Matrix<double,1,3> > (markers.at(10).p.transpose())+ " [mm]" );
...
void initTrial()
{
	...
	if(experiment)
	{
		...
		double objtheta = atan((objwidth/2)/objdepth);
...
void idle()
{
	...
	// calibration object orientation
	objectAlignment = asin(
			abs((markers.at(calibrationobj1).p.z()-markers.at(calibrationobj3).p.z()))/
			sqrt(
			pow(markers.at(calibrationobj1).p.x()-markers.at(calibrationobj3).p.x(), 2) +
			pow(markers.at(calibrationobj1).p.z()-markers.at(calibrationobj3).p.z(), 2)
			)
			)*180/M_PI;
