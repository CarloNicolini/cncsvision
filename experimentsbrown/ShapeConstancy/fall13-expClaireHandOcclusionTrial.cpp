...
/********** EYES AND MARKERS **********************/
Vector3d eyeLeft, eyeRight, index, thumb, platformIndex(0,0,0), platformThumb(0,0,0), upperPinMarker(0,0,0), lowerPinMarker(0,0,0);
...
// thumb triggers disappeareance
// randomization blocks
// which blocks 70, 30, 20 , nf, 10, 0
...
int actualOffset[6] = {70,30,20,15,10,5};

ParametersLoader parameters, parametersOpenLoop, parametersClosedLoop;
BalanceFactor<double> trial[sizeof(actualOffset)/sizeof(int)+2];
...
void drawInfo()
{
	...
	if(interoculardistance>0.0)
	{
		...
		text.draw(stringify<double>(trial[cycle].getCurrent()["AbsDepth"]));
		text.draw(stringify<int>(trial[cycle].getCurrent()["RelDepthObj"]));
	...
	if (fingerCalibrationDone==4 && isStimulusDrawn)
	{
		if(!endOfTrial){
			if(timer.getElapsedTimeInMilliSec() < str2num<int>(parameters.find("trialDuration")))
			{
				...
				drawFingers(trial[cycle].getCurrent()["Offset"]);
...
void drawFingers(double offsetZ)
{
	...
	if(indexObjectDistance >= trial[cycle].getCurrent()["RelDepthObj"] + offsetZ && !indexDisappeared)
	{
		glPushMatrix();
		glLoadIdentity();
	...
	if(thumbObjectDistance >= trial[cycle].getCurrent()["RelDepthObj"] + offsetZ && !thumbDisappeared)
	{
		glPushMatrix();
		glLoadIdentity();
...
void initTrial()
{
	...
	string trialFileName = "C:/Users/visionlab/Dropbox/Shape Lab Shared/Claire/" + parameters.find("SubjectName") + "/" + parameters.find("SubjectName") + "_" + stringify<int>(cycle) + "trial_" + stringify<int>(trialNumber) + ".txt";
	trialFile.open(trialFileName.c_str());
	trialFile << fixed << "subjName\ttrialN\ttime\tframeN\tindexXraw\tindexYraw\tindexZraw\tthumbXraw\tthumbYraw\tthumbZraw\teyeRXraw\teyeRYraw\teyeRZraw\teyeLXraw\teyeLYraw\teyeLZraw\tfingersOccluded\tindexDisappeared\tthumbDisappeared\tmirrorTheta" << endl;
...
void advanceTrial()
{
	...
	if(abs(mirrorAlignment - 45.0) < 0.25)
	{
		...
		} else 
		{
			cycle++;
			trialNumber = 0;
			if(cycle < sizeof(actualOffset)/sizeof(int)+2)
			{
				trial[cycle].next();
				initTrial();
			} else
...
void idle()
{
	...
	if (fingerCalibrationDone==4 )
	{
		...
		if(handAtStart && !fingersDisappeared)
		{
			//if(timer.getElapsedTimeInMilliSec() < str2num<int>(parameters.find("trialDuration")))
			//{
				timer.start();
			...
			//{
				...
				//advanceTrial();
			//}
		} else if(handAtStart && fingersDisappeared)
			endOfTrial = true;
...
void initVariables()
{
	...
	trial[0].init(parametersOpenLoop);
	trial[0].addFactor("Offset", vlist_of<double>(OLoffset));

	for(i=1; i<sizeof(actualOffset)/sizeof(int)+1; i++)
	{
		trial[i].init(parameters);
		trial[i].addFactor("Offset", vlist_of<double>(actualOffset[i-1]));
		trial[i].print();
	}

	trial[i].init(parametersClosedLoop);
	trial[i].addFactor("Offset", vlist_of<double>(CLoffset));
}
...
void initStreams()
{
	...
	parametersFile.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parametersClaireHandOcclusionTrial.txt");
	parameters.loadParameterFile(parametersFile);

	ifstream parametersFileOpenLoop;
	parametersFileOpenLoop.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parametersClaireHandOcclusionTrialOpenLoop.txt");
	parametersOpenLoop.loadParameterFile(parametersFileOpenLoop);

	ifstream parametersFileClosedLoop;
	parametersFileClosedLoop.open("C:/workspace/cncsvisioncmake/experimentsbrown/parameters/parametersClaireHandOcclusionTrialClosedLoop.txt");
	parametersClosedLoop.loadParameterFile(parametersFileClosedLoop);

	// Subject name
    string subjectName = parameters.find("SubjectName");
	
	string dirName  = "C:/Users/visionlab/Dropbox/Shape Lab Shared/Claire/" + subjectName;
	mkdir(dirName.c_str());

		// Principal streams file
    string responseFileName =  "C:/Users/visionlab/Dropbox/Shape Lab Shared/Claire/" + parameters.find("SubjectName") + "/claireHandOcclusion_"   + subjectName + ".txt";
	...
	responseFile << fixed << "subjName\tIOD\ttrialN\ttempTrial\tRelDepthObj\tAbsDepth\toffset\ttime" << endl;
