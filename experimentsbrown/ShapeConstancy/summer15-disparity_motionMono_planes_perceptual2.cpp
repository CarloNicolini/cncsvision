...
// stimuli
StimulusDrawer boxLeftDrawer, boxRightDrawer, stereocheckDrawer[4], rodMiddleDrawer, boxOblique_pedestalDrawer, boxOblique_stimulusDrawer;
BoxNoiseStimulus boxLeft, boxRight, stereocheck[4], rodMiddle, boxOblique_stimulus, boxOblique_pedestal;
...
void initTrial();
void draw_two_planes_stereo();
void draw_two_planes_motion_mono();
...
#ifndef SIMULATION
string experiment_directory = "S:/Domini-ShapeLab/carlo/2014-2015/disparity_motionMono_planes_perceptual/";
//string experiment_directory = "C:/Users/visionlab/Desktop/";
#else
string experiment_directory = "/media/shapelab/Domini-ShapeLab/carlo/2014-2015/disparity_motionMono_planes_perceptual/";
#endif

// paramters file directory and name
string parametersFile_directory = experiment_directory + "parameters_disparity_motionMono_planes_perceptual.txt";

// response file name
string responseFile_name = "summer15-disparity_motionMono_planes_perceptual_";
...
/*** STIMULI ***/
void draw_two_planes_stereo(double deltaz)
{
	glLoadIdentity();
	...
	} else if ( isStimulusDrawn )
	{
		if (timer.getElapsedTimeInMilliSec() < trial_duration)
		{
			...
			if (first_stimulus == "pedestal")
			{
				...
				{
					IOD = interoculardistance;
					draw_two_planes_stereo(depth_pedestal);
				}
				else
				{
					IOD = 0.0;
					draw_two_planes_motion_mono(depth_pedestal);
				}
			...
			} else
			{
				...
				{
					IOD = interoculardistance;
					draw_two_planes_stereo(depth_stimulus);
				}
				else
				{
					IOD = 0.0;
					draw_two_planes_motion_mono(depth_stimulus);		
				}
			}
		} 
...
{
	// BOX left
	boxLeft.setDimensions(20.0, 50.0, 0.1);
	...
	// BOX right
	boxRight.setDimensions(20.0, 50.0, 0.1);;
...
//		bool decision = ((depth_stimulus + JND) > (depth_pedestal + PSE) && first_stimulus == "stimulus") || (depth_stimulus + JND) > (depth_pedestal + PSE) && second_stimulus == "stimulus";
	...
	oblPlane_pedestal_length = sqrt( pow(depth_pedestal, 2) + pow(50.0, 2) );
	oblPlane_stimulus_length = sqrt( pow(depth_stimulus, 2) + pow(50.0, 2) );
	build_oblique_plane(oblPlane_pedestal_length, oblPlane_stimulus_length);

	// calculate the angle of the oblique plane
	oblTheta_pedestal = atan( depth_pedestal/50.0 ) * 180/M_PI;
	oblTheta_stimulus = atan( depth_stimulus/50.0 ) * 180/M_PI;
