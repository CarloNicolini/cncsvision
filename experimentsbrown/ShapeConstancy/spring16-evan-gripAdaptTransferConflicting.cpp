...
string experiment_directory = "S:/Domini-ShapeLab/evan/spring16-gripAdaptTransferConflicting/";
// paramters file directory and name
string parametersFile_directory = experiment_directory + "spring16-gripAdaptTransferConflicting_parameters.txt";
// summary file headers
string trialFile_headers = "subjName\tincreaseLocation\ttrialN\tattempt\tobjId\tobjHeight\tyPosn\tperturbation\tframeN\tfingersOccluded\ttime\tdistanceToObject\treachedObject\tindexXraw\tindexYraw\tindexZraw\tthumbXraw\tthumbYraw\tthumbZraw\tGA\ttargetX\ttargetY\ttargetZ\tstartDistance\tpercentOccluded\tbadTrial";
/*************************** FUNCTIONS ***********************************/
...
void drawStimulus()
{
	...
	double x1=markers.at(calibrationobj1).p.x();
	double y1=markers.at(calibrationobj1).p.y();
	double z1=markers.at(calibrationobj1).p.z();
	double x2=markers.at(calibrationobj2).p.x();
	double y2=markers.at(calibrationobj2).p.y();
	double z2=markers.at(calibrationobj2).p.z();
	double x3=markers.at(calibrationobj3).p.x();
	double y3=markers.at(calibrationobj3).p.y();
	double z3=markers.at(calibrationobj3).p.z();
	glVertex3d(x1,y1,z1);
	glVertex3d(x2,y2,z2);
	glVertex3d(x3,y3,z3);
	glEnd();*/
...
void initTrial()
{
	...
	if (trialNumber>=baseTrials && trialNumber<(baseTrials+adaptTrials)){ // 30 <= tr < 120
		perturbation=1;
		perturb = trial.getCurrent()["perturbation"];
	}
	...
	if(!practice){
		// if presenting on top, set yAdjust non-zero
		if( (trial.getCurrent()["perturbation"]>0 && TOP>0) || (trial.getCurrent()["perturbation"]<0 && TOP<0)){ 
			yAdjust = 135; //120
		}
		// move the arm
