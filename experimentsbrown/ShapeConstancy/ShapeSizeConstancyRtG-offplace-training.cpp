...
int yMouseCoord = 0;
bool visibleStim = false;
// square measures
double edge = 0.0, dedge = 0, jitterX = 0.0, xedge = 0.0, zedge = 0.0, jitter = 0.0, estsize = 0.0, estdepth = 0.0, objPos = 0.0;
...
void drawInfo()
{
	...
	text.draw("#");
	text.draw("# FGA = " +stringify< Eigen::Matrix<double,1,3> >(index.transpose() - thumb.transpose()));
	text.draw("#");
	text.draw("#######################");
...
void drawStimulus()
{
	if( fingerCalibrationDone==3 && index.z() < (startingPoint - 40.0) && visibleStim)
	{
...
void initStreams()
{
	...
	string markersFileName  = "markersFileOffplace-training_" + subjectName + ".txt";
	string responseFileName =	"responseFileOffplace-training_"	+ subjectName + ".txt";
