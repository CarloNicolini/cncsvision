...
void homeEverything(int objectspeed, int screenspeed)
{
    ...
    string buffer("F,C,S"+stringify<int>(BROWN_OBJECT_Z_AXIS)+"M"+stringify<int>(objectspeed)+
                  ...
                  ",IA"+stringify<int>(BROWN_OBJECT_Z_AXIS)+"M-0"+
                  ",IA"+stringify<int>(BROWN_OBJECT_Y_AXIS)+"M-0"+
                  ",IA"+stringify<int>(BROWN_SCREEN_X_AXIS)+"M-0"+
                  ",R");
    ...
    if ( out=='^' )
        ...
        cerr << "Error in homeEverything()" << endl;
#endif
}
...
void moveObjectAbsolute(const Eigen::Vector3d &point, const Eigen::Vector3d &calibration, int objectspeed)
{
    ...
    string buffer("F,C,S"+stringify<int>(BROWN_OBJECT_Z_AXIS)+"M"+stringify<int>(objectspeed)+
                  ",S"+stringify<int>(BROWN_OBJECT_Y_AXIS)+"M"+stringify<int>(objectspeed)+
                  ",IA"+stringify<int>(BROWN_OBJECT_Z_AXIS)+"M"+stringify<int>(nstepsZ)+
                  ",IA"+stringify<int>(BROWN_OBJECT_Y_AXIS)+"M"+stringify<int>(nstepsY)+
                  ",R");
    cerr << "Sending:" << buffer << endl;
    serial << buffer << endl;
    char out=0;
    ...
    if ( out=='^' )
        ...
        cerr << "Error in moveObjectAbsolute()" << endl;
#endif
}
...
void moveScreenAbsolute(double focalDistance, double homeFocalDistance, int screenspeed)
{
    ...
    int nstepsX = (int)(focalDistance-homeFocalDistance)/(BROWN_MOTORSTEPSIZE);
    string buffer("F,C,S"+stringify<int>(BROWN_SCREEN_X_AXIS)+"M"+stringify<int>(screenspeed)+
                  ",IA"+stringify<int>(BROWN_SCREEN_X_AXIS)+"M"+stringify<int>(nstepsX)+",R");
    cerr << "Sending:" << buffer << endl;
    serial << buffer << endl;
    char out=0;
    serial >> out;
    if ( out=='^' )
        ...
        cerr << "Error in moveScreenAbsolute()" << endl;
#endif
}
...
{
#if SIMULATION
    cerr << "moveScreenAbsoluteAsynchronous" << endl;
#else
...
{
#if SIMULATION
    cerr << "homeScreenAsynchronous" << endl;
#else
...
void moveObject(const Eigen::Vector3d &delta, int speed)
{
...
#else
    cerr << "moveObject " << delta.transpose() << endl;
    if ( delta.norm() > 1e20 )
    {
        std::cerr << "Impossible position to reach! Check marker visibility!" << endl;
        return;
    ...
    Eigen::Vector3d displacement(0,-delta.y(),delta.z());
    VmxLinearMotor objectMotor;
    objectMotor.moveSteps(BROWN_MOTORS_COM_PORT, BROWN_OBJECT_Y_AXIS ,displacement.y(),speed,BROWN_MOTORSTEPSIZE);
    objectMotor.moveSteps(BROWN_MOTORS_COM_PORT, BROWN_OBJECT_Z_AXIS ,displacement.z(),speed,BROWN_MOTORSTEPSIZE);
#endif
}
...
{
#if SIMULATION
    cerr << "moveScreen "<< delta.transpose() << endl;
#else
    if ( delta.norm() > 1e20 )
    {
        std::cerr << "Impossible position to reach! Check marker visibility!" << endl;
...
{
#if SIMULATION
    cerr << "moveObjectAndScreen" << objectDelta.transpose() << " " << screenDelta.transpose() << " @" << objectspeed << endl;
#else
