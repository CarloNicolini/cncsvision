    ...
    QGLWidget( QGLFormat(QGL::SampleBuffers), parent)
{
    ...
    screen.init(300,300.0*768.0/1024.0,0.0,0.0,750);
    vrcam.init(screen,true);
...
void VolumetricHelicoidWidget::initializeGL()
{
    ...
    void main()
    {
        ...
        float parametricSurfaceEquation = 240.0*(pvertex.x*pvertex.x)/(240.0*240.0);
        if ( (pvertex.z < parametricSurfaceEquation+thickness) && (pvertex.z > parametricSurfaceEquation-thickness) )
        {
        ...
        if ( ((pvertex.x*pvertex.x) + (pvertex.z*pvertex.z)) <= 120.0 )
            gl_FragColor = vec4(0.0);
    }
    );
    volume->initializeSurfaceShaders(vertexShader,fragmentShader);
    volume->meshStruct.shader->setUniform1f("thickness",0.5);
    ////////////////////////////
 ...
 * @param val
 */
void VolumetricHelicoidWidget::setEyeZ(double val)
{
    this->eyeZ=val;
}
...
*/
    ...
    cam = new CameraDirectLinearTransformation(points2Dfilename.toStdString(),points3Dfilename.toStdString(),decomposePMatrix,computeOpenGLMatrices,0,0,width,height,this->zNear,this->zFar);
