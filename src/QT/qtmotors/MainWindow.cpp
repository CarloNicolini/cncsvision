...
{
    string buffer = string("F,C,")+string("S2M")+stringify<int>((ui->boxMonSpeedX->value())>2000 ? 2000 : ui->boxMonSpeedX->value())+
            string(",I2M0,IA2M-0,R");

    sendCommand(MONITOR_SERIAL_PORT,buffer);
}

void MainWindow::on_pushButtonHomeMonZ_clicked()
{
    string buffer = string("F,C,")+string("S1M")+stringify<int>((ui->boxMonSpeedZ->value())>2000 ? 2000 : (ui->boxMonSpeedZ->value()))+
...
{
    string buffer = string("F,C,")+string("S1M")+stringify<int>(ui->boxMirrSpeedX->value())+string(",S2M")+stringify<int>(ui->boxMirrSpeedZ->value())+
            string(",I1M0,I2M0,IA1M-0,IA2M-0,R");

    sendCommand(MIRROR_SERIAL_PORT,buffer);
}
...
void MainWindow::on_pushButtonMoveMonAbs_clicked()
{
    ...
    {
        buffer +=
                string("S2M")+stringify<int>(ui->boxMonSpeedX->value())+
                string(",IA2M")+stringify<int>(nStepsX)+string(",");
    }
    if (nStepsZ!=0)
    {
        buffer +=
                string("S1M")+stringify<int>(ui->boxMonSpeedZ->value())+
...
void MainWindow::on_pushButtonMoveMonRel_clicked()
{
    ...
    if (nStepsX==nStepsZ && nStepsX==0)
        return;
    string buffer("F,C,");
    buffer+= string("S2M")+stringify<int>(ui->boxMonSpeedX->value())+","+string("S1M")+stringify<int>(ui->boxMonSpeedZ->value());
    if (nStepsX!=0)
    {
        buffer +=
                string(",I2M")+stringify<int>(nStepsX)+string(",");
    }
    if (nStepsZ!=0)
    {
        buffer +=
...
{/*
    ...
    SerialStream serial(MONITOR_SERIAL_PORT);    // bleah it's very unelegant!!
    char Z[10],Y[10],X[10];
    try
    {
...
{
    // XXX controllare!
    int nStepsX = ui->boxMirrAbsIndexX->value();
    ...
    {
        buffer +=
                string("S2M")+stringify<int>(ui->boxMirrSpeedX->value())+
                string(",IA2M")+stringify<int>(nStepsX)+string(",");
    }
    if (nStepsZ!=0)
    {
...
{
    // XXX controllare!
    int nStepsX = ui->boxMirrRelIndexX->value();
...
void MainWindow::on_pushButtonHomeMirrX_clicked()
{
    int speed  = ui->boxMirrSpeedX->value();
    string buffer = string("F,C,")+string("S2M")+stringify<int>(speed>2000 ? 2000 : speed)+
            string(",I2M0,IA2M-0,R");
...
void MainWindow::on_pushButtonReadMirr_clicked()
{
    ...
    SerialStream serial(MIRROR_SERIAL_PORT);    // bleah it's very unelegant!!
    string Z,X,Y;
    try
    {
    // Home object on Z axis
...
void MainWindow::on_pushButtonMoveObj_clicked()
{
    ...
    string buffer("F,C,(");
    buffer+=
            string("S1M")+stringify<int>(ui->boxObjSpeedZ->value())+
            string(",S2M")+stringify<int>(ui->boxObjSpeedY->value())+
            string(",S3M")+stringify<int>(ui->boxObjSpeedX->value());

    if (nstepsX!=0)
        buffer+= string(",IA3M")+stringify<int>(nstepsX);
    if (nstepsY!=0)
        buffer+= string(",IA2M")+stringify<int>(nstepsY);
    if (nstepsZ!=0)
        buffer+= string(",IA1M")+stringify<int>(nstepsZ);
    ...
    sendCommand(OBJECT_SERIAL_PORT,buffer);
}
...
void MainWindow::on_pushButtonMoveObjRel_clicked()
{
    ...
    if (nstepsX!=0)
    {
        buffer+= string(",I3M")+stringify<int>(nstepsX);
    }
    if (nstepsY!=0)
        buffer+= string(",I2M")+stringify<int>(nstepsY);
    if (nstepsZ)
        buffer+= string(",I1M")+stringify<int>(nstepsZ);
...
void MainWindow::on_pushButtonReadObj_clicked()
{
    ...
    SerialStream serial(OBJECT_SERIAL_PORT);    // bleah it's very unelegant!!
    string Z,X,Y;
    try
    {
    // Home object on Z axis
...
void MainWindow::on_pushButtonHomeObjZ_clicked()
{
    ...
    string buffer = string("F,C,")+string("S1M")+stringify<int>(speed > 4000 ? 4000 : speed)+
            string(",I1M-0,IA1M-0,R");
...
void MainWindow::on_homeObject_clicked()
{
    ...
    string buffer = string("F,C,")+string("S1M")+stringify<int>(speedz>4000 ? 4000 :speedz )+string(",S2M")+stringify<int>(speedy>4000 ? 4000 :speedy)+string(",S3M")+stringify<int>(speedx>4000 ? 4000 :speedx)+
...
void MainWindow::on_boxMirrRelmmX_valueChanged(double val)
{
        int d = val*200;
    ui->boxMirrRelIndexX->setValue(-d);
}
...
void MainWindow::on_boxMirrAbsmmX_valueChanged(double val)
{
        int d = val*200;
    ui->boxMirrAbsIndexX->setValue(-d);
}
...
void MainWindow::resetAllBoxes()
{
    ...
    ui->boxMirrRelmmX->setValue(0);
    ui->boxMirrRelIndexX->setValue(0);
    ui->boxMirrAbsmmX->setValue(0);
    ui->boxMirrAbsIndexX->setValue(0);
