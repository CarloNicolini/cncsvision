...
#ifndef _PARABOLIC_CYLINDERPOINTS_STIMULUS_H
#define _PARABOLIC_CYLINDERPOINTS_STIMULUS_H
...
class ParabolicCylinderPointsStimulus : public PointwiseStimulus
{
private:
    double domainRadiusXZ;
    double curvatureY;

public:
   ParabolicCylinderPointsStimulus();
   ~ParabolicCylinderPointsStimulus();

   void setDomainXZ(double domainRadius);
   void setCurvature(double curvatureY);

   // The method to override which are defined in the interface
   void compute();

   int getStimulusType()
   {  return Stimulus::StimParaboloidPointwise ;
   }
