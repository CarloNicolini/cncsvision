...
{  randomizeStart();
   ...
   {  Staircase *s = parStairCase.getCurrentStaircase();
      bool isHorizontal = unifRand(0.0,1.0) >= 0.5;
      char stimtype;
      if ( isHorizontal )
         stimtype='h';
      else
      ...
      cerr << fixed << "Staircase" << s->getID() << ":\t" << id2string[s->getID()] << "\t" << stimtype << "\tDistance " << s->getState() << "\tResponse? (1 or 2) " ;
      char answer;
      do
      {  cin >> answer;
      }
      ...
      bool response = (isHorizontal) == ( answer=='1');
