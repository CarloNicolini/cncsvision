...
void initRendering()
{
    ...
    void main()
    {
        ...
        {
            finalColor = vec4(0.0,0.0,0.0,1.0); //Can be uniformColor to color again the thing
        }
        else
        {
            finalColor = uniformColor*texture3D(my_color_texture, texture_coordinate);
        }

        float parametricSurfaceEquation = 100.0*(pvertex.x*pvertex.x)/(240.0*240.0);
        ...
        {
            finalColor.rgb = vec3(0.1);
            gl_FragColor = finalColor;
            //gl_FragColor = vec4(1.0,0.0,0.0,1.0);
        }
    }
    );
    surface.initializeSurfaceShaders(vertexShader,fragmentShader);
    surface.meshStruct.shader->setUniform1f("thickness",0.5);
