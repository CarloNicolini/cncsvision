...
const double density::MinGammaPar = pow(2.0, -128.0);
const double density::MaxGammaPar = pow(2.0, 128.0);

namespace {

  inline double rd0(const bool give_log) {
    return give_log ? log(std::numeric_limits<double>::min()) : 0.0;
  }
...
#undef M_PI
#endif
  static const double M_PI = 3.141592653589793238462643383280;
  static const double M_2PI = 6.283185307179586476925286766559;
  static const double M_LN_SQRT_2PI = 0.918938533204672741780329736406;
  ...
  void dpsifn(double x, int n, int kode, int m, double *ans, int *nz,
    ...
    *nz = 0;
    ...
    r1m5 = log10(2.0);
    r1m4 = pow(fltRadix, (double)(1 - dblMantDig)) * 0.5;
                ...
                const bool give_log, const bool include_const) 
{
  ...
  for ( int k = 0; (k < nElem) && (l > log(std::numeric_limits<double>::min())); k++) {
    l += (a[k] - 1.0) * log(p[k]);
  }
...
/// \return          0 or log(std::numeric_limits<double>::min()) if x < 0
///
/// \f[ \mbox{E}(x) = \mbox{b} \f]
                ...
                const bool give_log, const bool include_factorial) 
{
  ...
  for (unsigned k = 0; k < nElem; k++) {
    ...
    if (p[k] >= std::numeric_limits<double>::min()) {
      l += n[k] * log(p[k]);
    } else {
      l += n[k] * log(std::numeric_limits<double>::min());
    }
  }
  return ((give_log) ? l : exp(l));
}
