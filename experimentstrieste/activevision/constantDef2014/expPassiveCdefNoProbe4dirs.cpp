...
void drawProbe()
{
    ...
    glBegin(GL_LINES);
    glVertex3f(0,0,focalDistance);
    glVertex3f(probeRadius*cos(toRadians(probeAngle+probeStartingAngle) ), probeRadius*sin(toRadians(probeAngle+probeStartingAngle)),focalDistance);
    glEnd();
    glColor3fv(glRed);
...
void update(int value)
{
    ...
    if ( trialMode != PROBEMODE )
    {
        ...
        variable = -factors.at("Onset")*mathcommon::trianglewave( timeFrame , factors.at("StimulusDuration")/(TIMER_MS*factors.at("FollowingSpeed")) );
