...
void idle()
{
    ...
    allVisibleHead =  isVisible(markersRealTime.at(18).p) && allVisiblePatch;
...
void readPassiveCalibrations(ifstream &markersFile)
{
    ...
    while ( headCalibrationFromFileDone < 3 )
    {
        ...
        if ( headCalibrationFromFileDone == 1 && oldHeadCalibration==0 )
        {
            ...
            headCoordsFromFile.init( markersFromFile.at(18).p-Vector3d(ChinRestWidth,0,0),markersFromFile.at(18).p, markersFromFile.at(1).p,markersFromFile.at(2).p,markersFromFile.at(3).p,interocularDistance );
...
void keyPressEvent(unsigned char key, int x, int y)
{
    ...
    case ' ':
    {
        ...
        case 0:
        {
            if (allVisibleHead && allVisiblePatch )
            {
                headCoordsRealtime.init( markersRealTime.at(18).p-Vector3d(ChinRestWidth,0,0),markersRealTime.at(18).p,	markersRealTime.at(1).p,markersRealTime.at(2).p,markersRealTime.at(3).p,interocularDistance);
                //headCoordsRealtime.init(markersRealTime.at(18).p,	markersRealTime.at(1).p,markersRealTime.at(2).p,markersRealTime.at(3).p,interocularDistance);
