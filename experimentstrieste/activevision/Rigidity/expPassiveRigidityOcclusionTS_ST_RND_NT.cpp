...
{
    Affine3d plane0 = objectActiveTransformation[0];
    Affine3d plane1 = objectActiveTransformation[1];
    plane0.translation() << (screen.pb + screen.pc + screen.pd + screen.pa)/4;
    plane1.translation() << (screen.pb + screen.pc + screen.pd + screen.pa)/4;

    plane0.linear().setIdentity();
    glPushMatrix();     // PUSH MATRIX
    glLoadIdentity();
    glMultMatrixd(plane0.data());
    glTranslated(0,0,str2num<double>(parameters.find("OccluderDistance")));
...
{
    // La logica della trasformazione desiderata è questa:
    // 1. copio la trasformazione originale dell'oggetto in attivo nell'oggetto plane0
    // 2. modifico la trasformazione plane0 dandogli come traslazione il centro dello schermo, in questo caso nel passivo
    // risulterà essere ferma al centro
    // 3. mantenendo inalterata la parte lineare (rotazione) di plane0 applico la trasformazione
    // 4. nel nuovo sistema di coordinate effettuo una traslazione per i due piani esattamente pari a quella che si farebbe nell'attivo
    // lungo gli assi X', Y' ma mantengo inalterata la traslazione su Z
    // 5. faccio la correzione con backprojection con glScaled
    Affine3d plane0 = objectActiveTransformation[0];
    Affine3d plane1 = objectActiveTransformation[1];
    plane0.translation() << (screen.pb + screen.pc + screen.pd + screen.pa)/4;
    plane1.translation() << (screen.pb + screen.pc + screen.pd + screen.pa)/4;
    ...
    glMultMatrixd(plane0.data());
    glTranslated(objectActiveTransformation[0].translation().x(),objectActiveTransformation[0].translation().y(),0);
    ...
    glPushMatrix();     // PUSH MATRIX
    glLoadIdentity();
    glMultMatrixd(plane1.data());
    glTranslated(objectActiveTransformation[1].translation().x(),objectActiveTransformation[1].translation().y(),0);
...
void drawProbe()
{
    ...
    glColor3fv(glRed);
    glBegin(GL_POINTS);
    glVertex3d(-10.25,-16.25,focalDistance);
    glEnd();
    glPopAttrib();
}
    ...
    case STIMULUSMODE:
    {
        ...
        */
        ...
        if ( str2num<int>(parameters.find("Occluder")))
            ...
            rectangle(str2num<double>(parameters.find("OccluderWidth")),str2num<double>(parameters.find("OccluderHeight")),focalDistance ); /// Added: Draw an occluding rectangle infront of the twsited structure
