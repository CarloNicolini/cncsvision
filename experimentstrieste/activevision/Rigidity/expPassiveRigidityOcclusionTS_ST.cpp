...
double focalDistance= baseFocalDistance-100.0;
...
{   switch ( trialMode )
    ...
    case STIMULUSMODE:
    {
        ...
        num_dots= int(floor((x_dim*str2num<double>(parameters.find("planeYdimension")))*(str2num<int>(parameters.find("numberdots"))/(str2num<double>(parameters.find("planeXdimension"))*str2num<double>(parameters.find("planeYdimension"))))));
        redDotsPlane[0].setNpoints(num_dots);  //XXX controllare densita di distribuzione dei punti
        redDotsPlane[0].setDimensions(x_dim,str2num<double>(parameters.find("planeYdimension")),0.01);
        x_dim= str2num<double>(parameters.find("planeXdimension")) + mathcommon::unifRand(0,100);
        num_dots= int(floor((x_dim*str2num<double>(parameters.find("planeYdimension")))*(str2num<int>(parameters.find("numberdots"))/(str2num<double>(parameters.find("planeXdimension"))*str2num<double>(parameters.find("planeYdimension"))))));
        redDotsPlane[1].setNpoints(num_dots);  //XXX controllare densita di distribuzione dei punti
        redDotsPlane[1].setDimensions(x_dim,str2num<double>(parameters.find("planeYdimension")),0.01);
...
{   if ( is.eof() )
    ...
    is >> factors["StillPlaneSlant"] >>  factors["DeltaSlant"] >>  factors["PlanesCentersYDistance"] >>  alphaMultiplier >>
       factors["Onset"] >>  whichPlaneDrawUp >> currentStaircaseIsAscending >> overallTilt;
...
{   // Load the parameters file
    string parametersFileName = "C:/cncsvisiondata/parametersFiles/parametersActiveTwoPlanesOcc.txt";
...
{   interoculardistance = str2num<double>(parameters.find("IOD"));
    ...
    for (int i=0; i<2; i++)
    {
        ...
        redDotsPlane[i].setDimensions(x_dim,str2num<double>(parameters.find("planeYdimension")),0.01);
        redDotsPlane[i].compute();
