...
static const double focalDistance= baseFocalDistance-200.0;
...
void rectangle(void)
{
    ...
    glBegin(GL_POLYGON);
    glVertex3f (0.0, 0.0, focalDistance);
    glVertex3f (10.0, 0.0, focalDistance);
    glVertex3f (10.0, 10.0, focalDistance);
    glVertex3f (0.0, 10.0, focalDistance);
    glEnd();
    glPopAttrib();
    glFlush ();
}
...
void initVariables()
{
    ...
    for (int i=0; i<2; i++)
    {
        redDotsPlane[i].setNpoints(100);  //XXX controllare densita di distribuzione dei punti
...
void initStreams()
{
    string parametersFileName = "C:/cncsvisiondata/parametersFiles/parametersActiveTwoPlanes.txt";
...
{   switch (key)
    ...
    case '4':
    {
        if ( trialMode == PROBEMODE )
        {
            ...
            if( (int) factors.at("OverallTilt") == 0 )
                answer=false;
            else
                answer=true;
