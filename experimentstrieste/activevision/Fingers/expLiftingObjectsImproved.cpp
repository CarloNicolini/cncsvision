...
void drawGLScene()
{
    ...
    if ( gameMode )
        ...
        text.init(640,480,glWhite,GLUT_BITMAP_HELVETICA_12);
    text.enterTextInputMode();
    text.draw("LONTANO/VICINO=" + stringify<int>((int)trial.getCurrent().at("Distance")));
    text.leaveTextInputMode();
...
{   switch (key)
    ...
    case 'a':
    {
        cout << trial.getRemainingTrials() << endl;
        break;
    }
    ...
    case 13:
    {
        ...
        if (trial.isEmpty() && isSavingMarkers==true)
            exit(0);
...
void idle()
{
    ...
    if ( headCalibrationDone==3 && fingerCalibrationDone==3 && isSavingMarkers )
    {
        if ( totalFrame==0)
        {
            markersFile << fixed << setprecision(3) <<
                        "SubjectName\tfingerDistance\tTrialNumber\tFrame\tDeltaT\tTime\tInside\tDistance\tEyeLeftXraw\tEyeLeftYraw\tEyeLeftZraw\tEyeRightXraw\tEyeRightYraw\tEyeRightZraw\tWristXraw\tWristYraw\tWristZraw\tObjXraw\tObjYraw\tObjZraw\tThumbXraw\tThumbYraw\tThumbZraw\tIndexXraw\tIndexYraw\tIndexZraw" << endl;
            totalFrame++;
        }
        markersFile << fixed << setprecision(3) <<
                    ...
                    ( isVisible(markers.at(19).p) ? markers.at(19).p.transpose() : junk) << "\t" << //// Wrist
                    ( isVisible(markers.at(20).p) ? markers.at(20).p.transpose() : junk) << "\t" << /// Object
    ...
    if ( headCalibrationDone==3 && fingerCalibrationDone==3) //&& isSavingMarkers )
    {
        ...
        outputfile << "Wrist: " << markers.at(19).p.transpose() << endl;
        outputfile << "Object: " << markers.at(20).p.transpose() << endl;
        outputfile << "Distance: "<< trial.getCurrent().at("Distance")<<endl;
        outputfile << "Time: "<< globalTimer.getElapsedTimeInMilliSec()<<endl;
        outputfile << "Time: "<<deltaT*1000 << "\t" <<endl;
        outputfile << "SAVE: "<< isSavingMarkers<<endl;
#endif
    }
