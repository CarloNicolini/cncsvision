...
void main() {
    ...
    light_vector  = tbni * (inverse(Model) * vec4(gl_LightPos[0] - v.xyz, 0)).xyz;  // light vector in tangent space

    v = View * Model * vec4(vertex, 1.0);                       // vertex in eye space
    vec4 light_vector_eye  = normalize((View * vec4(gl_LightPos[0], 1.0)) - v); // light vector in eye space
